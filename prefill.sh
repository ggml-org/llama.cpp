...
# prepare examples
cp pyproject.toml base.txt
echo "Question: What is the numpy version?" > query.txt
cat base.txt query.txt >full.txt

# precompute mypy.ini into prefill.save
rm prefill.save
./llama-cli -m ~/funstreams/granite-3.1-1b-a400m-instruct-IQ4_NL.gguf -c 2000 -st --prompt-cache prefill.save -f base.txt -n 1
