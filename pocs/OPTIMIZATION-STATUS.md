...
| DFlash2 width-six rows/block=2 | Explicit `GGML_HIP_GFX1030_DFLASH_MMVQ_ROWS2=1` route for gfx1030 Q4_0/Q4_K/Q6_K width-six MMVQ; unset/`0`, unsupported shapes/types, and global auto-off retain prior dispatch | With backend sampling active: `72.717 -> 77.783 tok/s` (**+6.97%**), cycle `51.872 -> 48.554 ms`; target `-2.317 ms`, draft `-1.064 ms`. Raw-F32 synthetic gates and Q4/Q8 long-output, reasoning, grammar, cache, probabilities, parallel, cancellation, and repeated-request checks passed. Retained as opt-in; representative mean `83.85 vs 44.74` AR (`1.87x`), not a sustained 2x claim. |
...
| DFlash2 2x sprint alternatives | Width-six rows/block=4, Q4_0 eight-wave, generic helper/shape-gated rows2, `p_split` 0/.1/.2, forced Ring/LL or one-channel/128-thread RCCL, and guessed width-two P2P schedules | Rows4 was slower; eight-wave failed `4916/24576` raw-F32 values; generalized helpers erased the end-to-end gain; `p_split` was neutral; RCCL overrides regressed to `71.49–73.81 tok/s` versus `77.55` default (Ring/LL also changed output/acceptance); both P2P schedule guesses failed exact startup self-tests and fell back. Removed or never promoted. |
...
- DFlash2 sustained 2x remains open. The retained n5 sampler+rows2 candidate measures `77.783 tok/s` at `48.554 ms/cycle` versus a `89.784 tok/s` / `41.477 ms` target. Continue only exact TP4 M6 work with enough ceiling to remove roughly `7 ms/cycle`: skinny-GEMM/weight reuse, collective scheduling or fusion, LM-head reduction, or GPU-side acceptance. Do not retry single-GPU verification, naïve chunking, the rejected RCCL/P2P schedules above, or sub-5% orchestration micro-optimizations.
