...
| DFlash2 2x sprint alternatives | Width-six rows/block=3/4, Q4_0 eight-wave, generic helper/shape-gated rows2, `p_split` tuning, forced RCCL settings, host-snapshot width changes, and gate+up+GLU fusion | Rows3 was exact but dominant Q4_K regressed `16.48%`; rows4 was slower; eight-wave failed `4916/24576` raw-F32 values; helpers erased the gain; `p_split` was neutral; RCCL overrides regressed. The old width-two P2P POC targeted a nonexistent payload; a corrected exact width-six route saved only `0.347 ms/cycle` (`+0.738%`). Width-six GLU fusion raised VGPR `64 -> 96` and regressed DFlash `4.63%`. Removed or never promoted. |
