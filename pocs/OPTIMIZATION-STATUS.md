...
| gfx1030 TP4 MTP greedy target sampling | Automatic backend target sampling for the exact structural MTP `n_max=4`, large-vocabulary, neutral temperature-zero case; explicit request choice and auto-disable paths are preserved | Full-vocabulary CPU target sampling cost `~5.64 ms/cycle`. Fresh production-tree A/B/B/A improved `65.7854 -> 75.9478 tok/s` (**+15.45%**) and cycle time `45.345 -> 39.356 ms`, with exact output and acceptance; 1,024-token `73.4756 -> 85.7679 tok/s`; prompt-cache, varied-prompt stress, graph, fallback, and parallel-slot checks passed. Q4_0 verified; implementation is quant-independent. Evidence: `/home/edwin/.ralph/speculative-mtp-target-backend-production-abba-20260821T211038Z`. |
...
| TP4 ordinary 5120-FP32 host-snapshot expansion + consumer fusion | `GGML_HIP_GFX1030_P2P_ALLREDUCE=auto-expanded` extends the existing exact host-snapshot AllReduce to ordinary `linear_attn_out-*`, `ffn_out-*`, and `attn_output-*` with exact contiguous `[5120,1,1,1]` F32 and `RESHAPE -> ADD -> RMS_NORM -> MUL` graph-prefix guards; misses fall back to the existing host/RCCL paths; unset/`auto` is unchanged | Fresh host expansion: `50.645675 -> 53.552075 tok/s` (`+5.7387%`). Integrated consumer-fused candidate versus production control: `53.4202 -> 53.8177 tok/s` (`+0.744%` mean, `+0.742%` median); deterministic long/cache/grammar/FA/graph/fallback matrix exact/healthy. Ordinary-only; MTP/DFlash are structurally unaffected because the fused prefix requires `ne[1]=1`. Evidence: `/home/edwin/.ralph/gfx1030-host-fused-poc/FINAL-REPORT.md`. |
