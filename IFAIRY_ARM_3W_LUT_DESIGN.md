...
### H) 已确认的困难与修复方向（重要）
1) **“strict 仍乱码”意味着不是 LUT 近似误差**  
   当前 `strict` 路径是逐权重解码并应用 per-block 缩放的数学重建（不依赖 LUT 表的 int8 饱和与查表），但 `llama-cli` 仍乱码，说明问题更可能是 **路由语义/形状一致性问题**（而非简单的 LUT 溢出或饱和）。

2) **方案 A 的“drop 每 block 最后一个权重”会改变模型**  
   为了 255=85*3，当前实现通过丢弃每个 256-block 的最后一维权重（相当于强制把该维权重置 0）。这会让线性层不再等价于原模型，理论上足以让输出不可读。单测能 0 diff，是因为单测 reference 也同步跳过了该元素，因此无法证明对真实模型正确。

3) **更合理的 A'（不改变模型）**  
   方案 A 的目标是“分组不跨 block”，但不应丢弃真实维度。更合理的实现是：
   - 在每个 block 内生成 85 个三元组覆盖 255 个元素；
   - 对剩余 1 个元素，生成一个“尾组”并在预处理/LUT/qgemm 中只累加有效元素（缺失项视作 0），保证与 vec_dot 完全等价。
   这样既保持 per-block 缩放一致，又不修改模型语义。

4) **方案 B（每组三独立缩放）作为过渡修复**  
