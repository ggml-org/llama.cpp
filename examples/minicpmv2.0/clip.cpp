// NOTE: This is modified from clip.cpp only for an older version of LLaVA,
// there might be still unnecessary artifacts hanging around
// I'll gradually clean and adapt it to the latest version 
// Note: Even when using identical normalized image inputs (see normalize_image_u8_to_f32()) there exists a significant difference in resulting embeddings compared to pytorch
#include "clip.h"
#include "log.h"
#include "ggml.h"
#include "ggml-cpu.h"
...
#   define LOG_TEE(...) do { fprintf(stdout, __VA_ARGS__); } while (0)
...
struct clip_ctx * clip_model_load(const char * fname, const int verbosity = 1) {
    ...
    if (new_clip->has_vision_encoder) {
        ...
        try {
            ...
            vision_model.post_ln_w           = get_tensor(new_clip->ctx_data, format(TN_LN_POST, "weight"));
            vision_model.post_ln_b           = get_tensor(new_clip->ctx_data, format(TN_LN_POST, "bias"));
        } catch(const std::exception& e) {
