...
if not defined TRAIN_FILE set TRAIN_FILE=examples\qlora_training\zorvian_focus_data.jsonl
if not defined LORA_OUT set LORA_OUT=examples\qlora_training\zorvian_focus_adapter.gguf
if not defined EPOCHS set EPOCHS=10
if not defined LR set LR=1e-4
if not defined LORA_RANK set LORA_RANK=16
if not defined LORA_ALPHA set LORA_ALPHA=16
@REM Tiny repetitive datasets can leave Adam state pushing against the latest adapter weights.
@REM The default restart interval is intentionally aggressive for this tiny repro.
@REM It keeps the weights and drops only optimizer state,
@REM matching the useful part of manually stopping and resuming training.
if not defined OPT_RESTART_EVERY set OPT_RESTART_EVERY=1
if not defined RESUME set RESUME=0
if not defined SHUFFLE set SHUFFLE=0
