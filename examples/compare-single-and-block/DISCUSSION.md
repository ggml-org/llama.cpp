...
Hope you can test the difference on your own backend and share your results. How to run: [README.md](README.md).
...
Method details are in [README.md](README.md). Two things keep the comparison valid:
...
Data flow of the pos-14 flip from the CPU run (window `t = 14`, offset `j = 0`):

```
        prefix = prompt + AR[0..13]  ("… in geometry that")
        (bit-identical KV on both contexts after the re-prefill)
                                |
          +---------------------+----------------------+
          | single path         | block path           |
          | decode ['that']     | decode ['that',      |
          | as one 1-token call |  'relates',' the',   |
          | (GEMV-shaped)       |  'sides',' of'] as   |
          |                     | one 5-token call     |
          |                     | (GEMM-shaped)        |
          | relates   31.3991   | relates   31.4009    |
          | describes < 31.3991 | describes 31.4210    |
          +---------------------+----------------------+
          argmax: relates       argmax: describes  ==>  FLIP
          (matches AR baseline: relates 31.4505)
```
...
3. Can others reproduce this run (same prompt, Qwen3-4B bf16, `W=5`, `-n 100`) on your backend and share the pairwise stats / flip count? How to run: [README.md](README.md).
