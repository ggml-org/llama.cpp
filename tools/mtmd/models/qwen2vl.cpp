...
ggml_tensor * clip_graph_qwen2vl::build_inp_with_temporal_merge() {
    ...
    if (n_batch == 1) {
        ...
        return ggml_add(ctx0,
            ggml_conv_2d(ctx0, resolve_weight(model.patch_embeddings_0), inp_raw, patch_size, patch_size, 0, 0, 1, 1),
            ggml_conv_2d(ctx0, resolve_weight(model.patch_embeddings_1), inp_raw, patch_size, patch_size, 0, 0, 1, 1));
    } else if (n_batch == 2) {
        ...
        return ggml_add(ctx0,
            ggml_conv_2d(ctx0, resolve_weight(model.patch_embeddings_0), inp_0, patch_size, patch_size, 0, 0, 1, 1),
            ggml_conv_2d(ctx0, resolve_weight(model.patch_embeddings_1), inp_1, patch_size, patch_size, 0, 0, 1, 1));
    } else {
