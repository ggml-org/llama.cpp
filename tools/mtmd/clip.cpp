    ...
    ggml_cgraph * build_jina2() {
        ...
        for (int il = 0; il < max_feature_layer; il++) {
...

            {
                ...
                // Apply 2D RoPE position encoding for JinaCLIP (skip CLS).
                ggml_tensor * Q_cls =
                ...
                cur = build_attn(layer.o_w, layer.o_b, Qcur, Kcur, Vcur, nullptr,
                                 kq_scale, il);
            }
...
private:
    ...
    ggml_tensor * build_attn(
        ...
        if (il >= 0 && il < (int) model.layers.size()) {
            auto & layer = model.layers[il];
            attn_ln_w = layer.attn_ln_w;
            attn_ln_b = layer.attn_ln_b;
        }
    ...
    ggml_tensor * build_jinaclip_rope(ggml_context * ctx0,
        ...
        const int64_t n_pos_patches  = cur->ne[1];
...
struct clip_model_loader {
    ...
    void load_tensors(clip_ctx & ctx_clip) {
        ...
        for (int il = 0; il < hparams.n_layer; ++il) {
            ...
            layer.attn_ln_w = get_tensor(string_format(TN_ATTN_LN, prefix, il, "weight"), false);
            layer.attn_ln_b = get_tensor(string_format(TN_ATTN_LN, prefix, il, "bias"), false);
            ...
            case PROJECTOR_TYPE_JINACLIP2:
                {
                    ...
                    // No additional mm projection tensors are loaded for JinaCLIP2
                } break;
            default:
