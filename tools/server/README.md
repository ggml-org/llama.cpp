...
| `--tools TOOL1,TOOL2,/path/to/TOOLS.md,...` | experimental: whether to enable built-in tools for AI agents - do not enable in untrusted environments (default: no tools)<br/>specify "all" to enable all tools or a Markdown file to load user tools from it.<br/>available tools: read_file, file_glob_search, grep_search, exec_shell_command, write_file, edit_file, apply_diff, get_datetime, execute_user_tools<br/>(env: LLAMA_ARG_TOOLS) |
