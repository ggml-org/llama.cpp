...
| `--tensor-read-lazy MODE` | on-demand reading of certain tensors, for example per-layer embeddings (default: auto)<br/>- on: on-demand rows for tensors marked by the architecture (requires mmap)<br/>- auto: only tensors larger than 4 GiB<br/>- off: always read them up front<br/>(env: LLAMA_ARG_TENSOR_READ_LAZY) |
