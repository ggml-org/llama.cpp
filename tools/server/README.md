...
While the server sleeps, `/metrics` is served from the snapshot taken before sleeping. The `memory_*` values then describe the model that was loaded, not memory in use. See [Sleeping on idle](#sleeping-on-idle).

### GET `/memory`: Get the per-device memory breakdown as JSON

The same figures as the `memory_*` metrics above, for API consumers. Requires `--memory`. Unlike `/metrics`, this endpoint reads the live model, so it wakes a sleeping server.
