...
| `-lzm, --lazy-mode MODE` | on-demand reading of certain tensors, for example per-layer embeddings (default: auto)<br/>- on: read the rows of such tensors from disk on demand instead of keeping them resident<br/>- auto: on when using an integrated GPU, otherwise on only for tensors larger than 4 GiB<br/>- off: always keep them resident<br/>- --check-tensors and --load-mode mmap+mlock load them in full<br/>(env: LLAMA_ARG_LAZY_MODE) |
