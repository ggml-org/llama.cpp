...
| `-lzm, --lazy-mode MODE` | on-demand reading of certain tensors, for example per-layer embeddings (default: auto)<br/>- on: read the rows of such tensors from disk on demand instead of keeping them resident<br/>- auto: on only for tensors larger than 4 GiB<br/>- off: always keep them resident<br/>note: --check-tensors and --load-mode mmap+mlock force this option to 'off'<br/>(env: LLAMA_ARG_LAZY_MODE) |
