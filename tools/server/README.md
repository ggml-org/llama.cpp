...
| `--ctx-pool-slots N` | slots worth of context to provision in the shared `--ctx-per-slot` pool (default: all `n_parallel` slots).<br/>with unified KV, values below `n_parallel` overcommit the pool<br/>(env: LLAMA_ARG_CTX_POOL_SLOTS) |
