...
| `-kvu, --kv-unified, -no-kvu, --no-kv-unified` | use single unified KV buffer shared across all sequences - enables the block-radix prefix cache with zero-copy cross-slot KV sharing (default: enabled; use --no-kv-unified for the legacy per-sequence cache with serialized prompt snapshots)<br/>(env: LLAMA_ARG_KV_UNIFIED) |
