...
static inline raw_buffer base64_decode(const std::string & encoded_string) {
    raw_buffer ret;
    ret.reserve(base64::max_decode_size(encoded_string.length()));
    try {
        base64::decode(encoded_string.begin(), encoded_string.end(), std::back_inserter(ret));
    } catch (const base64_error & e) {
        SRV_DBG("base64_decode failed: %s\n", e.what());
        ret.clear();
    }
    return ret;
}
