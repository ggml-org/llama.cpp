...
export function generateModalityErrorMessage(
    ...
    const imageFiles = unsupportedFiles.filter(f => getFileTypeCategory(f.type) === FileTypeCategory.IMAGE);
    const audioFiles = unsupportedFiles.filter(f => getFileTypeCategory(f.type) === FileTypeCategory.AUDIO);
...
export function generateModalityAwareAcceptString(): string {
    ...
    if (hasVision) {
        acceptedExtensions.push(...Object.values(ImageExtension));
        acceptedMimeTypes.push(...Object.values(ImageMimeType));
    }
    
    // Include audio only if audio is supported
    if (hasAudio) {
        acceptedExtensions.push(...Object.values(AudioExtension));
        acceptedMimeTypes.push(...Object.values(AudioMimeType));
    }
