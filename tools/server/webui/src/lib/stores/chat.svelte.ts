...
const FALLBACK_CONTEXT_SIZE = 4096;
...
class ChatStore {
	...
	private async createAssistantMessage(
		...
		const targetConvId = convIdOverride || conversationsStore.activeConversation?.id;
	...
	private async processToolCallsAndContinue(
		...
		const sourceConvId = sourceAssistant.convId;

		const toolMessages: DatabaseMessage[] = [];
		let parentForNextTool: string = sourceAssistant.id;
		const shouldUpdateActiveConversation =
			conversationsStore.activeConversation?.id === sourceConvId;
		...
		this.setChatLoading(sourceConvId, true);
		this.clearChatStreaming(sourceConvId);

		const allSourceMessages = await conversationsStore.getConversationMessages(sourceConvId);
		const sourcePathWithAssistant = filterByLeafNodeId(
			allSourceMessages,
			newAssistant.id,
			false
		) as DatabaseMessage[];
		...
		await this.streamChatCompletion(
			sourcePathWithAssistant.slice(0, -1),
			newAssistant,
			undefined,
			undefined,
			modelOverride || null
