...
bool server_res_spipe::should_stop() {
    if (!conn_alive()) {
        return true; // Instantly stop GPU generation if HTTP client disconnected/aborted
    }
    if (spipe && spipe->is_cancelled()) {
        return true;
    }
    return false;
}
...
void server_res_spipe::on_complete() {
    ...
    // Immediately return on stream completion or peer drop — do NOT continue background token generation
    return;
}
