...
int llama_server(int argc, char ** argv) {
    ...
    // a plain router launch (no model at all) never loads a model and must not
    // touch the GPU: skip llama_backend_init() entirely so the CUDA primary
    // context stays uncreated. Child processes spawned by the router call
    // llama_backend_init() on their own. A -hf launch may still turn out to be
    // a remote preset (router mode), but that is only known after the fetch
    // below, so the backend is initialized for it as upstream does.
