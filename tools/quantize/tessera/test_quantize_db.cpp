...
int main(int argc, char ** argv) {
    ...
    std::string err;
    ts_quantize_db * db = ts_quantize_db_open(path, &err);
    CHECK(db != nullptr, ("open failed: " + err).c_str());
    if (db == nullptr) return 1;

    // Begin run
    std::string run_id = ts_quantize_db_begin_run(db, "/tmp/fake.gguf",
                                                  "deadbeef", "test-build",
                                                  "{\"k\":1}", &err);
    ...
    // Insert a tensor
    ts_quantize_db_tensor t;
    t.run_id      = run_id;
    t.name        = "blk.12.ffn_gate.weight";
    t.family      = "ffn";
    t.layer_depth = ts_quantize_db_layer_depth(t.name);
    t.out_dim     = 4096;
    t.in_dim      = 11008;
    ...
    t.eff_rank    = 0.42f;
    t.source_type = "f16";
    CHECK(ts_quantize_db_insert_tensor(db, t, &err) == 0, "insert_tensor failed");
    CHECK(t.layer_depth == 12, "layer_depth(blk.12.) should be 12");

    // layer_depth edge cases
    CHECK(ts_quantize_db_layer_depth("blk.0.attn_q") == 0, "blk.0 -> 0");
    CHECK(ts_quantize_db_layer_depth("blocks.99.ffn") == 99, "blocks.99 -> 99");
    CHECK(ts_quantize_db_layer_depth("h.7.") == 7, "h.7 -> 7");
    CHECK(ts_quantize_db_layer_depth("output.weight") == 0, "non-block -> 0");
    CHECK(ts_quantize_db_layer_depth("token_embd.weight") == 0, "embd -> 0");

    // Appender: bulk GA eval rows (the hot path)
    ts_quantize_db_appender * ap = ts_quantize_db_appender_open(
        ...
        for (int32_t g = -1; g < 4; g++) {
            for (int32_t isl = 0; isl < 2; isl++) {
                for (int32_t ci = 0; ci < 4; ci++) {
                    ...
                    CHECK(ts_quantize_db_appender_row(ap, row) == 0,
                          "appender row failed");
                }
            }
        }
        CHECK(ts_quantize_db_appender_flush(ap) == 0, "appender flush failed");
        ts_quantize_db_appender_close(ap);
    }
    ...
    CHECK(ts_quantize_db_insert_ga_result(db, gr, &err) == 0,
    ...
    ar.verdict      = "pass";
    CHECK(ts_quantize_db_insert_acceptance(db, ar, &err) == 0,
          "insert_acceptance failed");

    // Insert L5 fixup
    ts_quantize_db_l5_fixup fx;
    fx.run_id       = run_id;
    ...
    CHECK(ts_quantize_db_insert_l5_fixup(db, fx, &err) == 0,
          "insert_l5_fixup failed");

    // Complete the run
    CHECK(ts_quantize_db_complete_run(db, run_id, "completed", &err) == 0,
          "complete_run failed");

    // Reload ga_result
    ts_quantize_db_ga_result loaded;
    CHECK(ts_quantize_db_load_ga_result(db, run_id, "blk.12.ffn_gate.weight",
                                        &loaded),
    ...
    CHECK(ts_quantize_db_list_converged_for_model(db, "deadbeef", &done) == 0,
    ...
    CHECK(ts_quantize_db_list_converged(db, run_id, &done_run) == 0,
    ...
    std::string run_id2 = ts_quantize_db_begin_run(db, "/tmp/fake.gguf",
                                                   "deadbeef", "test-build",
                                                   "{\"k\":2}", &err);
    ...
    CHECK(ts_quantize_db_lookup_family_seed(db, "ffn", run_id2, &seed),
    ...
    ts_quantize_db_family_seed seed_x;
    CHECK(!ts_quantize_db_lookup_family_seed(db, "router", run_id2, &seed_x),
