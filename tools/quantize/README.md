...
* `--allow-qat-requantize` allows obvious Google/Gemma QAT sources to be requantized to ROCmFP4. This is blocked by default because QAT-to-ROCmFP4 requantization can damage coherence.
  Existing ROCmFP4 tensor formats, kernels, and non-QAT ROCmFP4 recipes are unchanged.
...
```bash
# Google/Gemma QAT-safe 4-bit recipe. This keeps the runtime layout native Q4_0
# and forces token embeddings plus output/logit tensors to Q8_0.
./llama-quantize input-google-gemma-qat-bf16.gguf output-google-gemma-qat-Q4_0.gguf Q4_0_QAT 8
```

```bash
# Existing Google QAT GGUF files should normally be preserved, not requantized.
./llama-quantize google-gemma-qat-q4_0.gguf google-gemma-qat-q4_0-current.gguf COPY
```
