...
static std::string get_test_file() {
    return (fs::temp_directory_path() / "test-llama-session.jsonl").string();
}

static void cleanup() {
    fs::remove(get_test_file());
}
...
static void test_write_and_load() {
    ...
    {
        session_file sf;
        assert(sf.open(get_test_file()));
        sf.write_header(fs::temp_directory_path().string());
    ...
    // Load it back
    auto loaded = session_file::load(get_test_file());
...
static void test_load_nonexistent() {
    auto loaded = session_file::load((fs::temp_directory_path() / "nonexistent-session-file.jsonl").string());
    assert(!loaded.has_value());
...
static void test_resume_append() {
    ...
    {
        session_file sf;
        sf.open(get_test_file());  // opens for append
    ...
    auto loaded2 = session_file::load(get_test_file());
