...
For remote overlay or LAN runs where the base and patch `rpc-server` processes are
...
python3 tools/rpc/bench_rpc_remote.py \
  ...
  --model /path/to/model.gguf \
  --base-rpc 192.0.2.20:50052 \
  --patch-rpc 192.0.2.21:50052 \
  --prompt 512 \
  --gen 128 \
  --repetitions 7 \
  --ngl 99 \
...
but raw RPC over TCP does not pass through those layers unless you explicitly
wrap the RPC connection in such a tunnel. Encrypted overlay and tunnel protocols
protect IP packets but do not provide a transparent per-TCP-stream compression
stage for RPC payloads.
