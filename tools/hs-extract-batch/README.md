...
- `--save-per-story` - Also write per-story vectors to `<output>.stories.bin` (per-story sidecar format: prompt_idx + group_id + mask_id + layer_idx + float32[n_embd] per record)
