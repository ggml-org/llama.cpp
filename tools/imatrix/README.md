...
* `--telemetry-out PATH` path to write a per-step accept/reject JSONL stream when `--model-draft` is set (default: disabled, no output). The schema is `llama.tessera.spec.v1` with one record per spec step carrying `seq_id`, `step_idx`, `prime_token`, `drafted`, `accepted`, `drafted_tokens[]`, `accepted_tokens[]`, and a per-position `confidence[]` array. The stream is append-only and flushed after each step, so it survives an interrupted run.
* `--telemetry-topk N` when `> 0` together with `--telemetry-out`, additionally include the per-position top-N verifier and drafter distributions (`topk`, `verifier_argmax[]`, `drafter_argmax[]`, `verifier_topk_*[]`, `drafter_topk_*[]`) in the `llama.tessera.spec.v1` record (default: 0, cheap per-step payload only). Suggested value: 64. The top-k fields are what the drafter fine-tuning / distillation tooling consumes; without them, only the accept/reject signal is available.
