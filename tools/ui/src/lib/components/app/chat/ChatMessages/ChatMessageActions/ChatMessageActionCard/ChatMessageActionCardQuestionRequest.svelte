<script lang="ts">
	...
	import type { AgenticQuestionAnswers, AgenticQuestionPrompt } from '$lib/types';

	interface Props {
		questions: AgenticQuestionPrompt[];
		onAnswer: (answers: AgenticQuestionAnswers) => void;
		onDismiss: () => void;
	}
	...
	function questionType(
		question: AgenticQuestionPrompt | null
	): 'freeform' | 'multiple_choice' | 'single_choice' {
		if (!question?.options || question.options.length === 0) return 'freeform';
		return question.multiple ? 'multiple_choice' : 'single_choice';
	}

	function questionHint(question: AgenticQuestionPrompt | null): string {
	...
	function answered(questionIndex: number): boolean {
		...
		if (type === 'freeform') return Boolean(customAnswer);
		return (
			optionAnswers.length > 0 ||
			(isCustomSelected(questionIndex, question) && Boolean(customAnswer))
		);
	}

	function isCustomSelected(questionIndex: number, question: AgenticQuestionPrompt): boolean {
		if (questionType(question) === 'freeform') return true;
		return customSelections[questionIndex] === true;
	}

	function toggleOption(questionIndex: number, label: string, question: AgenticQuestionPrompt) {
	...
	function selectOption(questionIndex: number, label: string, question: AgenticQuestionPrompt) {
	...
	function selectCustom(questionIndex: number, question: AgenticQuestionPrompt) {
		const type = questionType(question);
	...
	function submit() {
		const answers = questions.map((question, index) => {
			...
			if (type === 'freeform') return customAnswer ? [customAnswer] : [];
			if (type === 'multiple_choice') {
								...
								{#if !isDirectSingleChoice}
									<span data-slot="question-option-check">
										<span
											...
											data-type={questionType(currentQuestion) === 'multiple_choice'
												? 'checkbox'
												: 'radio'}
											data-picked={isSelected(currentIndex, option.label)}
										>
