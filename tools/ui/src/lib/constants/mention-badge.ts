/**
 * Shared visual contract between `MentionBadge.svelte` and the two DOM-only
 * paths (contenteditable tokenizer + rehype plugin). Svelte cannot be
 * mounted at the per-keystroke tokenizer hot path nor from a hast tree, so
 * both emit the badge with the same class string literal; Tailwind's
 * scanner picks it up in all three sources.
 */
 ...
 * `<path>` child of the wrapper `<svg>` so the DOM-built and hast-built
 * badges match `MentionBadge.svelte`'s lucide component. Paths match
 * `lucide-svelte`'s current `File` and `Folder` glyphs.
 */
