 ...
 */
export const STRIP_BLOCK_MARKDOWN_PATTERNS: [RegExp, string | ((match: string) => string)][] = [
	// Replaces ``` ... ``` blocks entirely
	[/```[\s\S]*?```/g, ''],
	// HTML tags
	[/<[^>]*>/g, ''],
	// Bold markers
...
export const STRIP_INLINE_MARKDOWN_PATTERNS: [RegExp, string | ((match: string) => string)][] = [
	// Inline code
	[/`[^`]*`/g, ''],
	// Italic markers
	[/\*(.*?)\*/g, '$1'],
