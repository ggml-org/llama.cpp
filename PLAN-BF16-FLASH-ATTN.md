# PLAN: Native BF16 Flash Attention MMA Kernel for NVIDIA

Authoritative implementation plan for adding a native BF16 Flash Attention tensor-core
(MMA) kernel to llama.cpp for NVIDIA GPUs (sm_80+/AMPERE). Consolidates the verified findings
of the v3-v6 exploration rounds. All line numbers were verified against the current tree.

Status: design complete and verified. Code changes have NOT been made yet.
...
The existing F16 MMA kernel (`fattn-mma-f16.cuh`) has PPL drift because both VKQ
...
(native BF16 is consumed directly; F32 is converted to F16, never to BF16). This also means
`fattn-common.cuh` needs NO changes: with native BF16 the case function calls
...
(`fattn.cu:351`), so BF16 caches already reach the tensor-core branch today and currently get
MMA_F16 (BF16->F16 conversion + FP16 accumulation). The new gate intercepts them.

Constraints (unchanged from the existing code conventions):
...
- The only edit to `fattn-mma-f16.cuh` is adding `#pragma once` as the first line (it
  currently has no include guard).

## 3. Design: tile types (verified)

Mirror the F16 Turing/Ampere `mma_tile_sizes` sets (`fattn-mma-f16.cuh:1027-1043`) exactly,
replacing `half2` with `nv_bfloat162` for the A/B input tiles and with FP32 tiles for the VKQ
accumulator. The data_layout enums are UNCHANGED (plain `DATA_LAYOUT_I_MAJOR` default; the
MIRRORED layouts belong only to Volta/RDNA3 and do not apply here).
...
- The bf16 wide `T_C_VKQ = tile<16,16,float>` (8 f32/thread, +4 regs vs f16 wide) needs the
  new wide bf16 mma overload (section 5). VKQ_C register pressure: narrow 4 regs (same as
  f16), wide 8 regs (f16 wide is 4). Config retune may be needed (section 10).

## 4. Design: KQ_max_scale VKQ scaling (derived from tile get_j semantics)

Each element `l` of the FP32 VKQ tile covers a specific KQ column; the scaling factor must
match the KQ softmax column indexing. Derived from `get_j` in `mma.cuh:256-271`:
...
Both are scalar f32 loops (replace the half2 code) with bound `DV/T_C_VKQ::I` (equals DV/16
for all four cases). Apply at BOTH sites: the normal path (`fattn-mma-f16.cuh:865-887`) and
the attention-sinks path (`:1370-1421`). This is consistent with the KQ softmax `KQ_idx`
mapping (narrow `l%2` at `:721,:747`; wide `(l/2)%2` at `:798,:840`).

No manual zeroing of KQ_C / VKQ_C is needed: `tile::x[ne] = {0}` (`mma.cuh:228`, NVIDIA
I_MAJOR branch) value-initializes the arrays, and the mma overloads use `"+r"` accumulate.

## 5. Design: mma overload mapping (verified against mma.cuh)
...
(`#else NO_DEVICE_CODE`). Add a CUDA branch at the TOP of that overload, mirroring the wide
F16 mma (`mma.cuh:1196-1255`): `#if defined(AMPERE_MMA_AVAILABLE)`, two
`mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32` asm calls with Bxi[0],Bxi[2] into
Dxi[0-3] then Bxi[1],Bxi[3] into Dxi[4-7] (same Axi[0-3]). Use `AMPERE_MMA_AVAILABLE` (not
TURING) to match the narrow bf16 mma guard at `mma.cuh:1183`. No Turing m8n8k8 fallback
exists for bf16 (bf16 mma requires sm_80+), and none is needed because the bf16 kernel only
compiles under `AMPERE_MMA_AVAILABLE`.
...
combine helpers. NO other change.
...
3. `get_bf16`, mirroring `get_half2` at `:711-720`:
   ...
   (RN conversion, `convert.cuh:54-60`).
4. `get_transposed(const tile<16, 4, nv_bfloat162> &) -> tile<8, 8, nv_bfloat162>`, mirroring
   `:722-728` (two `ggml_cuda_movmatrix` calls).
...
6. Softmax (max/exp/rowsum, mask add, `:687-863`): unchanged - all scalar f32 ops on the
   FP32 KQ_C tiles. The wide mask add reads half2 from `tile_mask` (`:778`) - unchanged (mask
   stays half-based).
...
8. Q/K/V loads: keep `tile_Q/tile_K/tile_V` as `extern __shared__ half2[]` (SRAM is
   ...
   `load_ldmatrix` / `load_ldmatrix_trans` are T-templated and bit-agnostic (`mma.cuh:785-914`);
   cast the SRAM pointer to `nv_bfloat162*` at the call sites (see section 7).
...
11. Read-back (`:1667-1693`): `ggml_cuda_cast<float2>(nv_bfloat162)` instead of
    `__half22float2(half2)` (`convert.cuh:44-45`). Everything after is pure float arithmetic.

Kernel/case/extern scaffolding (mirror `fattn-mma-f16.cuh:1703-2033`):

- `flash_attn_ext_bf16` kernel defined unconditionally as an always-compiled `__global__`
  template, body guarded `#if defined(FLASH_ATTN_AVAILABLE) && defined(AMPERE_MMA_AVAILABLE)`
  with a `NO_DEVICE_CODE` fallback so it links on all builds.
- `case_fattn_mma_bf16` defined unconditionally; per-case dispatch guarded with the same
  condition. Same per-cc `fattn_mma_config` tables, `nbytes_shared_*` in `sizeof(half2)`
  units (byte-identical for bf16), `V_is_K_view = (DKQ == 576)` unchanged, and
  `launch_fattn<DV,ncols1,ncols2>(ctx, dst, kernel, nwarps, nbytes_shared_total, nbatch_fa,
  false, false, true, warp_size_host)`.
- `DECL_FATTN_MMA_BF16_CASE(DKQ, DV, ncols1, ncols2, nwarps)` (explicit instantiation) and
  `DECL_FATTN_MMA_BF16_CASE_ALL_NCOLS2(...)` (extern declarations) at global scope, mirroring
  `:1967-2033`. Generated instance .cu files provide the definitions.
- switch_ncols1/switch_ncols2 mirror the F16 versions minus the Volta block (`fattn.cu:67-89`)
  and minus the Turing-only-build condition; special DKQ cases identical to F16.
...
2. Routing (`:461-483`): inside the `turing_mma_available(cc) && Q->ne[0] != 40 &&
   ...
   ```
   `bf16_mma_hardware_available(cc)` (`common.cuh:322-326`) = NVIDIA && cc >= AMPERE. Placing
   the gate inside the turing branch keeps it NVIDIA-only (the hardware check alone would also
   be true on AMD CDNA/RDNA3+ and would misroute AMD). Placement preserves VEC priority: a
   BF16 cache with a single-prompt small batch still goes VEC on Ada+.
3. `switch_ncols1` / `switch_ncols2` and the per-DKQ dispatch (`:8-242`): mirror the F16
   versions (192 -> ncols2 in {8,16}, 320 -> 32, 512 -> {2,4,8}, 576 -> {4,16,32}; 40/72
   excluded; 576 with gqa_ratio==20 uses the ncols2==32 sub-branch).
4. `get_alloc_size` (`:550-562`): `case BEST_FATTN_KERNEL_MMA_BF16: break;` (native BF16 K/V:
   no conversion buffers).
5. Main entry (`:570-585`):
   ```cpp
   case BEST_FATTN_KERNEL_MMA_BF16:
       ggml_cuda_flash_attn_ext_mma_bf16(ctx, dst);
       break;
   ```
6. Add `#include "fattn-mma-bf16.cuh"` next to the F16 include.
...
{2,4,8}; 576 -> {4,16,32}; ncols2 in {16,32} only for those niche sizes. Then run the script.
No CMake change needed: `fattn-mma*.cu` is GLOBbed (`CMakeLists.txt:108`).
...
1. Q fill `:1202-1241` (section 6.3 rule 7).
...
5. Combine-write narrow `:1570-1581` (section 6.3 rule 9).
6. Combine-write wide `:1611-1624` (section 6.3 rule 9).
7. Read-back `:1670` (section 6.3 rule 11).
...
- `common.cuh`: AMPERE_MMA_AVAILABLE (282-284), bf16_mma_hardware_available (322-326),
  turing_mma_available (348-350).
...
3. Create `fattn-mma-bf16.cuh` (section 6.3 + section 7).
...
7. If register pressure hurts occupancy on the wide path, retune
   `mma_tile_sizes_bf16` / `get_nstages` / `get_nbatch_*` based on benchmark results (report,
   do not guess).
...
- Performance: `llama-bench -b 2048 -n 64 --cache-type-k bf16 --cache-type-v bf16` vs the
  F16-cache baseline, covering narrow (ncols<=8) and wide (ncols 16/32) paths, head dims
