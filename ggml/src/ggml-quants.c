...
static const float g_nvfp4_max_tensor_scale = 448.0f;
...
static inline int ggml_nvfp4_tune_state_load(const volatile int * ptr) {
    return (int) _InterlockedCompareExchange((volatile long *) ptr, 0, 0);
}

static inline bool ggml_nvfp4_tune_state_try_begin(volatile int * ptr) {
    return _InterlockedCompareExchange((volatile long *) ptr, 1, 0) == 0;
}

static inline void ggml_nvfp4_tune_state_store_ready(volatile int * ptr) {
    _InterlockedExchange((volatile long *) ptr, 2);
}
#else
static inline int ggml_nvfp4_tune_state_load(const int * ptr) {
    return __atomic_load_n(ptr, __ATOMIC_ACQUIRE);
}

static inline bool ggml_nvfp4_tune_state_try_begin(int * ptr) {
    int expected = 0;
    return __atomic_compare_exchange_n(ptr, &expected, 1, false, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE);
}

static inline void ggml_nvfp4_tune_state_store_ready(int * ptr) {
    __atomic_store_n(ptr, 2, __ATOMIC_RELEASE);
}
#endif
...
static void quantize_row_nvfp4_impl(
    ...
    if (ggml_nvfp4_tune_state_load(&g_nvfp4_tune_state) != 2) {
        if (ggml_nvfp4_tune_state_try_begin(&g_nvfp4_tune_state)) {
            nvfp4_autotune_scale_mul_on_row(x, n, quant_weights);
            ggml_nvfp4_tune_state_store_ready(&g_nvfp4_tune_state);
        } else {
            while (ggml_nvfp4_tune_state_load(&g_nvfp4_tune_state) != 2) { }
        }
    }
    ...
    for (int64_t ib = 0; ib < nb; ++ib) {
        ...
        if (!nvfp4_quantize_block(block, weights, codes, block_scales, &tensor_scale, NULL)) {
            continue;
        }

        for (int subblock = 0; subblock < QK_K / QK_NVFP4; ++subblock) {
            const float local_scale = tensor_scale * GGML_FP8_UE4M3_TO_FP32(block_scales[subblock]);
            block_scales[subblock] = GGML_FP32_TO_UE4M3(local_scale);
        }
