...
static float make_qx_quants(int                         n,
                            int                         nmax,
...
static float make_qkx1_quants(int                         n,
                              int                         nmax,
                              const float * GGML_RESTRICT x,
...
static float make_qkx2_quants(int                         n,
                              int                         nmax,
                              const float * GGML_RESTRICT x,
...
static float make_qkx3_quants(int                         n,
                              int                         nmax,
                              const float * GGML_RESTRICT x,
                              const float * GGML_RESTRICT weights,
                              uint8_t * GGML_RESTRICT     L,
...
static float make_qp_quants(int                         n,
                            int                         nmax,
                            const float * GGML_RESTRICT x,
                            uint8_t * GGML_RESTRICT     L,
...
static void quantize_row_q2_K_impl(const float * GGML_RESTRICT x,
...
size_t quantize_q2_K(const float * GGML_RESTRICT src,
...
static void quantize_row_q3_K_impl(const float * GGML_RESTRICT x,
...
size_t quantize_q3_K(const float * GGML_RESTRICT src,
                     void * GGML_RESTRICT        dst,
                     int64_t                     nrow,
                     int64_t                     n_per_row,
                     const float *               quant_weights) {
...
static void quantize_row_q4_K_impl(const float * GGML_RESTRICT x,
...
size_t quantize_q4_K(const float * GGML_RESTRICT src,
                     void * GGML_RESTRICT        dst,
                     int64_t                     nrow,
                     int64_t                     n_per_row,
                     const float *               quant_weights) {
...
static void quantize_row_q5_K_impl(const float * GGML_RESTRICT x,
...
size_t quantize_q5_K(const float * GGML_RESTRICT src,
                     void * GGML_RESTRICT        dst,
                     int64_t                     nrow,
                     int64_t                     n_per_row,
                     const float *               quant_weights) {
...
static void quantize_row_q6_K_impl(const float * GGML_RESTRICT x,
...
size_t quantize_q6_K(const float * GGML_RESTRICT src,
                     void * GGML_RESTRICT        dst,
                     int64_t                     nrow,
                     int64_t                     n_per_row,
                     const float *               quant_weights) {
...
static void quantize_row_q4_0_impl(const float * GGML_RESTRICT x,
...
size_t quantize_q4_0(const float * GGML_RESTRICT src,
                     void * GGML_RESTRICT        dst,
                     int64_t                     nrow,
                     int64_t                     n_per_row,
                     const float *               quant_weights) {
...
static void quantize_row_q4_1_impl(const float * GGML_RESTRICT x,
                                   block_q4_1 * GGML_RESTRICT  y,
                                   int64_t                     n_per_row,
                                   const float *               quant_weights) {
...
size_t quantize_q4_1(const float * GGML_RESTRICT src,
                     void * GGML_RESTRICT        dst,
                     int64_t                     nrow,
                     int64_t                     n_per_row,
                     const float *               quant_weights) {
...
static void quantize_row_q5_0_impl(const float * GGML_RESTRICT x,
                                   block_q5_0 * GGML_RESTRICT  y,
                                   int64_t                     n_per_row,
                                   const float *               quant_weights) {
...
size_t quantize_q5_0(const float * GGML_RESTRICT src,
                     void * GGML_RESTRICT        dst,
                     int64_t                     nrow,
                     int64_t                     n_per_row,
                     const float *               quant_weights) {
...
static void quantize_row_q5_1_impl(const float * GGML_RESTRICT x,
                                   block_q5_1 * GGML_RESTRICT  y,
                                   int64_t                     n_per_row,
                                   const float *               quant_weights) {
...
size_t quantize_q5_1(const float * GGML_RESTRICT src,
                     void * GGML_RESTRICT        dst,
                     int64_t                     nrow,
                     int64_t                     n_per_row,
                     const float *               quant_weights) {
...
size_t quantize_q8_0(const float * GGML_RESTRICT src,
                     void * GGML_RESTRICT        dst,
                     int64_t                     nrow,
                     int64_t                     n_per_row,
                     const float *               quant_weights) {
...
size_t quantize_mxfp4(const float * GGML_RESTRICT src,
                      void * GGML_RESTRICT        dst,
                      int64_t                     nrow,
                      int64_t                     n_per_row,
                      const float *               quant_weights) {
...
size_t quantize_tq1_0(const float * GGML_RESTRICT src,
                      void * GGML_RESTRICT        dst,
                      int64_t                     nrow,
                      int64_t                     n_per_row,
                      const float *               quant_weights) {
...
size_t quantize_tq2_0(const float * GGML_RESTRICT src,
                      void * GGML_RESTRICT        dst,
                      int64_t                     nrow,
                      int64_t                     n_per_row,
                      const float *               quant_weights) {
...
// ====================== Ultra-low-bit quantization (Q1_5_K ~1.8125 bpw, Q2_K_S ~2.1875 bpw)

void quantize_row_q1_5_K_ref(const float * GGML_RESTRICT x, block_q1_5_K * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;

    for (int64_t i = 0; i < nb; i++) {
        // Compute per-sub-block absolute maximum (8 sub-blocks of 32 elements)
        float sub_amax[8];
        for (int sb = 0; sb < 8; sb++) {
            float amax = 0.0f;
            for (int j = 0; j < 32; j++) {
                amax = MAX(amax, fabsf(x[sb * 32 + j]));
            }
            sub_amax[sb] = amax;
        }

        // Find the global maximum of sub-block scales
        float global_amax = 0.0f;
        for (int sb = 0; sb < 8; sb++) {
            global_amax = MAX(global_amax, sub_amax[sb]);
        }

        if (global_amax == 0.0f) {
            memset(&y[i], 0, sizeof(block_q1_5_K));
            y[i].d = GGML_FP32_TO_FP16(0.0f);
            x += QK_K;
            continue;
        }

        // Quantize sub-block scales to 4 bits (0..15)
        const float iscale = 15.0f / global_amax;
        uint8_t     qscales[8];
        for (int sb = 0; sb < 8; sb++) {
            qscales[sb] = MIN(15, (int) (sub_amax[sb] * iscale + 0.5f));
        }

        // Pack 8 x 4-bit scales into 4 bytes
        for (int j = 0; j < 4; j++) {
            y[i].scales[j] = qscales[2 * j] | (qscales[2 * j + 1] << 4);
        }

        // Store super-block scale
        y[i].d = GGML_FP32_TO_FP16(global_amax / 15.0f);

        // Quantize each value to ternary {-1, 0, +1} using sub-block scales
        int8_t L[QK_K];
        for (int sb = 0; sb < 8; sb++) {
            float scale = (global_amax / 15.0f) * qscales[sb];
            float id    = scale > 0.0f ? 1.0f / scale : 0.0f;
            for (int j = 0; j < 32; j++) {
                int v          = lroundf(x[sb * 32 + j] * id);
                v              = MAX(-1, MIN(1, v));
                L[sb * 32 + j] = v;
            }
        }

        // Pack ternary values using base-3 encoding into qs[48] (5 values per byte, 240 values)
        // Layout must be interleaved for SIMD unpacking!
        // First 32 bytes of qs: 5 elements per byte, along 32 elements (160 values)
        for (int j = 0; j < 32; j++) {
            uint8_t q = 0;
            for (int n = 0; n < 5; n++) {
                int idx = j + n * 32;
                int xi  = L[idx] + 1;  // +1 maps {-1,0,+1} -> {0,1,2}
                q *= 3;
                q += xi;
            }
            // Ceiling division normalization (243 == 3^5)
            q          = ((uint16_t) q * 256 + (243 - 1)) / 243;
            y[i].qs[j] = q;
        }

        // Next 16 bytes of qs (indices 32..47): 5 elements per byte, along 16 elements (80 values)
        for (int j = 0; j < 16; j++) {
            uint8_t q = 0;
            for (int n = 0; n < 5; n++) {
                int idx = 160 + j + n * 16;
            ...
            // Ceiling division normalization
            q               = ((uint16_t) q * 256 + (243 - 1)) / 243;
            y[i].qs[32 + j] = q;
        }

        // Pack remaining 16 values (indices 240..255) into qh[4] (4 values per byte interleaved)
        for (int j = 0; j < 4; j++) {
            uint8_t q = 0;
            for (int n = 0; n < 4; n++) {
                int idx = 240 + j + n * 4;
            ...
            // Shift to fill 5-trit byte and normalize
            q *= 3;
            q          = ((uint16_t) q * 256 + (243 - 1)) / 243;
            y[i].qh[j] = q;
        }
...
void dequantize_row_q1_5_K(const block_q1_5_K * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
    ...
    for (int64_t i = 0; i < nb; ++i) {
        ...
        // Dequantize qs[48] -> 240 values (base-3, 5 per byte)
        ...
        for (int j = 0; j < 32; j++) {
            for (int n = 0; n < 5; n++) {
                ...
                int16_t xi  = ((uint16_t) q * 3) >> 8;
                int     sb  = idx / 32;
                y[idx]      = (float) (xi - 1) * sub_scales[sb];
            }
        }
        ...
        // Dequantize qh[4] -> 16 values (base-3, 4 per byte, interleaved by 4)
        for (int j = 0; j < 4; j++) {
            for (int n = 0; n < 4; n++) {
                int     idx = 240 + j + n * 4;
                uint8_t q   = x[i].qh[j] * pow3[n];
...
size_t quantize_q1_5_K(const float * GGML_RESTRICT src,
                       void * GGML_RESTRICT        dst,
                       int64_t                     nrow,
                       int64_t                     n_per_row,
                       const float *               quant_weights) {
    (void) quant_weights;  // imatrix not used for ternary quantization
    const size_t row_size = ggml_row_size(GGML_TYPE_Q1_5_K, n_per_row);
    quantize_row_q1_5_K_ref(src, dst, (int64_t) nrow * n_per_row);
    return nrow * row_size;
}

// ====================== Q2_K_S: Streamlined 2-bit quantization (~2.1875 bpw)

void quantize_row_q2_K_s_ref(const float * GGML_RESTRICT x, block_q2_K_s * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
    ...
    for (int64_t i = 0; i < nb; i++) {
        ...
        if (global_amax == 0.0f) {
            memset(&y[i], 0, sizeof(block_q2_K_s));
            y[i].d = GGML_FP32_TO_FP16(0.0f);
            x += QK_K;
            continue;
        }
        ...
        y[i].d           = GGML_FP32_TO_FP16(global_amax / (15.0f * 1.5f));
        const float d_16 = GGML_FP16_TO_FP32(y[i].d);

        // Quantize each value to 2-bit {0, 1, 2, 3}, centered: dequant = (q - 1.5) * scale
        // q = round(x / scale + 1.5), clamped to [0, 3]
        for (int sb = 0; sb < 8; sb++) {
            float scale = d_16 * qscales[sb];
            float id    = scale > 0.0f ? 1.0f / scale : 0.0f;
            for (int m = 0; m < 32; m++) {
                int idx      = sb * 32 + m;
                int q        = lroundf(x[idx] * id + 1.5f);
                q            = MAX(0, MIN(3, q));
...
void dequantize_row_q2_K_s(const block_q2_K_s * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
    ...
    for (int64_t i = 0; i < nb; ++i) {
        ...
        for (int j = 0; j < QK_K; j++) {
            ...
            int sb       = j / 32;
            y[j]         = ((float) q - 1.5f) * sub_scales[sb];
        }
...
size_t quantize_q2_K_s(const float * GGML_RESTRICT src,
                       void * GGML_RESTRICT        dst,
                       int64_t                     nrow,
                       int64_t                     n_per_row,
                       const float *               quant_weights) {
    (void) quant_weights;
    const size_t row_size = ggml_row_size(GGML_TYPE_Q2_K_S, n_per_row);
    quantize_row_q2_K_s_ref(src, dst, (int64_t) nrow * n_per_row);
    return nrow * row_size;
}
...
static void quantize_row_iq2_xxs_impl(const float * GGML_RESTRICT x,
...
static void quantize_row_iq2_xs_impl(const float * GGML_RESTRICT x,
                                     void * GGML_RESTRICT        vy,
                                     int64_t                     n,
                                     const float * GGML_RESTRICT quant_weights) {
...
size_t quantize_iq2_xxs(const float * GGML_RESTRICT src,
                        void * GGML_RESTRICT        dst,
                        int64_t                     nrow,
                        int64_t                     n_per_row,
                        const float *               quant_weights) {
...
size_t quantize_iq2_xs(const float * GGML_RESTRICT src,
                       void * GGML_RESTRICT        dst,
                       int64_t                     nrow,
                       int64_t                     n_per_row,
                       const float *               quant_weights) {
...
static void quantize_row_iq3_xxs_impl(int                         grid_size,
                                      const float * GGML_RESTRICT x,
                                      void * GGML_RESTRICT        vy,
                                      int64_t                     n,
                                      const float * GGML_RESTRICT quant_weights) {
...
size_t quantize_iq3_xxs(const float * GGML_RESTRICT src,
                        void * GGML_RESTRICT        dst,
                        int64_t                     nrow,
                        int64_t                     n_per_row,
                        const float *               quant_weights) {
...
static void quantize_row_iq3_s_impl(int                         block_size,
                                    const float * GGML_RESTRICT x,
                                    void * GGML_RESTRICT        vy,
                                    int                         n,
                                    const float * GGML_RESTRICT quant_weights,
...
size_t quantize_iq3_s(const float * GGML_RESTRICT src,
                      void * GGML_RESTRICT        dst,
                      int64_t                     nrow,
                      int64_t                     n_per_row,
                      const float *               quant_weights) {
...
static void quantize_row_iq1_s_impl(const float * GGML_RESTRICT x,
                                    void * GGML_RESTRICT        vy,
                                    int64_t                     n,
                                    const float * GGML_RESTRICT quant_weights,
                                    float *                     scales,
...
size_t quantize_iq1_s(const float * GGML_RESTRICT src,
                      void * GGML_RESTRICT        dst,
                      int64_t                     nrow,
                      int64_t                     n_per_row,
                      const float *               quant_weights) {
...
static void quantize_row_iq1_m_impl(const float * GGML_RESTRICT x,
                                    void * GGML_RESTRICT        vy,
                                    int64_t                     n,
                                    const float * GGML_RESTRICT quant_weights,
                                    float *                     scales,
...
size_t quantize_iq1_m(const float * GGML_RESTRICT src,
                      void * GGML_RESTRICT        dst,
                      int64_t                     nrow,
                      int64_t                     n_per_row,
                      const float *               quant_weights) {
...
static void quantize_row_iq4_nl_impl(const int                   super_block_size,
...
size_t quantize_iq4_nl(const float * GGML_RESTRICT src,
                       void * GGML_RESTRICT        dst,
                       int64_t                     nrow,
                       int64_t                     n_per_row,
                       const float *               quant_weights) {
...
size_t quantize_iq4_xs(const float * GGML_RESTRICT src,
                       void * GGML_RESTRICT        dst,
                       int64_t                     nrow,
                       int64_t                     n_per_row,
                       const float *               quant_weights) {
...
static void quantize_row_iq2_s_impl(const float * GGML_RESTRICT x,
                                    void * GGML_RESTRICT        vy,
                                    int64_t                     n,
                                    const float * GGML_RESTRICT quant_weights) {
...
size_t quantize_iq2_s(const float * GGML_RESTRICT src,
                      void * GGML_RESTRICT        dst,
                      int64_t                     nrow,
                      int64_t                     n_per_row,
                      const float *               quant_weights) {
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_Q1_5_K:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_q1_5_K, data, nb);
            }
            break;
        case GGML_TYPE_Q2_K_S:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_q2_K_s, data, nb);
            }
            break;
