...
//
// Blue-noise dither table for Q4_0_BLUE KV cache quantization
// Deterministic 64-value pattern designed to decorrelate quantization error
//
...
//
// Q4_0 quantization with deterministic blue-noise dithering
// Uses the same block format as Q4_0 but applies noise before rounding
// to decorrelate quantization error
//
void quantize_row_q4_0_blue_ref(const float * GGML_RESTRICT x, block_q4_0 * GGML_RESTRICT y, int64_t k) {
    static const int qk = QK4_0;
    ...
    // dither strength: fraction of one quantization step (2^4 = 16 levels, step ≈ 1/8)
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < qk/2; ++j) {
            // Apply blue-noise dither before rounding
            // Use deterministic seed from block index and element position
            const int seed = i * qk + j;
            const float noise0 = blue_noise_64[(seed + 0) & 63] * strength;
            const float noise1 = blue_noise_64[(seed + 1) & 63] * strength;

            const float x0 = x[i*qk + 0    + j] * id + noise0;
            const float x1 = x[i*qk + qk/2 + j] * id + noise1;
...
void quantize_row_q2_K_blue_ref(const float * GGML_RESTRICT x, block_q2_K * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int nb = k / QK_K;
    const float strength = 0.25f;
...
size_t quantize_q2_K_blue(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
        quantize_row_q2_K_blue_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * ggml_row_size(GGML_TYPE_Q2_K, n_per_row);
    }
    size_t row_size = ggml_row_size(GGML_TYPE_Q2_K, n_per_row);
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q2_K_blue_ref(src, (block_q2_K*)qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
...
void quantize_row_q3_K_blue_ref(const float * GGML_RESTRICT x, block_q3_K * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int nb = k / QK_K;
    const float strength = 0.25f;
...
size_t quantize_q3_K_blue(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    size_t row_size = ggml_row_size(GGML_TYPE_Q3_K, n_per_row);
    if (!quant_weights) {
        quantize_row_q3_K_blue_ref(src, dst, (int64_t)nrow*n_per_row);
    }
    else {
        char * qrow = (char *)dst;
        for (int64_t row = 0; row < nrow; ++row) {
            quantize_row_q3_K_blue_ref(src, (block_q3_K*)qrow, n_per_row);
            src += n_per_row;
            qrow += row_size;
        }
    }
...
size_t quantize_q4_0_blue(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    // blue-noise dithering is only applied on the reference path; weighted (imatrix) path falls back to normal q4_0
    if (!quant_weights) {
        quantize_row_q4_0_blue_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * ggml_row_size(GGML_TYPE_Q4_0, n_per_row);
    }
    size_t row_size = ggml_row_size(GGML_TYPE_Q4_0, n_per_row);
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
        // For weighted quantization, fall back to blue-ref (dithering still applied on top)
        quantize_row_q4_0_blue_ref(src, (block_q4_0*)qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
    return nrow * row_size;
...
void quantize_row_q4_1_blue_ref(const float * GGML_RESTRICT x, block_q4_1 * GGML_RESTRICT y, int64_t k) {
    const int qk = QK4_1;
    assert(k % qk == 0);
    const int nb = k / qk;
    const float strength = 0.25f;
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < qk/2; ++j) {
            ...
            const float noise0 = blue_noise_64[(seed + 0) & 63] * strength;
            const float noise1 = blue_noise_64[(seed + 1) & 63] * strength;
            const float x0 = (x[i*qk + 0    + j] - min)*id + noise0;
            const float x1 = (x[i*qk + qk/2 + j] - min)*id + noise1;
            const uint8_t xi0 = MIN(15, (int8_t)(x0 + 0.5f));
            const uint8_t xi1 = MIN(15, (int8_t)(x1 + 0.5f));
            y[i].qs[j]  = xi0;
            y[i].qs[j] |= xi1 << 4;
...
size_t quantize_q4_1_blue(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
        quantize_row_q4_1_blue_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * ggml_row_size(GGML_TYPE_Q4_1, n_per_row);
    }
    size_t row_size = ggml_row_size(GGML_TYPE_Q4_1, n_per_row);
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q4_1_blue_ref(src, (block_q4_1*)qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
    return nrow * row_size;
