...
static __global__ void l2_norm_f32(
        const float * x, float * dst, const int ncols, const int64_t stride_col, const int64_t stride_row,
...
static void l2_norm_f32_cuda(
    ...
    if (ncols < 1024) {
        ...
        ggml_cuda_kernel_launch(l2_norm_f32<WARP_SIZE>, launch_params, x, dst, ncols, stride_col, stride_row, stride_channel, stride_sample, eps);
    } else {
        const dim3 block_dims(1024, 1, 1);
        const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params{blocks_num, block_dims, block_dims.x > WARP_SIZE ? 32 * sizeof(float): 0, stream};
        ggml_cuda_kernel_launch(l2_norm_f32<1024>, launch_params, x, dst, ncols, stride_col, stride_row, stride_channel, stride_sample, eps);
    }
}
...
void ggml_cuda_op_l2_norm(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    l2_norm_f32_cuda(src0_d, dst_d, ne00, ne01, ne02, ne03, s00, s01, s02, s03, eps, stream);
}
