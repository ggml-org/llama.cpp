...
static __global__ void rms_norm_f32(
        ...
        const int64_t mul_stride_channel = 0, const int64_t mul_stride_sample = 0) {
    const int nrows     = gridDim.x;
    const int nchannels = gridDim.y;
...
static void rms_norm_mul_f32_cuda(
    ...
    if (ncols < 1024) {
        ...
        const dim3 block_dims(1024, 1, 1);
        rms_norm_f32<1024, true><<<blocks_num, block_dims, 0, stream>>>(x, dst, ncols, stride_row, stride_channel, stride_sample, eps, mul, mul_stride_row, mul_stride_channel, mul_stride_sample);
    }
}
...
void ggml_cuda_op_rms_norm_fused(ggml_backend_cuda_context & ctx, ggml_tensor * dst, ggml_tensor * mul_tensor) {
    ...
    const size_t ts_mul = ggml_type_size(mul_tensor->type);
    const int64_t mul_s01 = mul_tensor->nb[1] / ts_mul;
    const int64_t mul_s02 = mul_tensor->nb[2] / ts_mul;
    const int64_t mul_s03 = mul_tensor->nb[3] / ts_mul;

    rms_norm_mul_f32_cuda(src0_d, mul_d, dst_d, ne00, ne01, ne02, ne03, s01, s02, s03, mul_s01, mul_s02, mul_s03, eps, stream);
}
