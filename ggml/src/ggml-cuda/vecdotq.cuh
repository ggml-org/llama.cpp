...
// Experimental gfx1030 path. The quantizer has already split Q8 values
// into packed LO/HI words and precomputed the HI correction per 16 values.
template <int vdr> static __device__ __forceinline__ float vec_dot_q4_0_q8_1_dot8_impl(
    const uint32_t * v, const block_q8_1_dot8 * bq8_1, const int iqs,
...
#pragma unroll
    for (int i = 0; i < vdr; ++i) {
        ...
        const uint32_t qa_lo = bq8_1->lo[iqs + i];
        const uint32_t qa_hi = bq8_1->hi[iqs + i];

        sumi += ggml_hip_udot8_u4(qw_u, qa_lo, 0);
        sumi += 16 * ggml_hip_sdot8_i4((int) qw_s, (int) qa_hi, 0);
    }

    // Each Q4_0 MMVQ invocation covers either values [0, 15] or [16, 31]
    // of the Q8_1 block, selected by iqs = 0 or 2.
    sumi += 128 * (int) bq8_1->sum_hi[iqs / VDR_Q4_0_Q8_1_MMVQ];
...
static __device__ __forceinline__ float vec_dot_q4_0_q8_1_dot8(
    const void * __restrict__ vbq, const block_q8_1_dot8 * __restrict__ bq8_1, const int & kbx, const int & iqs) {
...
#pragma unroll
    ...
    return vec_dot_q4_0_q8_1_dot8_impl<VDR_Q4_0_Q8_1_MMVQ>(v, bq8_1, iqs, bq4_0->d, bq8_1->ds);
}
