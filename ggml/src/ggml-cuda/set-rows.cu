...
static void set_rows_cuda(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (dst->type == GGML_TYPE_F32) {
        ...
        int32_t no_rotate = 0;
        memcpy(&no_rotate, dst->op_params + 1, sizeof(int32_t));
        if (no_rotate) {
        ...
        memcpy(&no_rotate, dst->op_params + 1, sizeof(int32_t));
        if (no_rotate) {
            set_rows_cuda_quant<idx_t, block_planar4_0, QK_PLANAR4, quantize_f32_planar4_block_norot>(
        ...
        memcpy(&no_rotate, dst->op_params + 1, sizeof(int32_t));
        if (no_rotate) {
            set_rows_cuda_quant<idx_t, block_iso4_0, QK_ISO4, quantize_f32_iso4_block_norot>(
