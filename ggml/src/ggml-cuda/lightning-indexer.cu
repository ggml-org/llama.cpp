...
<<<<<<< ours
// CDNA matrix-core (MFMA) lightning indexer. Mirrors the NVIDIA wmma kernel but tiled for the
// gfx90a/gfx942 16x16x16 f16 MFMA and 64-wide wavefronts. Each wave computes one or more
// 16(head) x 16(kv) QK tiles (f32 accumulate over N_EMBD/16 embd tiles), the block reduces
// ReLU(QK)*w over all heads per KV and adds the mask. Q/K are staged to LDS as f16 (same input
// precision as the NVIDIA wmma path), the QK dot product accumulates in f32.
=======
>>>>>>> theirs
template <int WARPS_PER_BLOCK, int K_VECS_PER_BLOCK, int64_t N_EMBD, int64_t N_HEAD, ggml_type TYPE_K>
...
<<<<<<< ours
    // NOTE: ggml's WARP_SIZE is 32, but rocWMMA/MFMA operate on the physical 64-lane CDNA wave.
    // Each threadIdx.y is one full 64-lane wave, so the block must be launched as (64, WARPS).
    constexpr int WAVE_SIZE = 64;
    constexpr int THREADS_PER_BLOCK = WARPS_PER_BLOCK * WAVE_SIZE;
    constexpr int N_EMBD_PADDED = N_EMBD + 8; // pad LDS rows to reduce bank conflicts
=======
    constexpr int WAVE_SIZE = 64;
    constexpr int THREADS_PER_BLOCK = WARPS_PER_BLOCK * WAVE_SIZE;
    constexpr int N_EMBD_PADDED = N_EMBD + 8;
...
<<<<<<< ours
    constexpr int N_HEAD_TILES = N_HEAD / MMA_DIM;           // 4 for N_HEAD = 64
    constexpr int N_KV_TILES   = K_VECS_PER_BLOCK / MMA_DIM;
    constexpr int N_EMBD_TILES = N_EMBD / MMA_DIM;           // 8 for N_EMBD = 128
=======
...
<<<<<<< ours
    // phase 2 - load Q (f32 -> f16) to LDS, 4 embd elements at a time
=======
>>>>>>> theirs
...
<<<<<<< ours
    // phase 3 - load (and dequantize if needed) K to LDS as f16
=======
>>>>>>> theirs
...
<<<<<<< ours
    // phase 4 - QK = Q * K^T via MFMA. Each wave grabs (head-tile, kv-tile) pairs round-robin.
    // frag layout: A = Q[head][embd] (row-major), B = K[kv][embd] read col-major as B[embd][kv],
    // so C[head][kv] = sum_embd Q[head][embd] * K[kv][embd].
    for (int tile = i_warp; tile < N_TILES; tile += WARPS_PER_BLOCK) {
        const int ht = tile / N_KV_TILES; // head tile
        const int kt = tile % N_KV_TILES; // kv   tile
=======
    for (int tile = i_warp; tile < N_TILES; tile += WARPS_PER_BLOCK) {
...
<<<<<<< ours
    // phase 5 - reduce ReLU(QK)*w over heads per KV, add mask, write to VRAM
=======
>>>>>>> theirs
...
<<<<<<< ours
        // CDNA matrix-core (MFMA) path for the DeepSeek-V3.2/V4 indexer (n_embd=128, n_head=64).
=======
>>>>>>> theirs
...
<<<<<<< ours
            // rocWMMA/MFMA operate on the physical 64-lane CDNA wave, so launch (64, WARPS).
=======
>>>>>>> theirs
