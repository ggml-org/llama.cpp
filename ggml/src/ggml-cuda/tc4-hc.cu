...
static constexpr int TC4_HC = 4;
static constexpr float TC4_HC_CLAMP = 30.0f;


// normalize over ne1 (dst): each src row sums to 1 (eps only in denominator)
static __device__ void tc4_hc_comb_norm_dst(float * comb, float eps) {
    for (int isrc = 0; isrc < TC4_HC; ++isrc) {
        float sum = eps;
        for (int idst = 0; idst < TC4_HC; ++idst) {
            sum += comb[isrc + TC4_HC*idst];
        }

        const float inv_sum = 1.0f / sum;
        for (int idst = 0; idst < TC4_HC; ++idst) {
            comb[isrc + TC4_HC*idst] *= inv_sum;
        }
    }
}

// normalize over ne0 (src): each dst column sums to 1 (eps added in denominator)
static __device__ void tc4_hc_comb_norm_src(float * comb, float eps) {
    for (int idst = 0; idst < TC4_HC; ++idst) {
        float sum = eps;
        for (int isrc = 0; isrc < TC4_HC; ++isrc) {
            sum += comb[isrc + TC4_HC*idst];
...
// flat idx = src + TC4_HC*dst. Matches telechat4's
...
static __global__ void tc4_hc_comb_f32(
        const float * mixes,
        const float * scale,
        const float * base,
        float * dst,
        ...
        float eps,
        int32_t n_iter) {
    constexpr int comb_offset = 2*TC4_HC;
    ...
    float comb[TC4_HC*TC4_HC];

    for (int idst = 0; idst < TC4_HC; ++idst) {
        float max = -INFINITY;
        for (int isrc = 0; isrc < TC4_HC; ++isrc) {
            const int idx = isrc + TC4_HC*idst;
            const float v = mixes[(comb_offset + idx)*sm0 + it*sm1] * scale_comb + base[(comb_offset + idx)*sb0];
            comb[idx] = fminf(fmaxf(v, -TC4_HC_CLAMP), TC4_HC_CLAMP);
    ...
    for (int32_t i = 0; i < n_iter; ++i) {
        tc4_hc_comb_norm_src(comb, eps);
        tc4_hc_comb_norm_dst(comb, eps);
    }
...
static __global__ void tc4_hc_pre_f32(
        const float * x,
        const float * weights,
        float * dst,
        int64_t n_embd,
...
static __global__ void tc4_hc_post_f32(
        const float * x,
...
void ggml_cuda_op_tc4_hc_comb(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    constexpr int64_t hc_mix_dim = (2 + TC4_HC)*TC4_HC;

    GGML_ASSERT(mixes->ne[0] == hc_mix_dim);
    GGML_ASSERT(dst->ne[0] == TC4_HC);
    GGML_ASSERT(dst->ne[1] == TC4_HC);
    ...
    ggml_cuda_kernel_launch(tc4_hc_comb_f32, launch_params,
...
void ggml_cuda_op_tc4_hc_pre(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    ggml_cuda_kernel_launch(tc4_hc_pre_f32, launch_params,
...
void ggml_cuda_op_tc4_hc_post(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    ggml_cuda_kernel_launch(tc4_hc_post_f32, launch_params,
