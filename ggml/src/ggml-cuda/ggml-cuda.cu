...
static std::mutex g_xdev_host_stage_policy_mutex;
static std::atomic<uint8_t> g_xdev_host_stage_policy[GGML_CUDA_MAX_DEVICES][GGML_CUDA_MAX_DEVICES] = {}; // 0 = unknown, 1 = direct, 2 = host-staged
static std::atomic<uint8_t> g_xdev_host_stage_logged[GGML_CUDA_MAX_DEVICES][GGML_CUDA_MAX_DEVICES] = {}; // 0 = no, 1 = yes

static bool ggml_cuda_xdev_use_host_stage(int srcDevice, int dstDevice) {
    if (srcDevice == dstDevice) {
        return false;
    }

    const int lo = std::min(srcDevice, dstDevice);
    const int hi = std::max(srcDevice, dstDevice);

    const uint8_t cached = g_xdev_host_stage_policy[lo][hi].load(std::memory_order_relaxed);
    if (cached != 0) {
        return cached == 2;
    }

    std::lock_guard<std::mutex> lock(g_xdev_host_stage_policy_mutex);

    const uint8_t cached2 = g_xdev_host_stage_policy[lo][hi].load(std::memory_order_relaxed);
    if (cached2 != 0) {
        return cached2 == 2;
    }

    const char * host_stage_env = getenv("GGML_CUDA_HOST_STAGE");
    const int host_stage_val = host_stage_env ? atoi(host_stage_env) : -1;
    const bool force_direct = host_stage_env && host_stage_val == 0;
    const bool force_staged = host_stage_env && host_stage_val != 0;

    int can_access_lo_hi = 0;
    int can_access_hi_lo = 0;
    CUDA_CHECK(cudaDeviceCanAccessPeer(&can_access_lo_hi, lo, hi));
    CUDA_CHECK(cudaDeviceCanAccessPeer(&can_access_hi_lo, hi, lo));
    const bool p2p_available = (can_access_lo_hi != 0) && (can_access_hi_lo != 0);

    bool use_host_stage = false;
    const char * mode = "AUTO";
    if (force_direct) {
        mode = "FORCE_DIRECT";
        use_host_stage = false;
    } else if (force_staged) {
        mode = "FORCE_STAGED";
        use_host_stage = true;
    } else {
        use_host_stage = !p2p_available;
    }

    g_xdev_host_stage_policy[lo][hi].store(use_host_stage ? 2 : 1, std::memory_order_relaxed);

    if (g_xdev_host_stage_logged[lo][hi].exchange(1, std::memory_order_relaxed) == 0) {
        GGML_LOG_INFO("ggml_cuda_xdev: pair=%d<->%d host_stage=%s (mode=%s, p2p=%d, GGML_CUDA_HOST_STAGE=%s)\n",
                      lo, hi, use_host_stage ? "ON" : "OFF", mode, (int) p2p_available,
                      host_stage_env ? host_stage_env : "<unset>");
    }

    return use_host_stage;
}
...
// 1D cross-device memcpy with optional host staging
static cudaError_t ggml_cuda_MemcpyPeerAsync(
    ...
    const bool use_host_stage = ggml_cuda_xdev_use_host_stage(srcDevice, dstDevice);
    if (!use_host_stage) {
        GGML_UNUSED(srcStream);
        return cudaMemcpyPeerAsync(dst, dstDevice, src, srcDevice, size, stream);
    }

    // Producer-consumer gating: by default enabled for all host-staged transfers, to avoid races between the
    // kernel writing src on srcStream and the D2H stream reading from it. Can be disabled entirely by setting
    // GGML_CUDA_ACTXFER_GATE=0.
    const char * gate_env = getenv("GGML_CUDA_ACTXFER_GATE");
    bool gate_enabled = true;
    if (gate_env != nullptr) {
        gate_enabled = atoi(gate_env) != 0;
    }
...
static cudaError_t ggml_cuda_Memcpy2DPeerAsync(
    ...
    if (dstDevice != srcDevice) {
        // Fast path: contiguous region -> use 1D wrapper (policy handled inside)
        ...
        if (use_host_stage) {
            ...
            // GGML_CUDA_ACTXFER_GATE=0 disables this gating entirely.
            const char * gate_env = getenv("GGML_CUDA_ACTXFER_GATE");
            bool gate_enabled = true;
            if (gate_env != nullptr) {
                gate_enabled = atoi(gate_env) != 0;
...
#else
            // HIP does not support cudaMemcpy3DPeerAsync or vmm pools: peer per-row
