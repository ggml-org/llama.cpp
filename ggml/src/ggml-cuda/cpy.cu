...
#elif !defined(GGML_CUDA_USE_GRAPHS)
    GGML_UNUSED(disable_indirection_for_this_node);
    // When CUDA graphs are enabled, we must use copy kernels instead of cudaMemcpyAsync even for same-type contiguous tensors. This is because cudaMemcpyAsync cannot be
    // used with CUDA graph indirection (the mechanism that allows dynamic pointer updatesin captured graphs). Using cudaMemcpyAsync would force disabling CUDA graphs entirely,
    // causing significant performance regression in models like Nemotron Nano v2.
