...
__global__ __launch_bounds__(512) void q1_wgmma_ggml(const int8_t * __restrict__ Aq,
...
__global__ __launch_bounds__(512) void q1_wgmma_ggml_sk(const int8_t * __restrict__ Aq,
                                                        const float * __restrict__ dA,
                                                        const unsigned * __restrict__ Wbits,
                                                        const float * __restrict__ Wd,
                                                        float * __restrict__ C,
...
bool ggml_cuda_mul_mat_q1_hopper(ggml_backend_cuda_context & ctx,
    ...
    if (cc < 900 ||  // 900 = Hopper; no GGML_CUDA_CC_HOPPER macro in this tree
        src0->type != GGML_TYPE_Q1_0 || src1->type != GGML_TYPE_F32 || dst->type != GGML_TYPE_F32 ||
    ...
    if (ntiles < 8 * NSM) {
        ...
        if (!attr_sk_map[device]) {
            CUDA_CHECK(cudaFuncSetAttribute(hopper_q1::q1_wgmma_ggml_sk, cudaFuncAttributeMaxDynamicSharedMemorySize,
                                            SMEM_BYTES));
            attr_sk_map[device] = true;
        }
        const long total = (long) ntiles * (K / 128);
        cudaMemsetAsync(dst->data, 0, (size_t) M * N * sizeof(float), stream);
        hopper_q1::q1_wgmma_ggml_sk<<<NSM, 512, SMEM_BYTES, stream>>>(
            act_q.get(), act_d.get(), wq.bits, wq.dw, (float *) dst->data, (int) M, (int) N, (int) K, ntn, total, NSM);
    } else {
        dim3 grid(ntm, ntn);
        hopper_q1::q1_wgmma_ggml<<<grid, 512, SMEM_BYTES, stream>>>(act_q.get(), act_d.get(), wq.bits, wq.dw,
