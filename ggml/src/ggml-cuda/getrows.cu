...
template<dequantize_kernel_t dequantize_kernel, typename dst_t>
static __global__ void k_get_rows_nvfp4(
        const void * __restrict__ src0, const int32_t * __restrict__ src1, dst_t * __restrict__ dst,
        const int64_t ne00,
    ...
    for (int64_t z = blockIdx.z; z < ne11*ne12; z += gridDim.z) {
        for (int64_t i00 = 2*(blockIdx.y*blockDim.x + threadIdx.x); i00 < ne00; i00 += gridDim.y*blockDim.x) {
            ...
            const int iqs  = (i00 % QK_K) / 2;
            const int iybs =  i00 - i00 % QK_K;

            float2 v;
            dequantize_kernel(src0_row, ib, iqs, v);

            dst_row[iybs + 2*iqs + 0] = ggml_cuda_cast<dst_t>(v.x * tensor_scale);
            dst_row[iybs + 2*iqs + 1] = ggml_cuda_cast<dst_t>(v.y * tensor_scale);
        }
    }
}
...
template<dequantize_kernel_t dq, typename dst_t>
static void get_rows_cuda_nvfp4(
        const ggml_tensor * src0, const int32_t * src1_d, dst_t * dst_d,
        const int64_t ne00, const size_t nb01, const size_t nb02, const size_t nb03,
        const int64_t ne10, const int64_t ne11, const int64_t ne12, const size_t nb10, const size_t nb11, const size_t nb12,
        const size_t nb1, const size_t nb2, const size_t nb3,
        cudaStream_t stream) {
    ...
    const float tensor_scale = ggml_nvfp4_get_tensor_scale(src0);

    k_get_rows_nvfp4<dq><<<block_nums, block_dims, 0, stream>>>(
...
void ggml_cuda_op_get_rows(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src0->type == GGML_TYPE_NVFP4) {
        switch (dst->type) {
            case GGML_TYPE_F32:
                get_rows_cuda_nvfp4<dequantize_nvfp4>(src0, (const int32_t *) src1->data, (float *) dst->data,
                    ne00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb1, nb2, nb3, stream);
                return;
            case GGML_TYPE_I32:
                get_rows_cuda_nvfp4<dequantize_nvfp4>(src0, (const int32_t *) src1->data, (int32_t *) dst->data,
                    ne00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb1, nb2, nb3, stream);
                return;
            case GGML_TYPE_F16:
                get_rows_cuda_nvfp4<dequantize_nvfp4>(src0, (const int32_t *) src1->data, (half *) dst->data,
                    ne00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb1, nb2, nb3, stream);
                return;
            case GGML_TYPE_BF16:
                get_rows_cuda_nvfp4<dequantize_nvfp4>(src0, (const int32_t *) src1->data, (nv_bfloat16 *) dst->data,
                    ne00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb1, nb2, nb3, stream);
                return;
            default:
