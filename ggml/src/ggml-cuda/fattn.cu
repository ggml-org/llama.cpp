...
static void ggml_cuda_flash_attn_ext_mma_f16_switch_ncols2(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // sparse attention engages through one fixed tile shape per head dim, so check for it
    // before the GQA dispatch, whose ncols2 choice varies with mask and cache alignment
#if !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA)
...
size_t ggml_cuda_flash_attn_ext_get_alloc_size(int device, const ggml_tensor * dst) {
    ...
    switch (kernel) {
        ...
        case BEST_FATTN_KERNEL_MMA_F16:
            // the mma kernel dequantizes q8_0 K/V in its tile loaders, no f16 copy needed
            need_f16_K = !ggml_cuda_fattn_kv_q8_native(K);
            need_f16_V = !ggml_cuda_fattn_kv_q8_native(V);
            break;
