...
static constexpr __host__ __device__ int calc_max_nwarps(
    ...
    if (table_id == MMVQ_PARAMETERS_GB10) {
        const int generic = calc_max_nwarps(type, ncols_dst, MMVQ_PARAMETERS_GENERIC);
...
template <ggml_type type, int ncols_dst, bool has_fusion, bool small_k = false, bool halve_iters = false,
          bool dynamic_nwarps = false>
__launch_bounds__(calc_max_nwarps(type, ncols_dst, get_device_table_id(), small_k, halve_iters)*ggml_cuda_get_physical_warp_size(), 1)
static __global__ void mul_mat_vec_q(
    ...
    constexpr int max_nwarps = calc_max_nwarps(type, ncols_dst, table_id, small_k, halve_iters);
    ...
    // Read the launch width only in the reduced-warp specialization.
    const     int nwarps = dynamic_nwarps ? static_cast<int>(blockDim.y) : max_nwarps;
#else
...
#pragma unroll
    ...
    [[maybe_unused]] __shared__ float tmp_shared_gate[(has_fusion && (max_nwarps-1 > 0)) ? max_nwarps-1 : 1][ncols_dst][rows_per_cuda_block][warp_size];
...
static std::pair<dim3, dim3> calc_launch_params(
    ...
    int nwarps = calc_max_nwarps(type, ncols_dst, table_id, small_k, halve_iters);
    ...
    // Only applied for RDNA3/RDNA4 where MoE expert narrow matrices cause significant regression.
        if (ncols_dst == 1 && nwarps > 1 &&
            (table_id == MMVQ_PARAMETERS_RDNA3_0 || table_id == MMVQ_PARAMETERS_RDNA4)) {
        constexpr int qk  = ggml_cuda_type_traits<type>::qk;
        constexpr int qi  = ggml_cuda_type_traits<type>::qi;
        constexpr int vdr = get_vdr_mmvq(type);
...
static void mul_mat_vec_q_switch_fusion(
    ...
    if constexpr (c_ncols_dst == 1) {
        if (has_fusion) {
            ...
            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, true, small_k, halve_iters, dynamic_nwarps>, launch_params,
    ...
    const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(block_nums, block_dims, nbytes_shared, stream);
    ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, false, small_k, halve_iters, dynamic_nwarps>, launch_params,
        vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
        channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
        sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
}
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    const auto should_halve_iters = [&] {
        ...
        const int blocks_per_iter = calc_max_nwarps(type, 1, table_id) * blocks_per_iter_1warp;
        ...
        case 1: {
            ...
            const auto launch = [&](auto small_k_tag, auto halve_iters_tag) {
                ...
                constexpr bool c_promoted =
                    calc_max_nwarps(type, c_ncols_dst, MMVQ_PARAMETERS_GB10, false, true) !=
                    calc_max_nwarps(type, c_ncols_dst, MMVQ_PARAMETERS_GB10, false, false);

                constexpr bool c_halve_iters = decltype(halve_iters_tag)::value && c_promoted;

                const std::pair<dim3, dim3> dims = calc_launch_params<type>(c_ncols_dst, nrows_x, ncols_x, nchannels_dst,
                                                                              nsamples_dst, warp_size, table_id, c_small_k, c_halve_iters);
                const auto launch_kernel = [&](auto dynamic_nwarps_tag) {
                    constexpr bool c_dynamic_nwarps = decltype(dynamic_nwarps_tag)::value;
                    mul_mat_vec_q_switch_fusion<type, c_ncols_dst, c_small_k, c_halve_iters, c_dynamic_nwarps>(
                ...
                const int max_nwarps = calc_max_nwarps(type, c_ncols_dst, table_id, c_small_k, c_halve_iters);
                if (dims.second.y < static_cast<uint32_t>(max_nwarps)) {
