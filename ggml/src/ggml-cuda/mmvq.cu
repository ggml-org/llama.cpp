...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    if (has_ids && ncols_dst > 1) {
        ...
        const bool    use_small_k = should_use_small_k(ncols_dst);

        if (use_small_k) {
            std::pair<dim3, dim3> dims =
                calc_launch_params<type>(c_ncols_dst, nrows_x, nchannels_dst, ncols_dst, warp_size, table_id, true);
            ...
            std::pair<dim3, dim3> dims =
                calc_launch_params<type>(c_ncols_dst, nrows_x, nchannels_dst, ncols_dst, warp_size, table_id);
