...
#define FC_CB FC_bin_cb
    ...
    if (FC_RB) {
        ...
        if (FC_F == 1) {
            ...
            if (FC_OP == 0) {
                dst_row[i0] = src0_row[i0] + T(src1_row[i1]);
            }
            ...
            if (FC_OP == 0) {
                FOR_UNROLL (short j = 0; j < FC_F; ++j) {
                    res += T(((device const T1 *) (src1 + args.o1[j]))[i1]);
                }
            }
        ...
        if (FC_F == 1) {
            ...
            for (int i0 = tpitg.x; i0 < args.ne0; i0 += ntg.x) {
                ...
                if (FC_OP == 0) {
                    dst_ptr[i0] = src0_ptr[i0] + T(src1_ptr[i10]);
                }
...
typedef decltype(kernel_bin_fuse_impl<float, half,  float>) kernel_bin_fuse_f32_f16_f32_t;
...
template [[host_name("kernel_bin_fuse_f32_f16_f32")]]   kernel kernel_bin_fuse_f32_f16_f32_t kernel_bin_fuse_impl<float,  half,  float>;
template [[host_name("kernel_bin_fuse_f32_f16_f32_4")]] kernel kernel_bin_fuse_f32_f16_f32_t kernel_bin_fuse_impl<float4, half4, float4>;
...
// Correctness-first direct paged-KV attention. The page map translates each
// logical KV position to a physical cache row; K/V are never gathered.
struct tessera_paged_attn_args {
    uint d;
    uint dv;
    uint nq;
    uint hq;
...
template<typename T>
kernel void kernel_tessera_paged_attn(
    ...
    // dst is [dv, q_head, query, stream], so q_head is the
    // fastest-changing non-vector dimension.
    ...
    for (uint logical = 0; logical < args.n_kv; ++logical) {
        ...
        value += weight*float(v[v_index]);
    }
    dst[gid] = value/denom;
}

template [[host_name("kernel_tessera_paged_attn_f32")]] kernel void kernel_tessera_paged_attn<float>(
    constant tessera_paged_attn_args &, device const float *, device const float *, device const float *,
...
//   packed       [out_dim * nt * 32] uint     (nt = ceil(in_dim/640), 20 trits/u32 LSB-first)
//   page_scales  [out_dim * nt] half          (fp16, per-page max)
//   lane_scales  [out_dim * nt * 32] uchar     (int8 relative, per 20-trit lane)
//   input_vector [in_dim * n_tokens] half
//   output       [out_dim * n_tokens] half
//   outlier_{rows,cols,vals} are sparse addback (1.5 bpw effective)
//
// Reconstruction per trit (page p, lane l, trit v in lane, col k = p*640 + l*20 + v):
//   scale    = page_max(p) * (lane(l) / 127)
//   dequant  = (trit==1 ? +scale : trit==2 ? -scale : 0)
//   out[i,j] = sum_k dequant * B[k,j]  +  sum_outliers in_row(i) outlier_val * B[col,j]
//
// One threadgroup (64 threads) per (i, j) — simd reduction across 64.

constant int FC_tile640_in_dim  [[function_constant(FC_TILE640 + 0)]];
constant int FC_tile640_out_dim [[function_constant(FC_TILE640 + 1)]];
constant int FC_tile640_packing [[function_constant(FC_TILE640 + 2)]];
constant bool FC_tile640_input_f32 [[function_constant(FC_TILE640 + 3)]];
...
// Five base-3 digits packed as five two-bit fields. A Tile640 word contains
// four of these radix-243 groups, reducing twenty serial div/mod operations
// to four while preserving the exact 0/+/− trit encoding.
    ...
    uint  si  [[simdgroup_index_in_threadgroup]])
{
    ...
    const uint b = tgp.z;
    const int32_t in_dim  = FC_tile640_in_dim;
    const int32_t out_dim = FC_tile640_out_dim;
    ...
    const int32_t words_per_row = nt *
        (FC_tile640_packing != 0 ? 40 : T640_WORDS_PER_PAGE);
...
int32_t k = sl * 4;
    ...
    // column range for this row i; K_i = offsets[i+1] - offsets[i].
...
// [[kernel]] TILE640_MATMUL_ID — per-expert variant. For each (i, e, j) the kernel
// fetches the right expert's row block. We use a flat (i, eg) threadgrid where
// eg = e + n_expert_used * jg, so each threadgroup handles one (e, j) of one row i.
//
// Layout (v2 per-row K format):
//   packed:              [n_experts * out_dim * words_per_row] uint
//   page_scales:         [n_experts * out_dim * pages_per_row] ushort
//   lane_scales:         [n_experts * out_dim * words_per_row] uchar
//   outlier_row_offsets: [n_experts * (out_dim + 1)] uint   (per-expert CSR)
//   outlier_cols:        [total_outliers] uint
//   outlier_vals:        [total_outliers] ushort  (residual = original)
//   input:               [in_dim * n_tokens] half  (one slab per (b, s))
//   ids:                 [n_expert_used * n_tokens] int  (which expert for each (e, j))
//   act_scale:           [in_dim] or [n_experts * in_dim] half
//   output:              [out_dim * n_expert_used * n_tokens] half

kernel void kernel_TILE640_MATMUL_ID(
    constant ggml_metal_kargs_tile640_matmul_id & args,
    device const uint*    packed,
    device const half*    page_scales,
    device const uchar*   lane_scales,
    device const uint*    outlier_row_offsets,
    ...
    uint  si  [[simdgroup_index_in_threadgroup]])
{
    ...
    const int32_t in_dim        = FC_tile640_in_dim;
    const int32_t out_dim       = FC_tile640_out_dim;
    const int32_t n_broadcast   = args.ne11;
    const int32_t n_tokens      = args.ne13;       // for one (b, s) slab
    const int32_t n_expert_used = args.ne12;
    const int32_t tg_threads    = args.ne22;
    if (i >= out_dim) return;

    const int32_t e = eg % n_expert_used;
    const int32_t j = eg / n_expert_used;
    const int32_t expert_id = ids[eg];
    device const half* scale_row = act_scale +
        (args.ne21 > 1 ? (int64_t) expert_id * in_dim : 0);

    const int32_t nt            = (in_dim + T640_PAGE - 1) / T640_PAGE;
    const int32_t words_per_row = nt * T640_WORDS_PER_PAGE;
    const int32_t pages_per_row = nt;
    const int32_t words_per_expert  = out_dim * words_per_row;
    const int32_t pages_per_expert  = out_dim * pages_per_row;

    const int64_t expert_base = (int64_t) expert_id * words_per_expert;
    const int64_t ps_base     = (int64_t) expert_id * pages_per_expert;
    const int64_t ls_base     = (int64_t) expert_id * words_per_expert;
    const int64_t offs_base   = (int64_t) expert_id * (out_dim + 1);

    device const uint*   row_pack = packed       + expert_base + (int64_t)i * words_per_row;
    device const half*   row_ps   = page_scales  + ps_base     + (int64_t)i * pages_per_row;
    device const uchar*  row_ls   = lane_scales  + ls_base     + (int64_t)i * words_per_row;
    device const half*   in_col   = input        + (((int64_t)b * n_tokens + j) * n_broadcast + (e % n_broadcast)) * in_dim;
    device       float*  out_row  = output       + (((int64_t)b * n_tokens + j) * n_expert_used + e) * out_dim + i;

    float acc = 0.0f;
    for (int32_t wi = tid; wi < words_per_row; wi += tg_threads) {
        const int32_t p    = wi / T640_WORDS_PER_PAGE;
        const int32_t lane = wi % T640_WORDS_PER_PAGE;
        const int32_t col0 = p * T640_PAGE + lane * T640_LANE;
        const float page_max = float(row_ps[p]);
        const float scale    = page_max * (float)(row_ls[wi]) * (1.0f / 127.0f);
        ...
        for (int32_t group = 0; group < 4; ++group) {
            ...
            for (int32_t digit = 0; digit < 5; ++digit) {
                const int32_t col = col0 + group * 5 + digit;
                if (col >= in_dim) break;
                const uint d = (packed5 >> (2 * digit)) & 3u;
                const float input_value = float(in_col[col]) *
                    (args.ne21 != 0 ? float(scale_row[col]) : 1.0f);
                if (d == 1u) acc = fma(input_value,  scale, acc);
                else if (d == 2u) acc = fma(input_value, -scale, acc);
            }
        }
    }

    // Sparse outlier addback (v2 per-row K format). CSR offsets for the
    // selected expert, with K_i = offs_e[i+1] - offs_e[i].
    const int32_t row_off_lo = (int32_t) outlier_row_offsets[offs_base + i];
    const int32_t row_off_hi = (int32_t) outlier_row_offsets[offs_base + i + 1];
    const int32_t K_i = row_off_hi - row_off_lo;
    for (int32_t k = tid; k < K_i; k += tg_threads) {
        const int32_t gk  = row_off_lo + k;
        const int32_t col = (int32_t) outlier_cols[gk];
        if (col < in_dim) {
            const float v = float(outlier_vals[gk]);
            const float input_value = float(in_col[col]) *
                (args.ne21 != 0 ? float(scale_row[col]) : 1.0f);
            acc = fma(input_value, v, acc);
        }
    }

    acc = simd_sum(acc);
    threadgroup float shared_reduction[32];
    if (sl == 0) shared_reduction[si] = acc;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (tid == 0) {
        const uint nsimd = (tg_threads + 31) / 32;
        float total = 0.0f;
...
static inline float tile640_decode_element(
    device const uint*  packed,
    device const half*  page_scales,
    device const uchar* lane_scales,
    device const uint*  outlier_row_offsets,
    ...
    int32_t col)
{
    const int32_t pages_per_row = (row_width + T640_PAGE - 1) / T640_PAGE;
    const int32_t words_per_row = pages_per_row * T640_WORDS_PER_PAGE;
    const int32_t page = col / T640_PAGE;
    const int32_t in_page = col - page * T640_PAGE;
    const int32_t lane = in_page / T640_LANE;
    const int32_t trit = in_page - lane * T640_LANE;
    const int64_t word_index = (int64_t) row * words_per_row +
                               page * T640_WORDS_PER_PAGE + lane;

    uint rem = packed[word_index];
    for (int32_t i = 0; i < trit; ++i) {
        rem /= 3u;
    }
    const uint digit = rem % 3u;
    const float scale = float(page_scales[(int64_t) row * pages_per_row + page]) *
                        float(lane_scales[word_index]) * (1.0f / 127.0f);
    float value = digit == 1u ? scale : (digit == 2u ? -scale : 0.0f);

    const uint lo = outlier_row_offsets[row];
    const uint hi = outlier_row_offsets[row + 1];
    for (uint i = lo; i < hi; ++i) {
        if (outlier_cols[i] == (uint) col) {
            value += float(outlier_vals[i]);
            break;
        }
    }
    return value;
...
kernel void kernel_TILE640_GET_ROWS(
    constant ggml_metal_kargs_tile640_get_rows & args,
    device const uint*    packed,
    device const half*    page_scales,
    device const uchar*   lane_scales,
    device const uint*    outlier_row_offsets,
    ...
    uint gid [[thread_position_in_grid]])
{
    const uint n = (uint) args.row_width * (uint) args.n_ids;
    if (gid >= n) {
        return;
    }
    const int32_t id_index = gid / args.row_width;
    const int32_t col = gid - id_index * args.row_width;
    const int32_t row = ids[id_index];
    output[gid] = row >= 0 && row < args.n_rows
        ? tile640_decode_element(packed, page_scales, lane_scales,
                                 outlier_row_offsets, outlier_cols, outlier_vals,
                                 args.row_width, row, col)
        : 0.0f;
}

kernel void kernel_TILE640_DEQUANT(
    constant ggml_metal_kargs_tile640_dequant & args,
    device const uint*    packed,
    device const half*    page_scales,
    device const uchar*   lane_scales,
    device const uint*    outlier_row_offsets,
    ...
    uint gid [[thread_position_in_grid]])
{
    if (gid >= (uint) args.n_elements) {
        return;
    }
    const int32_t row = gid / args.row_width;
    const int32_t col = gid - row * args.row_width;
    output[gid] = row < args.n_rows
        ? tile640_decode_element(packed, page_scales, lane_scales,
                                 outlier_row_offsets, outlier_cols, outlier_vals,
                                 args.row_width, row, col)
        : 0.0f;
...
// Dense graph-resident importance-matrix observer. Each thread owns one
// channel and scans the token rows once.
constant bool FC_imatrix_observer_f16 [[function_constant(FC_IMATRIX_OBSERVER + 0)]];

kernel void kernel_IMATRIX_OBSERVER_CAST(
        constant ggml_metal_kargs_imatrix_observer & args [[buffer(0)]],
        device const float * src [[buffer(1)]],
        device uchar * storage [[buffer(3)]],
        uint channel [[thread_position_in_grid]]) {
    if (channel > (uint) args.ne0) {
        return;
    }
    const uint count = (uint) (args.ne1 * args.ne2 * args.ne3);
    device half * activation = (device half *) storage;
    device float * stats =
        (device float *) (storage + args.stats_offset);
    if (channel == (uint) args.ne0) {
        stats[4 * (uint) args.ne0] = (float) count;
        return;
    }
    float sum2 = 0.0f;
    float sumabs = 0.0f;
    ...
    for (uint row = 0; row < count; ++row) {
        const uint offset = row * (uint) args.ne0 + channel;
        const float value = src[offset];
        activation[offset] = half(value);
        const float magnitude = abs(value);
        const float square = value * value;
        sum2 += square;
        sumabs += magnitude;
        sum4 = fma(square, square, sum4);
        maxabs = max(maxabs, magnitude);
    }
    stats[channel] = sum2;
    stats[(uint) args.ne0 + channel] = sumabs;
    stats[2 * (uint) args.ne0 + channel] = sum4;
    stats[3 * (uint) args.ne0 + channel] = maxabs;
}

// The cast-and-observe path is the dense observer epilogue used by Tile640
// input conversion.  For prompt-sized row counts, give one SIMD group a
// channel: activation stores remain coalesced while moment accumulation stays
// in registers until a single final write per statistic.
kernel void kernel_IMATRIX_OBSERVER_CAST_TILED(
        constant ggml_metal_kargs_imatrix_observer & args [[buffer(0)]],
        device const float * src [[buffer(1)]],
        device uchar * storage [[buffer(3)]],
        uint channel [[threadgroup_position_in_grid]],
        uint lane [[thread_index_in_simdgroup]]) {
    const uint channels = (uint) args.ne0;
    const uint count = (uint) (args.ne1 * args.ne2 * args.ne3);
    device half * activation = (device half *) storage;
    device float * stats = (device float *) (storage + args.stats_offset);
    if (channel >= channels) {
        if (channel == channels && lane == 0) {
    ...
    float sum4 = 0.0f;
    float maxabs = 0.0f;
    for (uint row = lane; row < count; row += 32) {
        const uint offset = row * channels + channel;
        const float value = src[offset];
        activation[offset] = half(value);
        const float magnitude = abs(value);
        const float square = value * value;
    ...
    sum2 = simd_sum(sum2);
    sumabs = simd_sum(sumabs);
    sum4 = simd_sum(sum4);
    maxabs = simd_max(maxabs);
    if (lane == 0) {
        stats[channel] = sum2;
        stats[channels + channel] = sumabs;
        stats[2 * channels + channel] = sum4;
        stats[3 * channels + channel] = maxabs;
    }
}

kernel void kernel_IMATRIX_OBSERVER(
        constant ggml_metal_kargs_imatrix_observer & args [[buffer(0)]],
        device const uchar * src_bytes [[buffer(1)]],
        device const uchar * ids_bytes [[buffer(2)]],
        device float * dst [[buffer(3)]],
        uint gid [[thread_position_in_grid]]) {
    const uint work = (uint) args.ne0 + 1;
    if (gid >= work) {
        return;
    }
    ...
    const uint count = (uint) (args.ne1 * args.ne2 * args.ne3);
    if (channel < (uint) args.ne0) {
        for (int i3 = 0; i3 < args.ne3; ++i3) {
            for (int i2 = 0; i2 < args.ne2; ++i2) {
                for (int i1 = 0; i1 < args.ne1; ++i1) {
                    device const uchar * value_ptr =
                        src_bytes +
                        (ulong) channel * args.nb0 +
                        (ulong) i1 * args.nb1 +
                        (ulong) i2 * args.nb2 +
                        (ulong) i3 * args.nb3;
                    const float value = FC_imatrix_observer_f16
                        ? float(*(device const half *) value_ptr)
    ...
    if (channel < (uint) args.ne0) {
        dst[channel] = sum2;
        dst[(uint) args.ne0 + channel] = sumabs;
        dst[2 * (uint) args.ne0 + channel] = sum4;
        dst[3 * (uint) args.ne0 + channel] = maxabs;
    } else {
        dst[4 * (uint) args.ne0] = (float) count;
    }
}

// Long-context variant. One SIMD group owns one channel and divides the token
// axis across its 32 lanes before reducing the four moments in registers.
kernel void kernel_IMATRIX_OBSERVER_TILED(
        constant ggml_metal_kargs_imatrix_observer & args [[buffer(0)]],
        device const uchar * src_bytes [[buffer(1)]],
        device const uchar * ids_bytes [[buffer(2)]],
        device float * dst [[buffer(3)]],
    ...
    float sum4 = 0.0f;
    float maxabs = 0.0f;
    for (uint row = sl; row < count; row += 32) {
        const uint i1 = row % (uint) args.ne1;
        const uint plane = row / (uint) args.ne1;
        const uint i2 = plane % (uint) args.ne2;
        const uint i3 = plane / (uint) args.ne2;
        device const uchar * value_ptr =
            src_bytes +
            (ulong) channel * args.nb0 +
            (ulong) i1 * args.nb1 +
...
kernel void kernel_IMATRIX_OBSERVER_CLEAR(
        constant ggml_metal_kargs_imatrix_observer & args [[buffer(0)]],
        device float * dst [[buffer(3)]],
        uint gid [[thread_position_in_grid]]) {
    const uint work = (4 * (uint) args.ne0 + 1) * (uint) args.experts;
    if (gid < work) {
        dst[gid] = 0.0f;
    }
}

// Routed observer. Work is proportional to selected routes × channels rather
// than experts × selected routes × channels. Metal 3 device atomic_float is
// available on the M1 and lets independent selections update expert rows.
kernel void kernel_IMATRIX_OBSERVER_ROUTED(
        constant ggml_metal_kargs_imatrix_observer & args [[buffer(0)]],
        device const uchar * src_bytes [[buffer(1)]],
        device const uchar * ids_bytes [[buffer(2)]],
        device atomic_float * dst [[buffer(3)]],
        uint gid [[thread_position_in_grid]]) {
    const uint channels = (uint) args.ne0;
    const uint selections = (uint) args.ids_ne0 * (uint) args.ids_ne1;
    const uint work = channels * selections;
    if (gid >= work) {
        return;
    }

    const uint channel = gid % channels;
    const uint selection = gid / channels;
    const uint slot = selection % (uint) args.ids_ne0;
    const uint token = selection / (uint) args.ids_ne0;
    device const int * routed_ptr =
        (device const int *) (ids_bytes +
            (ulong) slot * args.ids_nb0 +
            (ulong) token * args.ids_nb1);
    const uint expert = (uint) *routed_ptr;
    const uint row = expert * (4 * channels + 1);

    device const uchar * value_ptr =
        src_bytes +
        (ulong) channel * args.nb0 +
        (ulong) (slot % (uint) args.ne1) * args.nb1 +
        (ulong) token * args.nb2;
    const float value = FC_imatrix_observer_f16
        ? float(*(device const half *) value_ptr)
        : *(device const float *) value_ptr;
    const float magnitude = abs(value);
    const float square = value * value;
    atomic_fetch_add_explicit(dst + row + channel, square, memory_order_relaxed);
    atomic_fetch_add_explicit(dst + row + channels + channel, magnitude, memory_order_relaxed);
    atomic_fetch_add_explicit(dst + row + 2 * channels + channel, square * square, memory_order_relaxed);
    // IEEE positive-float bit ordering is numeric ordering. The observer
    // only stores magnitudes, so a native integer max is equivalent to the
    // former compare/exchange retry loop and avoids serial retry traffic when
    // several prompt tokens route to the same expert/channel.
    device atomic_uint * max_bits = (device atomic_uint *) (dst + row + 3 * channels + channel);
    atomic_fetch_max_explicit(
        max_bits, as_type<uint>(magnitude), memory_order_relaxed);
    if (channel == 0) {
        atomic_fetch_add_explicit(
            dst + row + 4 * channels, 1.0f, memory_order_relaxed);
    }
}
