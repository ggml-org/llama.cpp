...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_conv_2d_dw_tiled(ggml_metal_library_t lib, const ggml_tensor * op) {
    assert(op->op == GGML_OP_CONV_2D_DW);
    ...
    snprintf(base, 256, "kernel_conv_2d_dw_tiled_%s_%s", ggml_type_name(op->src[0]->type), ggml_type_name(op->src[1]->type));
    snprintf(name, 256, "%s", base);
