...
// multi-column mat-vec (ne11 = 2..8) for Q4_0 / Q8_0 with f32 activations
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_r1(ggml_metal_library_t lib, const ggml_tensor * op, int r1) {
    GGML_TENSOR_LOCALS( int32_t, ne0, op->src[0], ne);
    GGML_TENSOR_LOCALS( int32_t, ne1, op->src[1], ne);
    ...
    switch (tsrc0) {
        ...
        case GGML_TYPE_Q8_0:
            {
                nsg = N_SG_Q8_0;
                nr0 = 4; // measured on M3 Ultra: 4 rows/thread beats 2 (N_R0_Q8_0) and 8 (register pressure)
                smem = 32*sizeof(float)*nr0;
            } break;
        default:
            {
                GGML_LOG_ERROR("mul_mv_r1: unsupported type %d\n", (int) tsrc0);
                GGML_ABORT("not implemented");
            }
    };

    GGML_ASSERT(r1 >= 2 && r1 <= 4);
    GGML_ASSERT(ne12 <= INT16_MAX && ne13 <= INT16_MAX);
    const int16_t r2 = (int16_t) (ne12 / ne02);
    const int16_t r3 = (int16_t) (ne13 / ne03);

    snprintf(base, 256, "kernel_mul_mv_%s_f32_r1_%d_nr0_%d", ggml_type_name(tsrc0), r1, nr0);
