...
static int ggml_metal_op_encode_impl(ggml_metal_op_t ctx, int idx) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_LOG_SOFT_MAX:
            {
                n_fuse = ggml_metal_op_log_soft_max(ctx, idx);
            } break;
...
int ggml_metal_op_log_soft_max(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    ...
    ggml_metal_kargs_log_soft_max args = {
        /*.ne00 =*/ ne00,
        /*.ne01 =*/ ne01,
        /*.ne02 =*/ ne02,
        /*.nb01 =*/ nb01,
    ...
    auto pipeline = ggml_metal_library_get_pipeline_log_soft_max(lib, op);
