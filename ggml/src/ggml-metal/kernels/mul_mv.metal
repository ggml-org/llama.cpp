...
// b-posit8 W8A8 (Anomly): same structure as the Q8_0 kernel; code -> lattice value, block scale 2^scale_exp
template<short NR0, typename args_t>
void kernel_mul_mv_bposit8_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    const int nb = args.ne00/QK_BPOSIT8;
  ...
  //device const block_bposit8 * x = (device const block_bposit8 *) (src0 + offset0);
    device const float      * y = (device const float      *) (src1 + offset1);

    // pointers to src0 rows
    device const block_bposit8 * ax[NR0];
    FOR_UNROLL (short row = 0; row < NR0; ++row) {
        const uint64_t offset0 = (r0 + row)*args.nb01 + (i12/FC_mul_mv_r2)*args.nb02 + (i13/FC_mul_mv_r3)*args.nb03;

        ax[row] = (device const block_bposit8 *) ((device char *) src0 + offset0);
    }
    ...
    device const float * yb = y + ib0*QK_BPOSIT8 + il*NQ;
    ...
    for (int ib = ib0; ib < nb; ib += NSG*NQ) {
        ...
        for (short row = 0; row < NR0; row++) {
            device const uint8_t * qs = ax[row][ib].qs + il*NQ;

            float sumq = 0.f;
            FOR_UNROLL (short i = 0; i < NQ; ++i) {
                sumq += bp8_lut_f[qs[i]] * yl[i];
            }

            sumf[row] += sumq*ldexp(1.0f, (int) ax[row][ib].scale_exp);
        }

        yb += NSG*NQ*QK_BPOSIT8;
    }
...
[[host_name("kernel_mul_mv_bposit8_f32")]]
kernel void kernel_mul_mv_bposit8_f32(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mv_bposit8_f32_impl<N_R0_BPOSIT8, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}
...
template [[host_name("kernel_mul_mv_ext_bposit8_f32_r1_2")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<2, block_bposit8, 32, dequantize_bposit8_t4>;
template [[host_name("kernel_mul_mv_ext_bposit8_f32_r1_3")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<3, block_bposit8, 32, dequantize_bposit8_t4>;
template [[host_name("kernel_mul_mv_ext_bposit8_f32_r1_4")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<4, block_bposit8, 32, dequantize_bposit8_t4>;
template [[host_name("kernel_mul_mv_ext_bposit8_f32_r1_5")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<5, block_bposit8, 32, dequantize_bposit8_t4>;
...
template [[host_name("kernel_mul_mv_id_bposit8_f32")]] kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_bposit8_f32_impl<N_R0_BPOSIT8>>>;
