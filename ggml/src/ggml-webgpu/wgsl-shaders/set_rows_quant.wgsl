...
fn atomic_store_masked(word_idx: u32, mask: u32, bits: u32) {
    loop {
...
fn store_u16_at_dst(byte_offset: u32, value: u32) {
    let word_idx = byte_offset / 4u;
    let shift = (byte_offset & 2u) * 8u;
    let mask = 0xFFFFu << shift;
    atomic_store_masked(word_idx, mask, (value & 0xFFFFu) << shift);
}

fn store_u32_at_dst(byte_offset: u32, value: u32) {
    let word_idx = byte_offset / 4u;
    ...
    atomic_store_masked(word_idx, lo_mask, value << shift);
    atomic_store_masked(word_idx + 1u, hi_mask, value >> (32u - shift));
}

@compute @workgroup_size(WG_SIZE)
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    let blocks_per_row = params.ne0 / BLOCK_SIZE;
    let total_blocks = params.ne3 * params.ne2 * params.n_rows * blocks_per_row;
    if (gid.x >= total_blocks) {
        return;
    }

    var i = gid.x / blocks_per_row;
    let block_in_row = gid.x % blocks_per_row;
    ...
    store_u16_at_dst(dst_block_byte, packed_d);

    for (var j: u32 = 0u; j < QS_WORDS; j++) {
        let base = src_block + j * 4u;
        let packed_q = (bitcast<u32>(i32(round(src[base + 0u] * id))) & 0xFFu) |
        ...
        store_u32_at_dst(dst_block_byte + 2u + j * 4u, packed_q);
    }
#elif defined(DST_Q4_0)
    var amax = 0.0;
    var max_val = 0.0;
