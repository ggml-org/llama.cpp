...
fn main(@builtin(workgroup_id) wid: vec3<u32>,
    ...
    shmem_idx[lid.x] = select(params.ne0, idx, idx < params.ne0);
    workgroupBarrier();
    ...
    while (k <= WG_SIZE) {
        ...
        while (j > 0) {
            ...
            if (ixj > lid.x) {
                ...
                let a_val = select(EXTREME_VALUE, src[row_base + a_idx], a_idx < params.ne0);
                let b_val = select(EXTREME_VALUE, src[row_base + b_idx], b_idx < params.ne0);
    ...
    if (idx < params.ne0) {
        ...
        dst[row_dst + idx] = i32(shmem_idx[lid.x]);
    }
}
