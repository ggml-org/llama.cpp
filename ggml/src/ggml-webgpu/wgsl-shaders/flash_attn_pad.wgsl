...
struct Params {
    ...
    ncpsg: u32, // number of context positions per group, used for padding logic
    nqptg: u32, // number of Q positions per group, used for padding logic

    nwg: u32, // total number of workgroups, used for padding logic
};
              ...
              for (var kv_base : u32 = 0u; kv_base < KV_TILE; kv_base += 4u) {
                  ...
                  if (kv_valid) {
                    for (var i = tx; i < (HEAD_DIM_QK / 4u); i += num_of_threads) {
                        ...
                        var idx : u32 = 0u;
                        if (use_pad_tile == true) {
                            idx = params.offset_pad + params.pad_k_base + kv_plane * params.stride_k1 * params.ncpsg + kv_idx * params.stride_k1 + i * 4u;
                  ...
                  for (var g: u32 = 0u; g < 4u; g++) {
                      let kv_idx_g = kv_base + g;
                      let active_threads = (ty == g) && (kv_idx_g < KV_TILE) && ((kv_tile + kv_idx_g) < params.seq_len_kv);

                      let contrib = select(0.0, partial_sum, active_threads);
                      let sum_g = subgroupAdd(contrib);

                      if (tx == 0u && ty == g && kv_idx_g < KV_TILE) {
                        let dst_idx = q_tile_row * KV_TILE + kv_idx_g;
                        inter_shmem[dst_idx] = f16(select(FLOAT_MIN, sum_g, (kv_tile + kv_idx_g) < params.seq_len_kv));
                      }
                  }
              }
          }
      ...
      for (var q_tile_row = subgroup_id;
           ...
           q_tile_row += num_subgroups) {
              for (var elem_base = sg_inv_id * 4u; elem_base < HEAD_DIM_V; elem_base += subgroup_size * 4u) {
                ...
                var acc = vec4<f32>(f32(o_shmem[o_base_idx]), f32(o_shmem[o_base_idx + 1u]), f32(o_shmem[o_base_idx + 2u]), f32(o_shmem[o_base_idx + 3u]));
                for (var kv_idx : u32 = 0u; kv_idx < KV_TILE; kv_idx += 1u) {
                    let p = f32(inter_shmem[kv_idx + q_tile_row * KV_TILE]);
                    let v_row = kv_tile + kv_idx;
                    if (v_row >= params.seq_len_kv) {
                        continue;
                    }
                    // let v_idx = v_head_offset + v_row * params.stride_v1 + elem_base;
                    var v_idx : u32 = 0u;
                    var v4: vec4<f32>;
                    if (use_pad_tile == true) {
                        v_idx = params.offset_pad + params.pad_v_base + kv_plane * params.stride_v1 * params.ncpsg + kv_idx * params.stride_v1 + elem_base;
                        v4 = vec4<f32>(f32(pad[v_idx]), f32(pad[v_idx + 1u]), f32(pad[v_idx + 2u]), f32(pad[v_idx + 3u]));
                    } else {
                        v_idx = v_head_offset + v_row * params.stride_v1 + elem_base;
                        v4 = vec4<f32>(V[v_idx/4u]);
                    }
                    acc += p * v4;
                }
                // todo: write acc back to o_shmem
                o_shmem[o_base_idx] = f16(acc.x);
                o_shmem[o_base_idx + 1u] = f16(acc.y);
                o_shmem[o_base_idx + 2u] = f16(acc.z);
                o_shmem[o_base_idx + 3u] = f16(acc.w);
              }
          }
