...
#define VTCM_LAYOUT_ALLOC(field, sz) do { (L)->field = off; off += (sz); } while (0)
#define VTCM_LAYOUT_ALLOC_OPTIONAL(field, sz, cond) do { if (cond) { VTCM_LAYOUT_ALLOC(field, sz); } else { (L)->field = 0; } } while (0)
