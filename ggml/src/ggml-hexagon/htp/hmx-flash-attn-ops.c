...
static size_t hmx_fa_compute_vtcm_usage(size_t gqa_factor, size_t DK, size_t DV, size_t Br, size_t Bc,
                                        size_t n_threads) {
...
static void fa_k_interleave_thread(unsigned int n, unsigned int i, void * data) {
    ...
    interleave_rows_to_tiles(factx->vtcm_k_tiles,
                             factx->vtcm_k_fp16[args->buf_idx],
                             total_rows, (int) factx->DK, (int) args->src_stride,
                             start, end);
}

static void fa_phase_k_interleave(struct hmx_fa_context * factx, int kv_rows,
                                   size_t src_stride, size_t buf_idx) {
...
static void fa_v_interleave_thread(unsigned int n, unsigned int i, void * data) {
    ...
    interleave_cols_to_tiles(factx->vtcm_v_tiles,
                             factx->vtcm_v_fp16[args->buf_idx],
                             total_rows, (int) factx->DV, (int) args->src_stride,
                             (int) args->n_col_tiles,
                             start, end);
}

static void fa_phase_v_interleave(struct hmx_fa_context * factx, int kv_rows,
                                   size_t src_stride, size_t buf_idx, size_t n_col_tiles) {
...
static void fa_softmax_thread(unsigned int n, unsigned int i, void * data) {
    ...
    if (vec_start >= n_row_vec_cnt) return;
            ...
            for (size_t c = 0; c < kv_rows; c += 64) {
                ...
                if (args->mask) {
                    ...
                    if (args->mask_vtcm) {
                        ...
                        const __fp16 * m0_ptr =
                            (const __fp16 *) ((const uint8_t *) mask->data + q_idx0 * mask->nb[1] +
            ...
            for (size_t c = 0; c < kv_rows; c += 64) {
                ...
                *pv_p_out1               = Q6_V_hi_W(vp_p_dual);
                ...
                v_p_rowsum0 = Q6_Vqf32_vadd_Vqf32Vqf32(
                    v_p_rowsum0, Q6_Vqf32_vadd_VsfVsf(Q6_V_lo_W(vp_p0), Q6_V_hi_W(vp_p0)));
...
static void fa_phase_softmax_and_build_d(struct hmx_fa_context * factx,
    ...
    if (factx->n_threads > 1 && n_row_vec_cnt >= factx->n_threads) {
        worker_pool_run_func(factx->worker_pool, fa_softmax_thread, sargs, factx->n_threads);
    } else {
...
static void fa_compute_slopes(fa_softmax_args_t * sargs,
                              ...
                              uint32_t kv_head, size_t n_rows_g) {
    ...
    for (uint32_t ib3 = 0; ib3 < neq3; ++ib3) {
        for (uint32_t kv_head = 0; kv_head < n_kv_heads; ++kv_head) {
            ...
            for (uint32_t q_start = 0; q_start < neq1; q_start += Br) {
                ...
                {
                    if (n_rows_g < g_br) {
                        hvx_fill_uh(factx.vtcm_q_tiles, 0, q_tile_bytes);
                    }

                    for (size_t r = 0; r < n_rows_g; r += 2) {
                        ...
                        const uint8_t * q_ptr1 = next_row_valid ?
                                                     ((const uint8_t *) q->data + (q_start + q_idx1) * q->nb[1] +
                ...
                hvx_fill_uh(factx.vtcm_m_vec, 0xfbff, col_vec_bytes);
                hvx_fill_uh(factx.vtcm_l_vec, 0, col_vec_bytes);
                hvx_fill_uh(factx.vtcm_d_tiles, 0, d_tile_bytes);

                __fp16 * o_tile_prev = factx.vtcm_o_tiles[0];
                __fp16 * o_tile_curr = factx.vtcm_o_tiles[1];
                hvx_fill_uh(o_tile_prev, 0, o_tile_bytes);
                ...
                if (factx.n_kv_blocks > 0) {
                    ...
                    const uint8_t * k_src =
                        (const uint8_t *) k->data + ik2 * k->nb[2] + ik3 * k->nb[3];
                    dma_queue_push(dma, dma_make_ptr(factx.vtcm_k_fp16[0], k_src), size_k_row_padded, k->nb[1],
                                   size_k_row, kv_rows0);

                    const uint8_t * v_src =
                        (const uint8_t *) v->data + iv2 * v->nb[2] + iv3 * v->nb[3];
                ...
                #define MASK_DMA_PUSH(kv_start_val, kv_rows_val, has_mask_dma_var)                       \
                    do {                                                                                  \
                        ...
                        if (mask && factx.mask_broadcast) {                                                \
                            ...
                            const uint8_t * _ms  = (const uint8_t *) mask->data                            \
                                                   + q_start * mask->nb[1]                                 \
                                                   + _im3 * mask->nb[3]                                    \
                                                   + (kv_start_val) * sizeof(__fp16);                       \
                            dma_queue_push(dma,                                                             \
                                dma_make_ptr(factx.vtcm_mask_buf, _ms),                                    \
                                m_line_bytes,                                                               \
                                mask->nb[1],                                                                \
                                (kv_rows_val) * sizeof(__fp16),                                             \
                                n_q_rows);                                                                  \
                ...
                #define DMA_PREFETCH_KV(blk_val)                                                        \
                    do {                                                                                 \
                        if ((blk_val) < factx.n_kv_blocks) {                                             \
                            ...
                            const uint8_t * _ks = (const uint8_t *) k->data + _ns * k->nb[1]            \
                                                  + ik2 * k->nb[2] + ik3 * k->nb[3];                    \
                            dma_queue_push(dma, dma_make_ptr(factx.vtcm_k_fp16[_nb], _ks),               \
                                           size_k_row_padded, k->nb[1], size_k_row, _nr);               \
                            const uint8_t * _vs = (const uint8_t *) v->data + _ns * v->nb[1]            \
                                                  + iv2 * v->nb[2] + iv3 * v->nb[3];                    \
                            dma_queue_push(dma, dma_make_ptr(factx.vtcm_v_fp16[_nb], _vs),               \
                                           size_v_row_padded, v->nb[1], size_v_row, _nr);               \
                        }                                                                                \
                    } while (0)
                ...
                if (factx.use_pipeline) {
                    ...
                    for (uint32_t kv_blk = 0; kv_blk < factx.n_kv_blocks; ++kv_blk) {
                        ...
                        if (kv_blk > 0) {
                            ...
                            ou_job.n_col_tiles      = hmx_ceil_div(hex_smin(Bc, nek1 - (kv_blk - 1) * Bc),
                    ...
                    if (factx.n_kv_blocks > 0) {
                        ...
                        const size_t   last_cols   = hmx_ceil_div(hex_smin(Bc, nek1 - last_blk * Bc),
                                                                  HMX_FP16_TILE_N_COLS);
                        ou_job.o_curr           = o_tile_curr;
                        ou_job.o_prev           = o_tile_prev;
                        ou_job.p_tiles          = factx.vtcm_p_tiles;
                ...
                TIMER_START(o_store);
                {
                    for (size_t r = 0; r < n_rows_g; ++r) {
                        ...
                        size_t         r0            = r / HMX_FP16_TILE_N_ROWS;
                        size_t         r1            = r % HMX_FP16_TILE_N_ROWS;
                        const __fp16 * tile_row_base = o_tile_curr + r0 * HMX_FP16_TILE_N_ROWS * DV;

                        if (factx.is_dst_fp32) {
                            float * out = (float *) dst_row;
                            for (uint32_t d = 0; d < DV / 32; ++d) {
                                const HVX_Vector * in_tile =
