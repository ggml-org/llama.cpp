...
bool ggml_backend_meta_get_mirrored_tensor(
    ...
    if (!ggml_backend_is_meta(meta_backend) || tensor == nullptr || tensor->buffer == nullptr ||
            !ggml_backend_buffer_is_meta(tensor->buffer) || simple_tensor == nullptr || simple_backend == nullptr) {
        return false;
    }

    auto * backend_ctx = (ggml_backend_meta_context *) meta_backend->context;
    ...
    ggml_tensor * resolved = ggml_backend_meta_buffer_simple_tensor(tensor, index);
    if (resolved == nullptr) {
        auto & stc = backend_ctx->stc_compute[backend_ctx->stc_index];
        const auto it = stc.simple_tensors.find(tensor);
        if (it == stc.simple_tensors.end() || index >= it->second.size()) {
            return false;
        }
        resolved = it->second[index];
    }
    if (resolved == nullptr || !ggml_are_same_shape(tensor, resolved) || !ggml_is_contiguous(resolved)) {
        return false;
    }

    *simple_tensor = resolved;
    *simple_backend = backend_ctx->backend_configs[index].backend;
    return *simple_backend != nullptr;
}
