...
struct ggml_backend_meta_buffer_type_context {
    ...
    ggml_backend_meta_buffer_type_context(std::vector<ggml_backend_buffer_type_t> types) : simple_bufts(std::move(types)) {
        name = "Meta(";
...
struct ggml_backend_meta_split_context {
    ...
    std::map<std::pair<const ggml_tensor *, bool>, std::pair<ggml_backend_meta_split_state, char[nbtc]>> cache;

    ggml_backend_meta_split_context(ggml_backend_buffer_type_t buft, ggml_backend_buffer_usage usage)
        : buft(buft), usage(usage) {
        GGML_ASSERT(ggml_backend_buft_is_meta(buft));
...
struct ggml_backend_meta_buffer_context {
    ...
    explicit ggml_backend_meta_buffer_context(ggml_backend_buffer_type_t buft) : split(buft, GGML_BACKEND_BUFFER_USAGE_ANY) {}
...
static size_t ggml_backend_meta_buffer_n_bufs(ggml_backend_buffer_t meta_buf) {
    ...
    return ggml_backend_meta_buft_n_bufts(meta_buf->buft);
}
...
static struct ggml_tensor * ggml_backend_meta_buffer_simple_tensor(const struct ggml_tensor * tensor, size_t index) {
    ...
    GGML_ASSERT(index < ggml_backend_meta_buffer_n_bufs(tensor->buffer));
...
static struct ggml_backend_meta_split_state ggml_backend_meta_get_split_state(
        ggml_backend_meta_split_context & split_ctx, const struct ggml_tensor * tensor, bool assume_sync) {
    ...
    const size_t n_bufs = ggml_backend_meta_buft_n_bufts(split_ctx.buft);
    ...
    auto calculate_split_state = [&]() -> ggml_backend_meta_split_state {
        ...
        for (size_t i = 0; i < GGML_MAX_SRC; i++) {
            ...
            src_ss[i] = ggml_backend_meta_get_split_state(split_ctx, tensor->src[i], /*assume_sync =*/ true);
    ...
    auto it = split_ctx.cache.find(key);
    if (it != split_ctx.cache.end() && memcmp(it->second.second, (const char *) tensor, sizeof(it->second.second)) != 0) {
        split_ctx.cache.clear();
        it = split_ctx.cache.end();
    }
        ...
        if (split_ctx.debug > 0) {
            auto format_ne = [n_bufs](const ggml_backend_meta_split_state & ss) {
                ...
                for (size_t j = 0; j < n_bufs; j++) {
                    ...
                    for (size_t s = 0; s < ss.n_segments; s++) {
                        ne += ss.ne[s*n_bufs + j] * ss.nr[s];
                    }
                    if (!result.empty()) {
                        result += ", ";
                    }
                    result += std::to_string(ne);
                }
                return result;
            };
            ...
            for (size_t i = 0; i < GGML_MAX_SRC; i++) {
                ...
                const ggml_backend_meta_split_state split_state =
                        ggml_backend_meta_get_split_state(split_ctx, tensor->src[i], true);
                const char * axis_name = ggml_backend_meta_split_axis_name(split_state.axis);
                const std::string ne_info = format_ne(split_state);
                srcs_info += std::string(tensor->src[i]->name) + "[" + ggml_op_name(tensor->src[i]->op) + ", " + axis_name + ", {" + ne_info + "}]";
            }
            const std::string ne_info = format_ne(split_ctx.cache[key].first);
            GGML_LOG_DEBUG("SPLIT_STATE: {%s} -> %s[%s, %s, {%s}]\n", srcs_info.c_str(), tensor->name, ggml_op_name(tensor->op),
                ggml_backend_meta_split_axis_name(split_ctx.cache[key].first.axis), ne_info.c_str());
        }
    }

    ggml_backend_meta_split_state ret = split_ctx.cache[key].first;
...
static struct ggml_backend_meta_split_state ggml_backend_meta_get_split_state(const struct ggml_tensor * tensor, bool assume_sync) {
    GGML_ASSERT(ggml_backend_buffer_is_meta(tensor->buffer));
    ggml_backend_meta_buffer_context * buf_ctx = (ggml_backend_meta_buffer_context *) tensor->buffer->context;
    return ggml_backend_meta_get_split_state(buf_ctx->split, tensor, assume_sync);
}

ggml_backend_meta_split_context * ggml_backend_meta_split_context_new(ggml_backend_buffer_type_t buft, ggml_backend_buffer_usage usage) {
    return new ggml_backend_meta_split_context(buft, usage);
}

void ggml_backend_meta_split_context_free(ggml_backend_meta_split_context * ctx) {
    delete ctx;
}

ggml_backend_meta_split_state ggml_backend_meta_split_context_get(
        ggml_backend_meta_split_context * ctx, const ggml_tensor * tensor, bool assume_sync) {
    GGML_ASSERT(ctx);
    return ggml_backend_meta_get_split_state(*ctx, tensor, assume_sync);
}
...
static void ggml_backend_meta_buffer_clear(ggml_backend_buffer_t buffer, uint8_t value) {
    ...
    for (const auto & physical : ctx->bufs) {
        ggml_backend_buffer_clear(physical.get(), value);
    }
}
...
static const ggml_backend_buffer_i ggml_backend_meta_buffer_iface = {
    ...
    /* .get_base        = */ nullptr,
    /* .init_tensor     = */ ggml_backend_meta_binding_init_tensor,
...
static ggml_status ggml_backend_meta_binding_init_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor) {
    auto * ctx = static_cast<ggml_backend_meta_buffer_context *>(buffer->context);
    auto it = ctx->bindings.find(tensor);
    if (ctx->retired || it == ctx->bindings.end() || tensor->data ||
...
static ggml_backend_buffer_t ggml_backend_meta_materialize(void * preparation, ggml_backend_buffer_set * domains, size_t n_domains) {
    auto * prepared = static_cast<ggml_backend_meta_preparation *>(preparation);
    if (!prepared || prepared->failed || n_domains != prepared->tensors.ctxs.size() || !domains) {
        return nullptr;
    }
    try {
        auto ctx = std::make_unique<ggml_backend_meta_buffer_context>(prepared->split.buft);
        std::set<ggml_backend_buffer_t> owned;
        size_t size = 0;
        for (size_t domain = 0; domain < n_domains; domain++) {
            const auto & buffers = domains[domain];
            if (buffers.n_buffers && !buffers.buffers) {
                return nullptr;
            }
            for (size_t i = 0; i < buffers.n_buffers; i++) {
                auto * buffer = buffers.buffers[i];
                if (!buffer || buffer->buft != ggml_backend_meta_buft_simple_buft(prepared->split.buft, domain) ||
                        buffer->binding || ggml_backend_buffer_is_multi_buffer(buffer) || !owned.insert(buffer).second ||
                        buffer->size > SIZE_MAX - size) {
                    return nullptr;
                }
                size += buffer->size;
            }
            ctx->domain_ends.push_back(owned.size());
            for (const auto & entry : prepared->tensors.simple_tensors) {
                auto * tensor = entry.second[domain];
                if (!tensor->buffer || !ggml_backend_tensor_is_bound(tensor)) {
                    return nullptr;
                }
                if (tensor->view_src) {
                    if (tensor->buffer != tensor->view_src->buffer ||
                            tensor->view_offs > ggml_nbytes(tensor->view_src) ||
                            ggml_nbytes(tensor) > ggml_nbytes(tensor->view_src) - tensor->view_offs) {
                        return nullptr;
                    }
                } else if (!buffers.n_buffers ||
                        std::find(buffers.buffers, buffers.buffers + buffers.n_buffers, tensor->buffer) == buffers.buffers + buffers.n_buffers) {
                    return nullptr;
                }
            }
        }
        ...
        owner->usage = prepared->split.usage;
        materialized->tensors = std::move(prepared->tensors);
        materialized->split = std::move(prepared->split);
        materialized->split.sources = {};
        for (size_t domain = 0; domain < n_domains; domain++) {
            auto & buffers = domains[domain];
            for (size_t i = 0; i < buffers.n_buffers; i++) {
                ggml_backend_buffer_set_usage(buffers.buffers[i], owner->usage);
                materialized->bufs.emplace_back(buffers.buffers[i]);
            }
...
void ggml_backend_meta_buffer_set_usage(ggml_backend_buffer_t buffer, enum ggml_backend_buffer_usage usage) {
    GGML_ASSERT(ggml_backend_buffer_is_meta(buffer));
    ggml_backend_meta_buffer_context * buf_ctx = (ggml_backend_meta_buffer_context *) buffer->context;
    if (buf_ctx->split.usage != usage) {
        buf_ctx->split.usage = usage;
        buf_ctx->split.cache.clear();
    }
    for (size_t i = 0; i < buf_ctx->bufs.size(); i++) {
        if (buf_ctx->bufs[i]) {
...
static size_t ggml_backend_meta_n_physical_buffers(ggml_backend_buffer_t owner, size_t domain) {
    ...
    GGML_ASSERT(domain < ctx->domain_ends.size());
...
static ggml_status ggml_backend_meta_release_buffers(ggml_backend_buffer_t owner, ggml_backend_buffer_set * domains, size_t n_domains) {
    ...
    auto * ctx = static_cast<ggml_backend_meta_buffer_context *>(owner->context);
    if (!domains || n_domains != ctx->domain_ends.size()) {
        return GGML_STATUS_FAILED;
    }
    for (size_t domain = 0; domain < n_domains; domain++) {
...
static ggml_backend_buffer_t ggml_backend_meta_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    GGML_LOG_ERROR("%s: %s requires tensor allocation metadata (requested %zu bytes)\n", __func__, ggml_backend_buft_name(buft), size);
    return nullptr;
}
