...
int entry_point(struct ggml_et_mul_mat_id_params* params, void* env) {
    ...
    if (src0->nb[0] != sizeof(block_q4_0) ||
    ...
    while (idx < my_end) {
        ...
        const uint64_t run_end_global = (uint64_t)batch_idx * per_batch
                                      + (uint64_t)slot_idx * (uint64_t)M
                                      + (uint64_t)M;
        ...
        const int32_t expert_id = *(const int32_t*)((const char*)src2_data
                                  + slot_idx * (int64_t)nb20
                                  + batch_idx * (int64_t)nb21);

        char* dst_slot = (char*)dst_data + slot_idx * (int64_t)nbd1
                                         + batch_idx * (int64_t)nbd2;
        ...
        const float* b_col_base = (const float*)((const char*)src1_data
                                  + col_idx * (int64_t)nb11
                                  + batch_idx * (int64_t)nb12);
