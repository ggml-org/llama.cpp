...
/*
 * High-performance F16 Matrix Multiply for ET-SoC-1 — TensorFMA16.
 * Double-buffered producer/consumer implementation.
 *
 * Hart 1 (producer): Interleaves and transposes weights into double-buffered L2 SCP.
...
#define B_L1_START 16   // L1 SCP lines 16..31 for B (single buffer)
#define A_L1_ALT   32   // L1 SCP lines 32..47 for A (buffer 1)

typedef uint16_t et_fp16_t;

#define SCP_PANEL_SIZE   (PANEL_LINES * TILE_K * (uint64_t)sizeof(et_fp16_t))  // 1024 bytes
...
pack_b_interleaved(et_fp16_t *out, const char *src0_batch,
    ...
    for (int j = 0; j < TILE_M; ++j) {
        ...
        __asm__ volatile(
            "flw.ps      f2, 0(%[src])  \n\t"   // load row[0..15] (32 bytes)
            "flw.ps      f3, 32(%[src]) \n\t"   // load row[16..31] (32 bytes)
            "fscw.ps     f2, f1(%[c0])  \n\t"   // store row[0..15] to lines 0..7, col j
            "fscw.ps     f3, f1(%[c8])  \n\t"   // store row[16..31] to lines 8..15, col j
            :
        ...
        if (is_hart1) {
            ...
            for (int64_t unit = my_start; unit < base_units; unit += tiles_stride) {
                ...
                for (int64_t kw = 0; kw < n_windows; ++kw) {
                    ...
                    et_fp16_t *cf = (et_fp16_t *) cache_buf[buf];
                    for (int64_t i = 0; i < kbn; ++i) {
                        pack_b_interleaved(cf + i * (SCP_PANEL_SIZE / sizeof(et_fp16_t)),
                                           src0_batch, mb, (kb0 + i) * TILE_K, nb1_0);
                    }
                    FENCE;
                    flush_to_l2_multi(cache_buf[buf], kbn * PANEL_LINES, 64);
                    WAIT_CACHEOPS;
...
#define A_TILE_ADDR(st)                                                                  \
    ((uint64_t)(src1_batch + nb * nb1_1 +                                                \
        (kb0 + (st)) * TILE_K * (int64_t) sizeof(et_fp16_t)))
                    ...
                    for (int64_t s = 0; s < nsteps; ++s) {
                        ...
                        // Start loading the next step of A
                        if (s + 1 < nsteps) {
                            const uint64_t a_nxt = ((s + 1) & 1) ? A_L1_ALT : A_L1_START;
                            tensor_load((n_cur < TILE_N), false, a_nxt, TENSOR_LOAD_PLAIN, 0,
                        ...
                        // Load B from L2 SCP to L1 SCP (TL1 engine, B_L1_START)
                        tensor_load(false, false, B_L1_START, TENSOR_LOAD_PLAIN, 0,
                                    (uint64_t)(cf + s * (SCP_PANEL_SIZE / sizeof(et_fp16_t))),
                                    0, 15, 64, 1);

                        tensor_wait(TENSOR_LOAD_WAIT_0);
                        tensor_wait(TENSOR_LOAD_WAIT_1);

                        tensor_fma((n_cur < TILE_N), 3, n_cur - 1, 15, 0,
                                   false, false, false, false,
                                   B_L1_START, a_cur, TENSOR_FMA_OP_FP16, first);
...
#undef A_TILE_ADDR
    ...
    et_fp16_t *scp_panel[2] = {
        (et_fp16_t *) et_shire_l2scp_local(scp_base),
        (et_fp16_t *) et_shire_l2scp_local(scp_base + SCP_PANEL_SIZE),
    };
    ...
    if (is_hart1) {
        ...
        for (int64_t tile = my_start; tile < base_tiles; tile += tiles_stride) {
            ...
            for (int64_t kb = k_start; kb < k_end; kb += TILE_K) {
                ...
                pack_b_interleaved(scp_panel[buf], src0_batch, mb, kb, nb1_0);
    ...
    for (int64_t tile = my_start; tile < base_tiles; tile += tiles_stride) {
        ...
        l2_prefetch((const void *)(src1_batch + nb * nb1_1 + k_start * sizeof(et_fp16_t)), n_cur, nb1_1);
        if (k_start + TILE_K < k_end) {
            l2_prefetch((const void *)(src1_batch + nb * nb1_1 + (k_start + TILE_K) * sizeof(et_fp16_t)), n_cur, nb1_1);
        }
        tensor_load(
            (n_cur < TILE_N), false, A_L1_START, TENSOR_LOAD_PLAIN, 0,
            (uint64_t)(src1_batch + nb * nb1_1 + k_start * (int64_t) sizeof(et_fp16_t)),
        ...
        for (int64_t kb = k_start; kb < k_end; kb += TILE_K) {
            ...
            if (kb + TILE_K < k_end) {
                ...
                tensor_load(
                    (n_cur < TILE_N), false, a_nxt, TENSOR_LOAD_PLAIN, 0,
                    (uint64_t)(src1_batch + nb * nb1_1 + (kb + TILE_K) * (int64_t) sizeof(et_fp16_t)),
                    0, n_cur - 1, (uint64_t) nb1_1, 0);
            }

            if (kb + 2 * TILE_K < k_end) {
                l2_prefetch((const void *)(src1_batch + nb * nb1_1 + (kb + 2 * TILE_K) * sizeof(et_fp16_t)), n_cur, nb1_1);
            }
            ...
            // Load B from L2 SCP to L1 SCP (TL1 engine, B_L1_START)
            tensor_load(false, false, B_L1_START, TENSOR_LOAD_PLAIN, 0, (uint64_t) scp_panel[buf], 0, 15, 64, 1);
            tensor_wait(TENSOR_LOAD_WAIT_1);
