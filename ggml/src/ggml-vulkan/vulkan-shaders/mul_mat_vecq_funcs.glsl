...
i32vec4 repack4(uint ib, uint iqs) {
    ...
    // Subtract 4 by twiddling bits rather than using re-packing as we have
    // the high bit to ourselves and can avoid the hardware having to deal with
    // the the ints separately.
...
i32vec4 repack4(uint ib, uint iqs) {
    ...
    // Subtract 32 by twiddling bits rather than using re-packing as we have
    // the high bits to ourselves and can avoid the hardware having to deal
    // with the the ints separately.
