...
static void add_tensor(ggml_tensor * tensor, const ggml_cgraph * cgraph, const socket_ptr & sock, std::vector<rpc_tensor> & tensors, std::unordered_set<ggml_tensor*> & visited) {
    if (tensor == nullptr) {
        return;
    }
    ...
    for (int i = 0; i < GGML_MAX_SRC; i++) {
        add_tensor(tensor->src[i], cgraph, sock, tensors, visited);
    }
    add_tensor(tensor->view_src, cgraph, sock, tensors, visited);
    rpc_tensor result = serialize_tensor(tensor, sock);
...
static void serialize_graph(uint32_t device, const ggml_cgraph * cgraph, const socket_ptr & sock, std::vector<uint8_t> & output) {
    ...
    for (uint32_t i = 0; i < n_nodes; i++) {
        add_tensor(cgraph->nodes[i], cgraph, sock, tensors, visited);
    }
