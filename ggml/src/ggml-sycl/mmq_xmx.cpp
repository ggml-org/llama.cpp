...
void mul_mat_q4_0_q8_1_xmx_largetile_kernel(
...
// =============================================================================
void mul_mat_q8_0_q8_1_xmx_kernel(
    const void* __restrict__ vx,      // Q8_0 weights [nrows_x, ncols_x/32 blocks]
    const void* __restrict__ vy,      // Q8_1 activations [ncols_y, ncols_x/32 blocks]
    float* __restrict__ dst,          // Output [nrows_x, ncols_y]
    const int ncols_x,                // K dimension (must be multiple of 32)
    const int nrows_x,                // M dimension (rows of weights to process)
    ...
    for (int k_block = 0; k_block < num_k_blocks; k_block++) {
        ...
        if (lane_id < XMX_M) {
            int row = row_base + lane_id;
            if (row < nrows_x) {
                const char* block_ptr = src0 + (row * num_k_blocks + k_block) * Q8_0_BLOCK_SIZE;
...
// =============================================================================
void mul_mat_q4_1_q8_1_xmx_kernel(
    const void* __restrict__ vx,      // Q4_1 weights [nrows_x, ncols_x/32 blocks]
    const void* __restrict__ vy,      // Q8_1 activations [ncols_y, ncols_x/32 blocks]
    float* __restrict__ dst,          // Output [nrows_x, ncols_y]
    const int ncols_x,                // K dimension (must be multiple of 32)
    const int nrows_x,                // M dimension (rows of weights to process)
    ...
    for (int k_block = 0; k_block < num_k_blocks; k_block++) {
        ...
        if (lane_id < XMX_M) {
            int row = row_base + lane_id;
            if (row < nrows_x) {
                const char* block_ptr = src0 + (row * num_k_blocks + k_block) * Q4_1_BLOCK_SIZE;
...
// =============================================================================
static void ggml_mul_mat_q4_1_q8_1_xmx_sycl(
    const void* vx, const void* vy, float* dst,
    const int ncols_x, const int nrows_x,
    const int ncols_y, const int nrows_dst,
    dpct::queue_ptr stream) {
    ...
    GGML_SYCL_DEBUG("[XMX] Launching Q4_1 kernel: grid=(%d,%d), M=%d, N=%d, K=%d\n",
                    num_row_tiles, num_col_tiles, nrows_x, ncols_y, ncols_x);
    ...
    constexpr int TOTAL_SLM_SIZE = SLM_A_SIZE + SLM_B_SIZE + SLM_C_SIZE * 4 +
                                   ...
                                   SLM_SCALES_A_SIZE * 4 + SLM_SCALES_B_SIZE * 4;  // mins_A, sums_B
    ...
    stream->submit([&](sycl::handler& h) {
        ...
        h.parallel_for(
            ...
            [=](sycl::nd_item<2> item) [[sycl::reqd_sub_group_size(XMX_SG_SIZE)]] {
                ...
                int offset = 0;
                int8_t* A_int8 = reinterpret_cast<int8_t*>(shared + offset); offset += SLM_A_SIZE;
                int8_t* B_int8 = reinterpret_cast<int8_t*>(shared + offset); offset += SLM_B_SIZE;
                int32_t* C_int32 = reinterpret_cast<int32_t*>(shared + offset); offset += SLM_C_SIZE * 4;
                float* A_scales = reinterpret_cast<float*>(shared + offset); offset += SLM_SCALES_A_SIZE * 4;
                float* B_scales = reinterpret_cast<float*>(shared + offset); offset += SLM_SCALES_B_SIZE * 4;
                float* A_mins = reinterpret_cast<float*>(shared + offset); offset += SLM_SCALES_A_SIZE * 4;
                float* B_sums = reinterpret_cast<float*>(shared + offset);

                mul_mat_q4_1_q8_1_xmx_kernel(
                    vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_dst,
                    A_int8, B_int8, C_int32, A_scales, B_scales, A_mins, B_sums, item);
            });
    });
}
...
// =============================================================================
void mul_mat_q5_0_q8_1_xmx_kernel(
    const void* __restrict__ vx,      // Q5_0 weights [nrows_x, ncols_x/32 blocks]
    const void* __restrict__ vy,      // Q8_1 activations [ncols_y, ncols_x/32 blocks]
    float* __restrict__ dst,          // Output [nrows_x, ncols_y]
    const int ncols_x,                // K dimension (must be multiple of 32)
    const int nrows_x,                // M dimension
    ...
    for (int k_block = 0; k_block < num_k_blocks; k_block++) {
        ...
        if (lane_id < XMX_M) {
            int row = row_base + lane_id;
            if (row < nrows_x) {
                const char* block_ptr = src0 + (row * num_k_blocks + k_block) * Q5_0_BLOCK_SIZE;
...
// =============================================================================
static void ggml_mul_mat_q5_0_q8_1_xmx_sycl(
    const void* vx, const void* vy, float* dst,
    const int ncols_x, const int nrows_x,
    const int ncols_y, const int nrows_dst,
    dpct::queue_ptr stream) {
    ...
    stream->submit([&](sycl::handler& h) {
        ...
        h.parallel_for(
            ...
            [=](sycl::nd_item<2> item) [[sycl::reqd_sub_group_size(XMX_SG_SIZE)]] {
                ...
                mul_mat_q5_0_q8_1_xmx_kernel(
                    vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_dst,
                    A_int8, B_int8, C_int32, A_scales, B_scales, item);
            });
    });
}
...
// =============================================================================
void mul_mat_q5_1_q8_1_xmx_kernel(
    const void* __restrict__ vx,      // Q5_1 weights [nrows_x, ncols_x/32 blocks]
    const void* __restrict__ vy,      // Q8_1 activations [ncols_y, ncols_x/32 blocks]
    float* __restrict__ dst,          // Output [nrows_x, ncols_y]
    const int ncols_x,                // K dimension (must be multiple of 32)
    const int nrows_x,                // M dimension
    ...
    for (int k_block = 0; k_block < num_k_blocks; k_block++) {
        ...
        if (lane_id < XMX_M) {
            int row = row_base + lane_id;
            if (row < nrows_x) {
                const char* block_ptr = src0 + (row * num_k_blocks + k_block) * Q5_1_BLOCK_SIZE;
...
// =============================================================================
static void ggml_mul_mat_q5_1_q8_1_xmx_sycl(
    const void* vx, const void* vy, float* dst,
    const int ncols_x, const int nrows_x,
    const int ncols_y, const int nrows_dst,
    dpct::queue_ptr stream) {
...
// =============================================================================
void mul_mat_q3_K_q8_1_xmx_kernel(
    const void* __restrict__ vx,      // Q3_K weights [nrows_x, ncols_x/256 super-blocks]
    const void* __restrict__ vy,      // Q8_1 activations [ncols_y, ncols_x/32 blocks]
    float* __restrict__ dst,          // Output [nrows_x, ncols_y]
    ...
    for (int k_block = 0; k_block < num_k_blocks; k_block++) {
        ...
        if (lane_id < XMX_M) {
            int row = row_base + lane_id;
            if (row < nrows_x) {
                const char* sb_ptr = src0 + (row * num_super_blocks + super_block_idx) * Q3_K_BLOCK_SIZE;
...
// =============================================================================
static void ggml_mul_mat_q3_K_q8_1_xmx_sycl(
    const void* vx, const void* vy, float* dst,
    const int ncols_x, const int nrows_x,
    const int ncols_y, const int nrows_dst,
    dpct::queue_ptr stream) {
...
// =============================================================================
void mul_mat_q5_K_q8_1_xmx_kernel(
    const void* __restrict__ vx,      // Q5_K weights [nrows_x, ncols_x/256 super-blocks]
    const void* __restrict__ vy,      // Q8_1 activations [ncols_y, ncols_x/32 blocks]
    float* __restrict__ dst,          // Output [nrows_x, ncols_y]
    const int ncols_x,                // K dimension (must be multiple of 256)
    const int nrows_x,                // M dimension
    ...
    for (int k_block = 0; k_block < num_k_blocks; k_block++) {
        ...
        if (lane_id < XMX_M) {
            int row = row_base + lane_id;
            if (row < nrows_x) {
                const char* sb_ptr = src0 + (row * num_super_blocks + super_block_idx) * Q5_K_BLOCK_SIZE;
...
// =============================================================================
static void ggml_mul_mat_q5_K_q8_1_xmx_sycl(
    const void* vx, const void* vy, float* dst,
    const int ncols_x, const int nrows_x,
    const int ncols_y, const int nrows_dst,
    dpct::queue_ptr stream) {
    ...
    constexpr int TOTAL_SLM_SIZE_Q5K = SLM_A_SIZE + SLM_B_SIZE + SLM_C_SIZE * 4 +
                                       SLM_SCALES_A_SIZE * 4 + SLM_SCALES_B_SIZE * 4 +
                                       SLM_SCALES_A_SIZE * 4 + SLM_SCALES_B_SIZE * 4;
...
// =============================================================================
void mul_mat_q4_K_q8_1_xmx_kernel(
    const void* __restrict__ vx,      // Q4_K weights [nrows_x, ncols_x/256 super-blocks]
    const void* __restrict__ vy,      // Q8_1 activations [ncols_y, ncols_x/32 blocks]
    float* __restrict__ dst,          // Output [nrows_x, ncols_y]
    const int ncols_x,                // K dimension (must be multiple of 256)
    const int nrows_x,                // M dimension
    ...
    for (int k_block = 0; k_block < num_k_blocks; k_block++) {
        ...
        if (lane_id < XMX_M) {
            int row = row_base + lane_id;
            if (row < nrows_x) {
                const char* sb_ptr = src0 + (row * num_super_blocks + super_block_idx) * Q4_K_BLOCK_SIZE;
...
// =============================================================================
static void ggml_mul_mat_q4_K_q8_1_xmx_sycl(
    const void* vx, const void* vy, float* dst,
    const int ncols_x, const int nrows_x,
    const int ncols_y, const int nrows_dst,
    dpct::queue_ptr stream) {
...
// =============================================================================
void mul_mat_q2_K_q8_1_xmx_kernel(
    const void* __restrict__ vx,
    const void* __restrict__ vy,
    float* __restrict__ dst,
    const int ncols_x,
    ...
    for (int k_block = 0; k_block < num_k_blocks; k_block++) {
        ...
        if (lane_id < XMX_M) {
            int row = row_base + lane_id;
            if (row < nrows_x) {
                const char* sb_ptr = src0 + (row * num_super_blocks + super_block_idx) * Q2_K_BLOCK_SIZE;
...
// =============================================================================
void mul_mat_q6_K_q8_1_xmx_kernel(
    const void* __restrict__ vx,
    const void* __restrict__ vy,
    float* __restrict__ dst,
    const int ncols_x,
    ...
    for (int k_block = 0; k_block < num_k_blocks; k_block++) {
        ...
        if (lane_id < XMX_M) {
            int row = row_base + lane_id;
            if (row < nrows_x) {
                const char* sb_ptr = src0 + (row * num_super_blocks + super_block_idx) * Q6_K_BLOCK_SIZE;
...
// =============================================================================
static void ggml_mul_mat_q8_0_q8_1_xmx_largetile_sycl(
    const void* vx, const void* vy, float* dst,
    const int ncols_x, const int nrows_x,
    const int ncols_y, const int nrows_dst,
    dpct::queue_ptr stream) {
...
// =============================================================================
static void ggml_mul_mat_q4_0_q8_1_xmx_largetile_sycl(
    const void* vx, const void* vy, float* dst,
    ...
    stream->submit([&](sycl::handler& h) {
        ...
        h.parallel_for(
            ...
            [=](sycl::nd_item<2> item) [[sycl::reqd_sub_group_size(XMX_SG_SIZE)]] {
                ...
                mul_mat_q4_0_q8_1_xmx_largetile_kernel(
                    vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_dst,
                    slm_A, slm_B, slm_scales_A, slm_scales_B, slm_sums_B, item);
            });
    });
}
...
static void ggml_mul_mat_q4_0_q8_1_xmx_sycl(
    ...
    if (ncols_y >= 64) {
        ggml_mul_mat_q4_0_q8_1_xmx_largetile_sycl(
...
void ggml_sycl_op_mul_mat_q_xmx(
    ...
    GGML_SYCL_DEBUG("[XMX] mul_mat: type=%d, M=%ld, N=%ld, K=%ld (padded=%ld), nrows_dst=%ld\n",
                    src0->type, row_diff, src1_ncols, ne00, src1_padded_row_size, nrows_dst);
    ...
    if (debug_count == 0 && getenv("GGML_SYCL_DEBUG_XMX_GEMM")) {
        fprintf(stderr, "[XMX DEBUG] K=%ld, K_padded=%ld, difference=%ld\n",
                (long)ne00, (long)src1_padded_row_size, (long)(src1_padded_row_size - ne00));
    }
    debug_count++;

    switch (src0->type) {
        case GGML_TYPE_Q4_0:
            ggml_mul_mat_q4_0_q8_1_xmx_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, src1_padded_row_size, row_diff, src1_ncols, nrows_dst, stream);
            break;
        case GGML_TYPE_Q4_1:
            ggml_mul_mat_q4_1_q8_1_xmx_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, row_diff, src1_ncols, nrows_dst, stream);
            break;
        case GGML_TYPE_Q8_0:
            // Use optimized large-tile kernel for Q8_0
            ggml_mul_mat_q8_0_q8_1_xmx_largetile_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, row_diff, src1_ncols, nrows_dst, stream);
            break;
        case GGML_TYPE_Q5_0:
            ggml_mul_mat_q5_0_q8_1_xmx_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, row_diff, src1_ncols, nrows_dst, stream);
            break;
        case GGML_TYPE_Q5_1:
            ggml_mul_mat_q5_1_q8_1_xmx_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, row_diff, src1_ncols, nrows_dst, stream);
            break;
        case GGML_TYPE_Q3_K:
            ggml_mul_mat_q3_K_q8_1_xmx_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, row_diff, src1_ncols, nrows_dst, stream);
            break;
        case GGML_TYPE_Q4_K:
            ggml_mul_mat_q4_K_q8_1_xmx_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, row_diff, src1_ncols, nrows_dst, stream);
            break;
        case GGML_TYPE_Q5_K:
            ggml_mul_mat_q5_K_q8_1_xmx_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, row_diff, src1_ncols, nrows_dst, stream);
            break;
        case GGML_TYPE_Q2_K:
            ggml_mul_mat_q2_K_q8_1_xmx_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, row_diff, src1_ncols, nrows_dst, stream);
            break;
        case GGML_TYPE_Q6_K:
            ggml_mul_mat_q6_K_q8_1_xmx_sycl(
                src0_dd_i, src1_ddq_i, dst_dd_i,
                ne00, row_diff, src1_ncols, nrows_dst, stream);
            break;
        default:
    ...
    q.submit([&](sycl::handler &h) {
        h.parallel_for(
            ...
            [=](sycl::nd_item<1> item) [[sycl::reqd_sub_group_size(16)]] {
                ...
                sycl_xmx::joint_matrix_load(sg, matA,
                    sycl::address_space_cast<sycl::access::address_space::global_space, sycl::access::decorated::no>(d_A), K);
                sycl_xmx::joint_matrix_load(sg, matB,
                    sycl::address_space_cast<sycl::access::address_space::global_space, sycl::access::decorated::no>(d_B), K);
                sycl_xmx::joint_matrix_fill(sg, matC, 0);

                sycl_xmx::joint_matrix_mad(sg, matC, matA, matB, matC);

                sycl_xmx::joint_matrix_store(sg, matC,
...
void ggml_sycl_op_mul_mat_q_xmx(
    ...
    (void)src0_dd_i; (void)src1_ddf_i; (void)src1_ddq_i;
    (void)dst_dd_i; (void)row_low; (void)row_high;
    (void)src1_ncols; (void)src1_padded_row_size; (void)stream;
}
