...
extern "C" void ggml_innerq_compute_k_squared_profile_sycl(
    ...
    if (out_scales == nullptr || head_dim <= 0 || head_dim > 128) {
        return;
    }
    ...
    q.submit([&](sycl::handler & h) {
        ...
        h.parallel_for(sycl::range<1>(D), [=](sycl::id<1> d) {
            // Accumulate in float so the device image contains no
            // `double`, which the A770 acm-g10 offline AOT path rejects.
            // This need not be bit-equivalent to the double-accumulating
            // C reference: the in-repo [8c] InnerQ K^2 profile harness
            // checks the SYCL result against the analytical constant-input
            // result and compares its per-position output with the C
            // reference, both at a 1e-5f tolerance. Its zero-input,
            // eight-probe constant-input, and two-probe per-position cases
            // cover the numerical behavior exercised here.
            float sumsq = 0.0f;
