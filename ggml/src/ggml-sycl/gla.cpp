...
template <unsigned int HEAD_SIZE>
static void gated_linear_attn_f32_kernel(const ggml_sycl::queue_ptr stream, unsigned int B, unsigned int T, unsigned int C, unsigned int H, float scale,
                                         const float * k, const float * v, const float * r, const float * td,
                                         const float * s, float * dst) {
    const unsigned int head_size    = HEAD_SIZE;
    const unsigned int state_size   = C * head_size;
    const unsigned int n_seq_tokens = T / B;
    ...
    ggml_sycl::ordered_submit(stream, [&](sycl::handler & cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<1>(grid_dims * block_dims, block_dims), [=](sycl::nd_item<1> item) {
            ...
            unsigned int batch_i = bid / H;
            unsigned int head_i  = bid % H;

            float state[head_size];

#pragma unroll
            for (unsigned int i = 0; i < head_size; i++) {
                state[i] = s[batch_i * state_size + head_i * head_size * head_size + i * head_size + tid];
            }

            for (unsigned int t = batch_i * n_seq_tokens * C + head_i * head_size + tid;
                ...
                for (unsigned int j = 0; j < head_size; j += 4) {
