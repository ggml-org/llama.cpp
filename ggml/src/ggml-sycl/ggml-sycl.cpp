...
bool test_backend_supports_graphs(ggml_backend_t backend) {
    ...
    return dpct::get_device(ctx->device).has(sycl::aspect::ext_oneapi_limited_graph);
}
...
static bool ggml_sycl_has_missing_layout_choice_for_tensor(const ggml_tensor * tensor, int device) {
    ...
    if (usage == tensor_usage::MOE_EXPERT_WEIGHT) {
        ...
        for (int64_t e = 0; e < n_experts; ++e) {
            ...
            layout_mode dummy = GGML_LAYOUT_AOS;
            if (!ggml_sycl_get_layout_choice(key, device, &dummy)) {
                return true;
            }
        }
        return false;
    ...
    layout_mode dummy = GGML_LAYOUT_AOS;
    return !ggml_sycl_get_layout_choice(key, device, &dummy);
}
...
static bool ggml_sycl_graph_has_missing_layout_choices(ggml_cgraph * cgraph, int device) {
    if (!cgraph) {
        return false;
    }
    for (int i = 0; i < cgraph->n_nodes; ++i) {
        ...
        for (int s = 0; s < GGML_MAX_SRC; ++s) {
            ggml_tensor * src = node->src[s];
            if (ggml_sycl_has_missing_layout_choice_for_tensor(src, device)) {
                return true;
            }
        }
    }
...
void ggml_backend_sycl_register_host_weight_tensor(ggml_backend_dev_t dev, ggml_tensor * tensor) {
    ...
    if (!insert.second) {
        if (insert.first->second != extra) {
            ...
            tensor->extra  = insert.first->second;
            tensor->layout = tensor->extra ? &static_cast<ggml_tensor_extra_gpu *>(tensor->extra)->layout : nullptr;
        }
        return;
    }
    if (!created_extra) {
...
bool ggml_sycl_get_layout_choice_for_tensor(const ggml_tensor * tensor, int device, layout_mode * out) {
    ...
    if (!ggml_sycl_get_layout_choice(key, device, out)) {
        ...
        if (target != GGML_LAYOUT_AOS && !ggml_sycl_reorder_enabled()) {
        ...
        ggml_sycl_register_layout_choice(key, device, target, tensor->name);
        *out = target;
        return true;
    }
...
static sycl::event ggml_sycl_fill_reordered_host(sycl::queue &                    queue,
    ...
    try {
        ...
        reorder_buf_raw         = sycl::malloc_host(alloc_size, queue);
    ...
    sycl::free(reorder_buf_raw ? reorder_buf_raw : reorder_buf, queue);
...
static bool ggml_sycl_fill_xmx_tiled_host(sycl::queue &                        queue,
    ...
    try {
        device_tmp = sycl::malloc_device<uint8_t>(dst_size, queue);
    } catch (...) {
        device_tmp = nullptr;
    }
    if (!device_tmp) {
...
static void ggml_sycl_drop_non_target_cache_entries(ggml_sycl::unified_cache * cache,
    ...
    const ggml_layout_mode layouts[] = {
        GGML_LAYOUT_AOS, GGML_LAYOUT_SOA, GGML_LAYOUT_COALESCED, GGML_LAYOUT_XMX_TILED, GGML_LAYOUT_XMX_GEMM_TILED,
...
static bool ggml_sycl_preload_moe_experts(const ggml_tensor * src0, int device, layout_mode layout) {
    ...
    sycl::queue &              stream = dpct::dev_mgr::instance().get_device(device).default_queue();
...
void * ggml_sycl_get_weight_layout_ptr(const ggml_tensor * tensor, int device, layout_mode target) {
    ...
    if (ggml_sycl_graph_recording_active()) {
        ...
        if (cached) {
            ...
            auto * extra = static_cast<ggml_tensor_extra_gpu *>(tensor->extra);
            ggml_sycl_update_layout_from_cache(extra, tensor, device, resolved, cached, dst_size, xmx_info);
            return cached;
        }
        return nullptr;
    }
    ...
    if (host_cache && resolved != GGML_LAYOUT_AOS && !src_is_device) {
...
static void ggml_backend_sycl_buffer_set_tensor(ggml_backend_buffer_t buffer,
    ...
    const tensor_usage usage = is_weight_buffer ? ggml_sycl_get_tensor_usage(tensor) : tensor_usage::UNKNOWN;
    ...
    layout_mode adjusted_layout   = ggml_sycl_adjust_layout_for_tensor(tensor, target_layout, ctx->device);
    ...
    const bool do_cpu_reorder = do_reorder;
    size_t     reorder_size   = size;
    if (do_cpu_reorder && use_coalesced) {
        const size_t layout_bytes = ggml_sycl_layout_bytes_for_tensor(tensor, GGML_LAYOUT_COALESCED, ctx->device);
        if (layout_bytes > 0) {
            reorder_size = layout_bytes;
        }
    }
    if (do_cpu_reorder) {
        if (ggml_sycl_reorder_guard_enabled()) {
        ...
        const size_t alloc_size = reorder_size + (reorder_guard * 2);
        reorder_buf_raw         = (char *) malloc(alloc_size);
        if (reorder_buf_raw && reorder_guard > 0) {
            std::memset(reorder_buf_raw, k_reorder_guard_pattern, reorder_guard);
            std::memset(reorder_buf_raw + reorder_guard + reorder_size, k_reorder_guard_pattern, reorder_guard);
        }
        reorder_buf         = reorder_buf_raw ? reorder_buf_raw + reorder_guard : nullptr;
        const int64_t ncols = tensor->ne[0];
        ...
        if (!ggml_sycl_check_reorder_guard_dual(reorder_buf_raw, reorder_size, reorder_guard, "cpu_reorder",
                                                tensor->name)) {
            GGML_ABORT("reorder guard corrupted");
        }
    ...
    if (has_preconverted_tiled) {
        ...
        if (reorder_buf_raw || reorder_buf) {
        ...
        if (copy_size > MAX_CHUNK_SIZE) {
            ...
            GGML_SYCL_DEBUG("[CHUNKED] Copied %zu bytes in %zu MB chunks for %s\n", copy_size,
                            MAX_CHUNK_SIZE / (1024 * 1024), tensor->name);
        } else {
...
#else
    if (reorder_buf) {
        SYCL_CHECK(CHECK_TRY_ERROR((*stream).memcpy((char *) tensor->data + offset, reorder_buf, reorder_size).wait()));
        free(reorder_buf);
    } else {
    ...
    if (use_unified_cache && adjusted_layout != GGML_LAYOUT_AOS && !has_preconverted_tiled) {
        ...
        if (ggml_sycl_get_layout_choice_for_tensor(tensor, ctx->device, &chosen_layout)) {
            ...
            ggml_sycl_cache_id cache_key = ggml_backend_sycl_get_weight_cache_key(tensor, ctx->device);
            if (cache_key.valid) {
                ggml_sycl_register_layout_choice(cache_key, ctx->device, adjusted_layout, tensor->name);
            }
        }
...
enum ggml_sycl_mem_policy {
    GGML_SYCL_MEM_POLICY_STATIC  = 0,  // Use mem_type as-is
    GGML_SYCL_MEM_POLICY_KV_AUTO = 1,  // Pick device vs host based on unified cache budget
};
// sycl buffer type
...
static ggml_backend_buffer_t ggml_backend_sycl_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft,
    ...
    // Device allocations consume unified cache budget so weights/kv stay within VRAM headroom.
    // EXCEPTION: KV_AUTO allocations already consulted cache->available() to decide device vs host.
    // Counting them would double-book, leaving no room for MoE expert streaming.
    const bool is_kv_auto = (buft_ctx->mem_policy == GGML_SYCL_MEM_POLICY_KV_AUTO);
    const bool counts_runtime_bytes = (effective_mem_type == GGML_SYCL_MEM_DEVICE) && !is_kv_auto;
    if (counts_runtime_bytes) {
    ...
    if (!ggml_backend_sycl_buffer_type_initialized) {
        for (int i = 0; i < ggml_sycl_info().device_count; i++) {
            ggml_backend_sycl_buffer_types[i] = {
                ...
                /* .context  = */
                new ggml_backend_sycl_buffer_type_context{ i, GGML_SYCL_NAME + std::to_string(i), GGML_SYCL_MEM_DEVICE,
                                                          GGML_SYCL_MEM_POLICY_STATIC, false, true, 0, ctx->stream(i, 0) },
            };
        }
        ggml_backend_sycl_buffer_type_initialized = true;
    }
        ...
        for (int i = 0; i < dev_count; i++) {
            ...
            ggml_backend_sycl_kv_buffer_types[i] = {
                ...
                new ggml_backend_sycl_buffer_type_context{ i, GGML_SYCL_NAME "_KV" + std::to_string(i),
                                                          GGML_SYCL_MEM_DEVICE, GGML_SYCL_MEM_POLICY_KV_AUTO,
                                                          true, true, safe_max, stream },
            };
        }
        initialized = true;
    }
...
void ggml_backend_sycl_submit_barrier(ggml_backend_t backend) {
    ...
    if (!sycl_ctx->last_graph_event.has_value()) {
        ...
        GGML_SYCL_DEBUG("[SYCL-BARRIER] Skip submit (no graph event)\n");
        return;
        ...
        for (int i = 0; i < dev_count; i++) {
            ...
            ggml_backend_sycl_host_compute_buffer_types[i] = {
                ...
                new ggml_backend_sycl_buffer_type_context{ i, GGML_SYCL_NAME "_Compute" + std::to_string(i),
                                                          GGML_SYCL_MEM_DEVICE, GGML_SYCL_MEM_POLICY_STATIC,
                                                          false, true, 0, stream },
            };
        }
        initialized = true;
...
bool ggml_sycl_cpu_fallback_graph(ggml_backend_sycl_context & ctx, ggml_tensor * dst, const char * reason) {
    ...
    if (!src0 || !src1) {
        GGML_LOG_WARN("[SYCL] CPU fallback missing inputs (%s)\n", reason ? reason : "unknown");
        return false;
    }
    auto ptr_is_device = [&](void * ptr) -> bool {
        if (!ptr) {
            return false;
        }
        try {
    ...
    if (ptr_is_device(src0_orig)) {
        src0_host.resize(src0_bytes);

        stream->memcpy(src0_host.data(), src0_orig, src0_bytes).wait();
        src0->data = src0_host.data();
    }
    if (ptr_is_device(src1_orig)) {
        src1_host.resize(src1_bytes);
        stream->memcpy(src1_host.data(), src1_orig, src1_bytes).wait();

        src1->data = src1_host.data();
    }
    if (ptr_is_device(dst_orig)) {

        dst_host.resize(dst_bytes);
        dst->data = dst_host.data();
    }

    const size_t graph_nodes = 256;
    const size_t graph_bytes = ggml_graph_overhead_custom(graph_nodes, false);
    void *       graph_mem   = std::malloc(graph_bytes);
    if (!graph_mem) {

        GGML_LOG_WARN("[SYCL] CPU fallback graph alloc failed (%s)\n", reason ? reason : "unknown");
        src0->data = src0_orig;
    ...
    if (plan.work_size > 0) {
        ...
        plan.work_data = static_cast<uint8_t *>(std::malloc(plan.work_size));
        if (!plan.work_data) {
...
struct ggml_sycl_mul_mat_stream_ctx {
    ggml_backend_sycl_context * ctx                  = nullptr;
    const ggml_tensor *         src0                 = nullptr;
    const ggml_tensor *         src1                 = nullptr;

    ggml_tensor *               dst                  = nullptr;
    ggml_sycl_op_mul_mat_t      op                   = nullptr;
...
static bool mul_mat_parse_env_mb_value(const char * name, size_t & out_mb) {
    ...
    if (end == env || mb < 0) {
        GGML_LOG_WARN("[MUL_MAT] Invalid %s='%s'\n", name, env);
        return false;
...
static bool mul_mat_parse_env_count_value(const char * name, size_t & out_count) {
    const char * env = std::getenv(name);
    if (!env || env[0] == '\0') {
        return false;
    }
...
static void mul_mat_resolve_dma_params(size_t row_bytes, size_t & slice_bytes, size_t & buffer_count) {
    size_t slice_mb = 1024;
    size_t buffers  = 2;
    size_t env_val  = 0;
    if (mul_mat_parse_env_mb_value("GGML_SYCL_DMA_SLICE_MB", env_val)) {
        slice_mb = env_val;

    }
    if (mul_mat_parse_env_count_value("GGML_SYCL_DMA_BUFFERS", env_val) ||

        mul_mat_parse_env_count_value("GGML_SYCL_DMA_SLICES", env_val)) {
        buffers = env_val;
...
static sycl::event ggml_sycl_mul_mat_stream_slice(sycl::queue &                    queue,
                                                  void *                           device_slice,
                                                  size_t                           slice_bytes,
                                                  ...
                                                  const std::vector<sycl::event> & deps) {
    const auto * ctx = static_cast<const ggml_sycl_mul_mat_stream_ctx *>(ctx_void);
    ...
    const int64_t unit_start = static_cast<int64_t>(offset_bytes / ctx->row_total_bytes);
    const int64_t unit_count = static_cast<int64_t>(slice_bytes / ctx->row_total_bytes);
    const int64_t row_unit   = ctx->row_unit > 0 ? ctx->row_unit : 1;
    int64_t       row_start  = unit_start * row_unit;
    int64_t       row_count  = unit_count * row_unit;
    if (ctx->row_limit > 0 && row_start + row_count > ctx->row_limit) {
        row_count = ctx->row_limit - row_start;
    }
    float * dst_ptr = ctx->dst_dd_i + row_start;
    if (row_count <= 0 || ctx->src1_ncols <= 0) {
        return queue.ext_oneapi_submit_barrier();

    }
    // Ensure DMA copy finishes before launching compute on out-of-order queues.

    // Use a barrier event to preserve dependency chains during graph recording.
    if (!deps.empty()) {
    ...
    static bool stream_debug_checked = false;
    static bool do_stream_debug      = false;
    if (!stream_debug_checked) {
        do_stream_debug      = (std::getenv("GGML_SYCL_MUL_MAT_STREAM_DEBUG") != nullptr);
        stream_debug_checked = true;
    }
    if (do_stream_debug) {
        GGML_LOG_DEBUG(
            "[MUL_MAT_STREAM] row_start=%lld row_count=%lld row_bytes=%zu slice_bytes=%zu offset_bytes=%zu "

            "src1_ncols=%lld src1_padded=%lld\n",
            (long long) row_start, (long long) row_count, ctx->row_total_bytes, slice_bytes, offset_bytes,
            (long long) ctx->src1_ncols, (long long) ctx->src1_padded_row_size);
    }
    ctx->op(*ctx->ctx, ctx->src0, ctx->src1, ctx->dst, static_cast<const char *>(device_slice), ctx->src1_ddf_i,
            ctx->src1_ddq_i, dst_ptr, 0, row_count, ctx->src1_ncols, ctx->src1_padded_row_size, &queue);
    return queue.ext_oneapi_submit_barrier();
}
...
static bool ggml_sycl_op_mul_mat(ggml_backend_sycl_context & ctx,
    ...
    auto infer_location = [](const void * ptr, const sycl::queue & queue) -> ggml_sycl::cache_location {
        if (!ptr) {
...
#ifdef GGML_SYCL_MMQ_XMX
    // XMX kernels assume contiguous Q8_1 blocks per row (no MATRIX_ROW_PADDING pitch).

    if (op == ggml_sycl_op_mul_mat_q_xmx) {
        src1_padded_row_size = ne10;
    }
#endif
    ...
    for (int64_t src1_col_0 = 0; src1_col_0 < ne11; src1_col_0 += src1_col_stride) {
        ...
        for (int i = 0; i < ggml_sycl_info().device_count; ++i) {
            ...
            for (int64_t i0 = 0; i0 < ne13 * ne12; ++i0) {
                ...
                // do the computation (stream if weights are host-resident)
                bool did_stream   = false;

                bool allow_stream = (op == ggml_sycl_op_mul_mat_sycl);
#ifdef GGML_SYCL_MMQ_XMX
                allow_stream = allow_stream || (op == ggml_sycl_op_mul_mat_q_xmx);
#endif
                bool streamable_layout = (src0_layout == GGML_LAYOUT_AOS);
#ifdef GGML_SYCL_MMQ_XMX
                streamable_layout = streamable_layout || (src0_layout == GGML_LAYOUT_XMX_GEMM_TILED);
#endif
                if (allow_stream && !split && src0_is_contiguous && ggml_sycl_tensor_is_weight(src0) &&
                    ggml_sycl::unified_cache_enabled() && streamable_layout && src0_row_bytes > 0) {
                    ggml_sycl::unified_cache * cache = ggml_sycl::get_unified_cache(*stream);
...
#ifdef GGML_SYCL_MMQ_XMX
                    if (src0_layout == GGML_LAYOUT_XMX_GEMM_TILED) {
                        size_t  tile_bytes = 0;
                        int64_t tile_m     = 0;
                        if (!ggml_sycl_xmx_gemm_tiled_tile_bytes(src0, i, tile_bytes, tile_m) || tile_m <= 0) {
                            GGML_ABORT("[MUL_MAT] XMX tiled streaming requires valid tile size");

                        }
                        const int64_t tiles = (row_diff + tile_m - 1) / tile_m;
                        stream_unit_bytes   = tile_bytes;
                        stream_unit_rows    = tile_m;
                        total_stream_bytes  = tile_bytes * static_cast<size_t>(tiles);

                    }
#endif
                    ggml_sycl::cache_ptr_view view{};
                    if (cache && cache_key.valid) {
                        view = cache->get_view(cache_key, src0_layout);
                    }
                    if (!view.ptr) {
                        view.ptr      = const_cast<char *>(src0_dd_i);
                        view.size     = total_stream_bytes;
                        view.layout   = src0_layout;
                        view.type     = ggml_sycl::cache_entry_type::DENSE_WEIGHT;
                        view.location = infer_location(view.ptr, *stream);

                    }
                    if (view.ptr && view.location != ggml_sycl::cache_location::DEVICE) {
                        if (!cache) {
                            GGML_ABORT("MUL_MAT streaming requires unified cache");
                        }
                        ggml_sycl_mul_mat_stream_ctx stream_ctx{};
                        stream_ctx.ctx                  = &ctx;
                        stream_ctx.src0                 = src0;
                        stream_ctx.src1                 = src1;
                        stream_ctx.dst                  = dst;
                        stream_ctx.op                   = op;
                        ...
                        stream_ctx.row_total_bytes      = stream_unit_bytes;
                        stream_ctx.row_unit             = stream_unit_rows;
                        stream_ctx.row_limit            = row_diff;
                        size_t slice_bytes  = 0;

                        size_t buffer_count = 0;
                        mul_mat_resolve_dma_params(stream_unit_bytes, slice_bytes, buffer_count);
                        if (cache_key.valid) {
                            cache->pin(cache_key, src0_layout);

                        }
                        auto result = cache->stream_dma(view, total_stream_bytes, slice_bytes, buffer_count,
                                                        ggml_sycl_mul_mat_stream_slice, &stream_ctx, {});
                        if (cache_key.valid) {
                            if (result.ok) {
                                cache->unpin_on_event(cache_key, src0_layout, result.event);
                            } else {
                                cache->unpin(cache_key, src0_layout);
                            }
                        }
                        if (!result.ok) {
                            if (result.mmap_direct_failed) {
                                GGML_LOG_WARN("[MUL_MAT] DMA from mmap failed, falling back to CPU (%s)\n",
                                              src0->name ? src0->name : "unknown");
                                if (ggml_sycl_cpu_fallback_graph(ctx, dst, "mul_mat streaming")) {
                                    return true;
                                }
                            }
                            GGML_ABORT("MUL_MAT streaming failed");
                        }
                        did_stream = true;
                    }
                }
    ...
    if (exc_ctx.active) {
        ...
        if (exc_ctx.src0) {
            const int src0_usage =
                exc_ctx.src0->buffer ? (int) ggml_backend_buffer_get_usage(exc_ctx.src0->buffer) : -1;
            const char * src0_buft = exc_ctx.src0->buffer ?
                                         ggml_backend_buft_name(ggml_backend_buffer_get_type(exc_ctx.src0->buffer)) :
                                         "null";
            fprintf(stderr, "[SYCL-EXC] src0 type=%s ne=[%lld,%lld,%lld,%lld] contig=%d weight=%d usage=%d buft=%s\n",
                    ggml_type_name(exc_ctx.src0->type), (long long) exc_ctx.src0->ne[0],

                    (long long) exc_ctx.src0->ne[1], (long long) exc_ctx.src0->ne[2], (long long) exc_ctx.src0->ne[3],
                    exc_ctx.src0_contig ? 1 : 0, exc_ctx.src0_weight ? 1 : 0, src0_usage, src0_buft);
        }
        if (exc_ctx.src1) {

            const int src1_usage =
                exc_ctx.src1->buffer ? (int) ggml_backend_buffer_get_usage(exc_ctx.src1->buffer) : -1;
            const char * src1_buft = exc_ctx.src1->buffer ?

                                         ggml_backend_buft_name(ggml_backend_buffer_get_type(exc_ctx.src1->buffer)) :
                                         "null";
            fprintf(stderr, "[SYCL-EXC] src1 type=%s ne=[%lld,%lld,%lld,%lld] contig=%d weight=%d usage=%d buft=%s\n",
                    ggml_type_name(exc_ctx.src1->type), (long long) exc_ctx.src1->ne[0],
                    (long long) exc_ctx.src1->ne[1], (long long) exc_ctx.src1->ne[2], (long long) exc_ctx.src1->ne[3],
        ...
        if (exc_ctx.dst) {
            ...
            const char * dst_buft = exc_ctx.dst->buffer ?
                                        ...
                                        ggml_backend_buft_name(ggml_backend_buffer_get_type(exc_ctx.dst->buffer)) :
                                        "null";

            fprintf(stderr, "[SYCL-EXC] dst type=%s ne=[%lld,%lld,%lld,%lld] usage=%d buft=%s\n",
                    ggml_type_name(exc_ctx.dst->type), (long long) exc_ctx.dst->ne[0], (long long) exc_ctx.dst->ne[1],
                    (long long) exc_ctx.dst->ne[2], (long long) exc_ctx.dst->ne[3], dst_usage, dst_buft);
        }

        if (exc_ctx.src0_extra) {
            const layout_mode effective = get_effective_layout_mode(exc_ctx.src0_extra);
            fprintf(stderr, "[SYCL-EXC] src0 layout_info mode=%d effective=%d device_id=%d data_ptr=%p\n",
                    (int) exc_ctx.src0_extra->layout.mode, (int) effective, exc_ctx.src0_extra->layout.device_id,

                    exc_ctx.src0_extra->layout.data_ptr);
        }
        for (int i = 0; i < ggml_sycl_info().device_count; ++i) {
            ...
            fprintf(stderr,
                    ...
                    i, (long long) exc_ctx.dev[i].row_low, (long long) exc_ctx.dev[i].row_high, exc_ctx.dev[i].src0_dd,
                    exc_ctx.dev[i].src0_ptr_origin, exc_ctx.dev[i].src0_layout_ptr_source, exc_ctx.dev[i].src1_ddf,
                    exc_ctx.dev[i].src1_ptr_origin, exc_ctx.dev[i].src1_ddq, exc_ctx.dev[i].dst_dd,

                    exc_ctx.dev[i].dst_ptr_origin);
        }
        if (exc_ctx.src0_extra) {
            int64_t is_max = 1;
...
static void ggml_sycl_mul_mat_batched_sycl(ggml_backend_sycl_context & ctx,
    ...
    GGML_ASSERT(!ggml_sycl_tensor_is_weight(src1));
    GGML_ASSERT(!ggml_is_transposed(src0));
    GGML_ASSERT(!ggml_is_transposed(src1));
    GGML_ASSERT(!ggml_backend_buffer_is_sycl_split(src0->buffer));
    GGML_ASSERT(src0->type == GGML_TYPE_F16);
...
static void finalize_layouts(ggml_backend_sycl_context & ctx, ggml_cgraph * cgraph) {
    ...
    if (cache) {
        ...
        if (g_ggml_sycl_debug && budget_fallback) {
            ...
            GGML_SYCL_DEBUG("[LAYOUT] Budget fallback: xmx_tiled=%d coalesced=%d\n", fallback_xmx, fallback_coalesced);
        }
    }
    ...
    for (const auto & entry : chosen_layouts) {
        ...
        ggml_sycl_cache_id  key    = ggml_backend_sycl_get_weight_cache_key(tensor, ctx.device);
    ...
    for (const auto & entry : moe_layouts) {
        ...
        const int64_t       n_experts = entry.second.n_experts;
        ggml_sycl_cache_id  base_key  = ggml_backend_sycl_get_weight_cache_key(tensor, ctx.device);
        auto *              extra     = static_cast<ggml_tensor_extra_gpu *>(tensor->extra);
        if (!extra) {
        ...
        if (base_key.valid) {
            ggml_sycl_register_layout_choice(base_key, ctx.device, target, ggml_get_name(tensor));
        }
    ...
    for (const auto & entry : chosen_layouts) {
        ...
        if (g_ggml_sycl_debug >= 2) {
            ggml_sycl_cache_id cache_key = ggml_backend_sycl_get_weight_cache_key(tensor, ctx.device);
            fprintf(stderr, "[LAYOUT-TRACE] lookup tensor=%p name=%s model=%llu name_hash=0x%llx target=%s cached=%p\n",
        ...
        if (host_cache && cache_key.valid && tensor->buffer && ggml_backend_buffer_is_host(tensor->buffer)) {
            ggml_sycl_drop_non_target_host_entries(host_cache, cache_key, ggml_sycl::cache_entry_type::DENSE_WEIGHT, -1,
...
bool ggml_sycl_update_moe_ptr_table(ggml_backend_sycl_context &  ctx,
    ...
    std::fill(host_ptrs.begin(), host_ptrs.end(), nullptr);
...
static bool graph_preload_moe_experts(ggml_backend_sycl_context & ctx, ggml_cgraph * cgraph) {
    ...
    std::vector<std::pair<ggml_sycl_cache_id, ggml_layout_mode>> experts_to_pin;
    std::vector<sycl::event>                                     table_events;
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (!ggml_sycl_copy_ids_to_host(ctx, ids, ids_host)) {
            ...
            GGML_LOG_ERROR("[GRAPH-PRELOAD] Failed to read MoE ids for %s\n", src0->name);
            return false;
        }

        const uint64_t ids_hash = ggml_sycl_hash_ids(ids_host);
        auto &     ids_entry   = ctx.moe_ids_cache[ids];

        const bool ids_changed = ids_entry.host_ids.empty() || ids_entry.hash != ids_hash;
        if (ids_changed) {
            ids_entry.hash     = ids_hash;
            ids_entry.host_ids = std::move(ids_host);
        }
        const bool ids_on_host = ids->buffer && ggml_backend_buffer_is_host(ids->buffer);
        ...
        if (ids_on_host) {
            ...
            if (!ids_entry.device_ids || ids_entry.device_bytes < ids_bytes) {
                ...
                ids_entry.device_ids   = sycl::malloc_device(ids_bytes, *ctx.stream());
    ...
    if (!table_events.empty()) {
        ...
        if (!ctx.stream()->has_property<sycl::property::queue::in_order>()) {
...
static layout_mode ggml_sycl_mul_mat_kernel_layout(ggml_sycl_mul_mat_kernel kernel) {
    ...
    return caps ? caps->layout : GGML_LAYOUT_AOS;
}
...
static bool ggml_sycl_can_use_layout_for_kernel(const ggml_tensor * tensor, layout_mode layout, int device) {
    ...
    if (layout != GGML_LAYOUT_AOS && !ggml_sycl_reorder_enabled()) {
...
static void ggml_sycl_mul_mat(ggml_backend_sycl_context & ctx,
    ...
    if (use_xmx_gemm) {
        // XMX requires K dimension to be divisible by the hardware tile K
    ...
    if (!split && !batched_has_weight && src0->type == GGML_TYPE_F16 && ggml_is_permuted(src0) &&
        ...
        const bool  enforce_layout_choice = layout_finalized && !has_override && ggml_sycl_tensor_is_weight(src0);
...
static bool ggml_sycl_mul_mat_id_fused(ggml_backend_sycl_context & ctx,
    ...
    if (!ids_d) {
        GGML_SYCL_DEBUG("[MoE FUSED] Missing ids device pointer for %s\n", src0->name);
        return false;
    }
    ...
    if (src0->type == GGML_TYPE_Q8_0) {
        ...
        if (use_soa) {
            ...
            launch_fused_moe_mxfp4_soa(src0_weight_ptr, (const float *) src1->data, ids_d, (float *) dst->data,
                                       total_qs_size, ncols,
            ...
            if (use_persistent) {
                launch_persistent_moe_mxfp4(src0_weight_ptr, (const float *) src1->data, ids_d, (float *) dst->data,
                ...
                launch_fused_moe_mxfp4(src0_weight_ptr, (const float *) src1->data, ids_d, (float *) dst->data,
                                       stride_expert, ncols, nrows, n_ids, num_tokens, ne11, ids_nb0, ids_nb1, nb11,
                                       nb12, nb1, nb2, *stream);
...
#if SYCL_XMX_MOE_AVAILABLE
struct ggml_sycl_xmx_moe_expert_dispatch_ctx {
    ...
    int64_t               nblocks_per_expert  = 0;
    int64_t               q8_qs_per_expert    = 0;
    int64_t               mxfp4_qs_per_expert = 0;
    const int8_t *        q_tokens            = nullptr;
    const sycl::half *    token_scales        = nullptr;
    sycl::half *          sorted_output       = nullptr;
    sycl::half *          expert_scale_buf    = nullptr;

    moe_xmx::MoEXMXConfig xmx_cfg{};
};
static void ggml_sycl_xmx_moe_dispatch_expert_ptr_table(const ggml_sycl_xmx_moe_expert_dispatch_ctx & ctx,
                                                        const void *                                  expert_weights,
                                                        sycl::queue &                                 queue) {
    if (!expert_weights || ctx.batch <= 0) {

        return;
    }
    if (ctx.type == GGML_TYPE_Q8_0) {
        if (ctx.is_coalesced) {

            const int8_t *     coalesced_qs = static_cast<const int8_t *>(expert_weights);
            const sycl::half * coalesced_d =
                reinterpret_cast<const sycl::half *>(static_cast<const char *>(expert_weights) + ctx.q8_qs_per_expert);
            moe_xmx::launch_xmx_moe_gemm_q8_0_coalesced<4, 4>(coalesced_qs, coalesced_d, ctx.q_tokens, ctx.token_scales,
                                                              ctx.sorted_output + ctx.output_offset, ctx.batch,
                                                              ctx.out_dim, ctx.in_dim, ctx.xmx_cfg, queue);
        } else if (ctx.is_soa) {
            const int8_t *     base_qs = static_cast<const int8_t *>(expert_weights);
            const sycl::half * base_d =
                reinterpret_cast<const sycl::half *>(static_cast<const char *>(expert_weights) + ctx.q8_qs_per_expert);
            moe_xmx::launch_xmx_moe_gemm_q8_0_soa<4, 4>(base_qs, base_d, ctx.q_tokens, ctx.token_scales,

                                                        ctx.sorted_output + ctx.output_offset, ctx.batch, ctx.out_dim,
                                                        ctx.in_dim, ctx.xmx_cfg, queue);
        } else {

            GGML_ASSERT(ctx.expert_scale_buf != nullptr);
            moe_xmx::extract_q8_0_scales(expert_weights, ctx.expert_scale_buf, ctx.out_dim, ctx.in_dim, queue);

            moe_xmx::launch_xmx_moe_gemm_q8_0<4, 4>(expert_weights, ctx.expert_scale_buf, ctx.q_tokens,
                                                    ctx.token_scales, ctx.sorted_output + ctx.output_offset, ctx.batch,
                                                    ctx.out_dim, ctx.in_dim, ctx.xmx_cfg, queue);
        }
        return;
    }
    if (ctx.type == GGML_TYPE_MXFP4) {

        if (ctx.is_coalesced) {
            const uint8_t * coalesced_qs = static_cast<const uint8_t *>(expert_weights);
            const uint8_t * coalesced_e  = coalesced_qs + ctx.mxfp4_qs_per_expert;
            moe_xmx::launch_xmx_moe_gemm_mxfp4_coalesced<4, 4>(coalesced_qs, coalesced_e, ctx.q_tokens,
                                                               ctx.token_scales, ctx.sorted_output + ctx.output_offset,

                                                               ctx.batch, ctx.out_dim, ctx.in_dim, ctx.xmx_cfg, queue);
        } else if (ctx.is_soa) {
            const uint8_t * base_qs = static_cast<const uint8_t *>(expert_weights);
            const uint8_t * base_e  = base_qs + ctx.mxfp4_qs_per_expert;

            moe_xmx::launch_xmx_moe_gemm_mxfp4_soa<4, 4>(base_qs, base_e, ctx.q_tokens, ctx.token_scales,
                                                         ctx.sorted_output + ctx.output_offset, ctx.batch, ctx.out_dim,
                                                         ctx.in_dim, ctx.xmx_cfg, queue);
        } else {
            moe_xmx::launch_xmx_moe_gemm_mxfp4<4, 4>(expert_weights, ctx.q_tokens, ctx.token_scales,
                                                     ctx.sorted_output + ctx.output_offset, ctx.batch, ctx.out_dim,
                                                     ctx.in_dim, ctx.xmx_cfg, queue);
        }
    }
}
static sycl::event ggml_sycl_xmx_moe_stream_slice(sycl::queue &                    queue,
                                                  ...
                                                  const void *                     ctx_void,
                                                  const std::vector<sycl::event> & deps) {
    GGML_UNUSED(slice_bytes);
    GGML_ASSERT(offset_bytes == 0);
    const auto * ctx = static_cast<const ggml_sycl_xmx_moe_expert_dispatch_ctx *>(ctx_void);
    if (!ctx || !device_slice) {
        return queue.ext_oneapi_submit_barrier();
    }
    if (!deps.empty()) {
    ...
    ggml_sycl_xmx_moe_dispatch_expert_ptr_table(*ctx, device_slice, queue);
    return queue.ext_oneapi_submit_barrier();
}
#endif
static bool try_xmx_sorted_moe(ggml_backend_sycl_context & ctx,
    ...
    const int32_t * ids_d   = ggml_sycl_get_moe_ids_device_ptr(ctx, ids, &ids_copy_event, &ids_nb0, &ids_nb1);
    if (!ids_d) {
        GGML_SYCL_DEBUG("[XMX MoE] Missing ids device pointer for %s\n", src0->name);
        return false;
    }
    if (g_ggml_sycl_graph_recording && ids->buffer && ggml_backend_buffer_is_host(ids->buffer)) {
        stream->ext_oneapi_submit_barrier({ ids_copy_event });
    ...
    if (use_ptr_table) {
        ...
        if (g_ggml_sycl_graph_recording) {
            if (!expert_ptrs_device || !expert_ptrs_host) {
                GGML_SYCL_DEBUG("[XMX MoE] Missing expert pointer table during graph recording for %s\n", src0->name);

                ctx.moe_graphs_disabled_once = true;
                return false;
            }
            table_event     = stream->memcpy(const_cast<void **>(expert_ptrs_device), expert_ptrs_host,
                                             src0_extra->moe_expert_ptrs_host[ctx.device].size() * sizeof(void *));
            has_table_event = true;
        }
        if (g_ggml_sycl_graph_recording && has_table_event) {

            stream->ext_oneapi_submit_barrier({ table_event });
        }
        if (!expert_ptrs_host && !g_ggml_sycl_graph_recording) {
            GGML_SYCL_DEBUG("[XMX MoE] Missing expert pointer table for %s\n", src0->name);
            return false;
        }
        if (g_ggml_sycl_graph_recording && !expert_ptrs_device) {
            GGML_SYCL_DEBUG("[XMX MoE] Missing device pointer table during graph recording for %s\n", src0->name);
            ctx.moe_graphs_disabled_once = true;
            return false;
        }
    }
    bool ptr_table_all_device = true;

    if (use_ptr_table && expert_ptrs_host) {
        const size_t ptr_count = src0_extra ? src0_extra->moe_expert_ptrs_host[ctx.device].size() : 0;
        for (size_t idx = 0; idx < ptr_count; ++idx) {
            const void * ptr = expert_ptrs_host[idx];
            if (!ptr) {
                ptr_table_all_device = false;
                break;
            }
            if (infer_location(ptr) != ggml_sycl::cache_location::DEVICE) {
                ptr_table_all_device = false;
    ...
    GGML_SYCL_DEBUG("[XMX MoE] ids tensor: ne=[%ld,%ld,%ld,%ld] nb=[%ld,%ld,%ld,%ld]\n", (long) ids->ne[0],
                    (long) ids->ne[1], (long) ids->ne[2], (long) ids->ne[3], (long) ids_nb0, (long) ids_nb1,
                    (long) ids->nb[2], (long) ids->nb[3]);
    const bool use_graph_buffers = g_ggml_sycl_graph_recording;
    const bool owns_buffers      = !use_graph_buffers;
    auto       free_if_owned     = [&](void * ptr) {
        if (owns_buffers && ptr) {
            sycl::free(ptr, *stream);

        }
    };
    auto submit_barrier = [&](const std::vector<sycl::event> & deps) -> sycl::event {
        if (deps.empty()) {

            return sycl::event{};
        }
        return stream->ext_oneapi_submit_barrier(deps);
    };
    if (use_graph_buffers) {
        const auto & graph_buffers = ctx.xmx_moe_buffers;
        if (!graph_buffers.initialized || total_pairs > graph_buffers.max_total_pairs ||
            in_dim > graph_buffers.max_in_dim || out_dim > graph_buffers.max_out_dim ||
            n_experts > graph_buffers.max_n_experts || n_input_rows > graph_buffers.max_n_input_rows) {
            GGML_SYCL_DEBUG("[XMX MoE] Graph buffers not initialized or too small\n");
            ctx.moe_graphs_disabled_once = true;
            return false;

        }
    }
    // Allocate temporary buffers (or reuse pre-allocated graph buffers)
    sycl::half *      tokens_sorted = use_graph_buffers ? ctx.xmx_moe_buffers.tokens_sorted :
                                                          sycl::malloc_device<sycl::half>(total_pairs * in_dim, *stream);
    MoETokenMapping * token_map = use_graph_buffers ? static_cast<MoETokenMapping *>(ctx.xmx_moe_buffers.token_map) :
                                                      sycl::malloc_device<MoETokenMapping>(total_pairs, *stream);
    ...
    const bool        needs_sorted_token_ids =
        (src0->type == GGML_TYPE_MXFP4) && (fused_enabled || require_tiled_mxfp4 || g_ggml_sycl_graph_recording);
    int32_t * sorted_token_ids = nullptr;
    if (needs_sorted_token_ids) {
        sorted_token_ids = use_graph_buffers ? ctx.xmx_moe_buffers.sorted_token_ids :
                                               sycl::malloc_device<int32_t>(total_pairs, *stream);
    }
    int32_t * expert_counts =
        use_graph_buffers ? ctx.xmx_moe_buffers.expert_counts : sycl::malloc_device<int32_t>(n_experts, *stream);
    // Allocate n_experts+1 for fused path (needs cumulative offsets including total)
    int32_t * expert_offsets =
        use_graph_buffers ? ctx.xmx_moe_buffers.expert_offsets : sycl::malloc_device<int32_t>(n_experts + 1, *stream);
    sycl::half * sorted_output = use_graph_buffers ? ctx.xmx_moe_buffers.sorted_output :
                                                     sycl::malloc_device<sycl::half>(total_pairs * out_dim, *stream);
    if (!tokens_sorted || !token_map || !expert_counts || !expert_offsets || !sorted_output) {
        ...
        free_if_owned(tokens_sorted);

        free_if_owned(token_map);
        free_if_owned(sorted_token_ids);
        free_if_owned(expert_counts);
        free_if_owned(expert_offsets);
        free_if_owned(sorted_output);
        if (use_graph_buffers) {
    ...
    const char * ids_base      = reinterpret_cast<const char *>(ids_d);
    const size_t ids_nb0_bytes = static_cast<size_t>(ids_nb0);
    const size_t ids_nb1_bytes = static_cast<size_t>(ids_nb1);

    // Use async versions during graph recording to avoid .wait() calls
    sycl::event count_event, offsets_event;

    if (g_ggml_sycl_graph_recording) {
        count_event   = moe_count_tokens_per_expert_async<64>(ids_base, ids_nb0_bytes, ids_nb1_bytes, expert_counts,
        ...
        moe_count_tokens_per_expert<64>(ids_base, ids_nb0_bytes, ids_nb1_bytes, expert_counts, n_tokens, n_ids,
                                        *stream);
    ...
    int32_t * expert_write_pos =
        use_graph_buffers ? ctx.xmx_moe_buffers.expert_write_pos : sycl::malloc_device<int32_t>(n_experts, *stream);
    if (!expert_write_pos) {
    ...
    if (has_offset_event) {
        copy_write_pos_event =
            stream->memcpy(expert_write_pos, expert_offsets, n_experts * sizeof(int32_t), offsets_event);
    } else {
    ...
    sycl::half * tokens_f16_input = use_graph_buffers ? ctx.xmx_moe_buffers.tokens_f16_input :
                                                        sycl::malloc_device<sycl::half>(n_input_rows * in_dim, *stream);
    if (!tokens_f16_input) {
        ...
        if (owns_buffers) {
            ...
            free_if_owned(expert_offsets);
            free_if_owned(sorted_output);
            free_if_owned(expert_write_pos);
        }
        if (use_graph_buffers) {
            ctx.moe_graphs_disabled_once = true;
        }
    ...
    if (g_ggml_sycl_graph_recording) {
        convert_event =
            moe_convert_f32_to_f16_async(tokens_f32, tokens_f16_input, n_tokens, in_dim, ne11, nb1, nb2, *stream);
    } else {
    ...
    sycl::event sort_event = moe_sort_tokens_by_expert_async(tokens_f16_input, tokens_sorted, ids_base, ids_nb0_bytes,
                                                             ids_nb1_bytes, expert_write_pos, token_map, n_tokens,
                                                             n_ids, ne11, in_dim, n_experts, *stream, sort_dep_event);
    ...
    // Host counts/offsets are only needed for the per-expert sorted path (non-graph).
    const bool has_graph_compatible_fused = is_soa && (src0->type == GGML_TYPE_Q8_0 || src0->type == GGML_TYPE_MXFP4);

    int64_t              actual_pairs = total_pairs;
    ...
    int8_t * q_tokens =
        ...
        use_graph_buffers ? ctx.xmx_moe_buffers.q_tokens : sycl::malloc_device<int8_t>(max_batch * in_dim, *stream);
    sycl::half * token_scales = use_graph_buffers ?

                                    ctx.xmx_moe_buffers.token_scales :
                                    sycl::malloc_device<sycl::half>(max_batch * (in_dim / QK8_0), *stream);

    if (!q_tokens || !token_scales) {
        GGML_SYCL_DEBUG("[XMX MoE] Failed to allocate q_tokens or token_scales\n");
        free_if_owned(q_tokens);
        free_if_owned(token_scales);
        free_if_owned(tokens_f16_input);
        free_if_owned(tokens_sorted);
        free_if_owned(token_map);
        free_if_owned(sorted_token_ids);
        free_if_owned(expert_counts);
    ...
    if (src0->type == GGML_TYPE_Q8_0 && !is_soa) {
        ...
        expert_scale_buf = use_graph_buffers ? ctx.xmx_moe_buffers.expert_scale_buf :
                                               sycl::malloc_device<sycl::half>(out_dim * num_k_blocks, *stream);
        if (!expert_scale_buf) {
    ...
    const size_t expert_size_bytes   = static_cast<size_t>(src0->nb[2]);

    size_t       expert_layout_bytes = expert_size_bytes;
    if (layout == GGML_LAYOUT_AOS) {
        expert_layout_bytes = ggml_sycl_get_padded_weight_bytes(src0->type, in_dim, expert_size_bytes);
    } else {

        const size_t layout_bytes = ggml_sycl_layout_bytes_for_dims(src0->type, in_dim, out_dim, layout, ctx.device);
        if (layout_bytes != 0) {
            expert_layout_bytes = layout_bytes;
        }
    }
#    if SYCL_XMX_MOE_AVAILABLE
    if (layout == GGML_LAYOUT_XMX_TILED) {
        const auto cfg      = moe_xmx_fused::MXFPXMXConfig::from_device(ctx.device);
        const auto info     = moe_xmx_fused::MXFPXMXLayoutInfo::compute(out_dim, in_dim, cfg);
        expert_layout_bytes = info.total_bytes;
    }
#    endif
    ...
    auto scatter_and_cleanup = [&]() -> bool {
        ...
        if (g_ggml_sycl_graph_recording) {
            ...
            moe_scatter_results_f16_to_f32(sorted_output, final_output, token_map, actual_pairs, out_dim, n_ids,
                                           out_nb1, out_nb2, *stream);
        }
    ...
    const bool can_use_tile_mapping = n_experts <= ggml_backend_sycl_context::xmx_moe_buffers_t::MAX_EXPERTS;
    if (src0->type == GGML_TYPE_Q8_0 && is_soa && can_use_tile_mapping && (!use_ptr_table || ptr_table_all_device)) {
        if (use_ptr_table && !expert_ptrs_device) {

            if (g_ggml_sycl_graph_recording) {
                GGML_SYCL_DEBUG("[XMX MoE] Missing device pointer table for fused Q8_0 path\n");
                ctx.moe_graphs_disabled_once = true;

                return fail_and_cleanup();
            }
            GGML_SYCL_DEBUG("[XMX MoE] Missing device pointer table for fused Q8_0 path, falling back\n");
        } else {
            ...
            sycl::event quant_event = moe_xmx::preprocess_tokens_q8_async(tokens_sorted, q_tokens, token_scales,
                                                                          total_pairs, in_dim, *stream, sort_event);
            ...
            // SoA weight pointers (contiguous layout)
            const int8_t *     base_qs = use_ptr_table ? nullptr : static_cast<const int8_t *>(src0_layout_ptr);
            const sycl::half * base_d =
            ...
            std::vector<sycl::event> gemm_deps = { tile_map_event, quant_event };
            if (has_table_event) {

                gemm_deps.push_back(table_event);
            }
            ...
            sycl::event gemm_event = moe_xmx::launch_fused_xmx_moe_q8_0_soa<4, 4>(
                *stream, submit_barrier(gemm_deps), base_qs, base_d,
                reinterpret_cast<const int8_t * const *>(expert_ptrs_device), use_ptr_table, q_tokens, token_scales,
                sorted_output, expert_offsets, expert_tile_offsets, total_tiles_host, n_experts, out_dim, in_dim,
                q8_qs_per_expert, xmx_cfg);
    ...
    } else if (src0->type == GGML_TYPE_Q8_0 && is_soa && g_ggml_sycl_graph_recording && !can_use_tile_mapping) {
        GGML_SYCL_DEBUG("[XMX MoE] Too many experts (%ld) for tile mapping\n", (long) n_experts);
        ctx.moe_graphs_disabled_once = true;
        return fail_and_cleanup();
    }
    ...
    if (require_tiled_mxfp4 && caps.supported) {
        ...
        if (use_ptr_table && !expert_ptrs_device) {
            GGML_SYCL_DEBUG("[MoE] Missing device pointer table for tiled MXFP4 path\n");
            if (g_ggml_sycl_graph_recording) {
                ctx.moe_graphs_disabled_once = true;
        ...
        sycl::event extract_event = stream->submit([&](sycl::handler & cgh) {
            cgh.depends_on(sort_event);
            cgh.parallel_for(sycl::range<1>(total_pairs),
        ...
        moe_xmx_fused::MXFPXMXLayoutInfo info = moe_xmx_fused::MXFPXMXLayoutInfo::compute(out_dim, in_dim, cfg);
        std::vector<sycl::event> gemm_deps = { extract_event, quant_event };
        if (has_table_event) {
            gemm_deps.push_back(table_event);

        }
        const uint8_t * tiled_base = use_ptr_table ? nullptr : static_cast<const uint8_t *>(src0_layout_ptr);
        auto [ok, evt]             = try_fused_xmx_moe_mxfp4_tiled(
            submit_barrier(gemm_deps), tiled_base, reinterpret_cast<const uint8_t * const *>(expert_ptrs_device),

            use_ptr_table, q_tokens, token_scales, sorted_token_ids, expert_offsets, sorted_output,
            static_cast<int>(total_pairs), static_cast<int>(n_experts), out_dim, in_dim,
    ...
    if ((fused_enabled || g_ggml_sycl_graph_recording) && !require_tiled_mxfp4 && src0->type == GGML_TYPE_MXFP4 &&
        is_soa && has_graph_compatible_fused && (!use_ptr_table || ptr_table_all_device)) {
        if (use_ptr_table && !expert_ptrs_device) {

            if (g_ggml_sycl_graph_recording) {
                GGML_SYCL_DEBUG("[MoE] Missing device pointer table for fused MXFP4 SoA path\n");
                ctx.moe_graphs_disabled_once = true;
                return fail_and_cleanup();

            }
            GGML_SYCL_DEBUG("[MoE] Missing device pointer table for fused MXFP4 SoA path, falling back\n");
        } else {
            ...
            const uint8_t * base_qs = use_ptr_table ? nullptr : src0_layout_u8;
            const uint8_t * base_e  = use_ptr_table ? nullptr : base_qs + soa_total_qs_mxfp4;
            std::vector<sycl::event> gemm_deps = { extract_event, quant_event };
            if (has_table_event) {
            ...
            auto [fused_ok, gemm_event] = try_fused_xmx_moe_mxfp4_soa(
                ...
                submit_barrier(gemm_deps), base_qs, base_e,
                reinterpret_cast<const uint8_t * const *>(expert_ptrs_device), use_ptr_table, q_tokens, token_scales,
                sorted_token_ids, expert_offsets, sorted_output, static_cast<int>(total_pairs),
                static_cast<int>(n_experts), out_dim, in_dim, mxfp4_qs_per_expert, nblocks_per_expert, ctx.device,
                *stream);
            if (fused_ok) {
    ...
    if (g_ggml_sycl_graph_recording) {
        GGML_SYCL_DEBUG("[MoE] Graph recording requires fused/tiled XMX path; falling back\n");
        return fail_and_cleanup();
    }
    ...
    GGML_SYCL_DEBUG("\n");
    std::vector<ggml_sycl_xmx_moe_expert_dispatch_ctx> stream_ctxs;
    stream_ctxs.reserve(static_cast<size_t>(n_experts));
    sycl::event stream_chain;

    bool        has_stream_chain = false;
    for (int64_t e = 0; e < n_experts; e++) {
        if (h_counts[e] == 0) {
            continue;
        }
        ...
        ggml_sycl_xmx_moe_expert_dispatch_ctx expert_ctx{};

        expert_ctx.type                = src0->type;
        expert_ctx.is_soa              = is_soa;
        expert_ctx.is_coalesced        = is_coalesced;

        expert_ctx.out_dim             = out_dim;
        expert_ctx.in_dim              = in_dim;
        expert_ctx.batch               = h_counts[e];
        expert_ctx.output_offset       = h_offsets[e] * out_dim;
        expert_ctx.nblocks_per_expert  = nblocks_per_expert;

        expert_ctx.q8_qs_per_expert    = q8_qs_per_expert;
        expert_ctx.mxfp4_qs_per_expert = mxfp4_qs_per_expert;

        expert_ctx.q_tokens            = q_tokens;
        expert_ctx.token_scales        = token_scales;
        expert_ctx.sorted_output       = sorted_output;
        expert_ctx.expert_scale_buf    = expert_scale_buf;
        ...
        if (use_ptr_table) {
            ...
            if (cache && expert_cache_key.valid) {
                view = cache->get_view(expert_cache_key, layout);
            }
            if (!view.ptr) {
                view.ptr      = const_cast<void *>(expert_weights);
                view.size     = expert_layout_bytes;
                view.layout   = layout;

                view.type     = ggml_sycl::cache_entry_type::MOE_EXPERT;
                view.location = infer_location(view.ptr);
            }

            if (!view.ptr) {
                GGML_LOG_ERROR("[XMX MoE] Failed to resolve cached expert layout for %s e=%ld\n", src0->name, (long) e);
                return fail_and_cleanup();
            }
            if (view.location != ggml_sycl::cache_location::DEVICE) {
                if (!cache) {
                    GGML_ABORT("[XMX MoE] Streaming requires unified cache");
                }
                stream_ctxs.push_back(expert_ctx);
                const auto * ctx_ptr = &stream_ctxs.back();
                std::vector<sycl::event> deps;
                if (has_stream_chain) {
                    deps.push_back(stream_chain);

                }
                if (expert_cache_key.valid) {

                    cache->pin(expert_cache_key, layout);
                }
                auto result = cache->stream_dma(view, expert_layout_bytes, expert_layout_bytes, 1,
                                                ggml_sycl_xmx_moe_stream_slice, ctx_ptr, deps);
                if (expert_cache_key.valid) {
                    if (result.ok) {
                        cache->unpin_on_event(expert_cache_key, layout, result.event);
                    } else {
                        cache->unpin(expert_cache_key, layout);
                    }
                }
                if (!result.ok) {
                    if (result.mmap_direct_failed) {
                        GGML_LOG_WARN("[XMX MoE] DMA from mmap failed (%s)\n", src0->name ? src0->name : "unknown");
                    }

                    return fail_and_cleanup();
                }
                stream_chain     = result.event;

                has_stream_chain = true;
                continue;

            }
            ggml_sycl_xmx_moe_dispatch_expert_ptr_table(expert_ctx, view.ptr, *stream);
            continue;
        }
        if (is_soa) {
            if (src0->type == GGML_TYPE_Q8_0) {
        ...
        ggml_sycl_xmx_moe_dispatch_expert_ptr_table(expert_ctx, expert_weights, *stream);
    }
...
static void ggml_sycl_mul_mat_id(ggml_backend_sycl_context & ctx, ggml_tensor * dst) try {
    ...
    if (ne12 == 1) {
        ...
        if (!ids_d) {
            ...
            GGML_LOG_ERROR("[MoE] Missing ids device pointer for %s\n", src0->name);
            return;
        ...
        const bool  use_fused_moe      = !fused_moe_disabled && fused_moe_esimd_available() && !use_expert_cache &&
        ...
        if (use_fused_moe) {
            ...
            if (src0->type == GGML_TYPE_Q8_0) {
                ...
                launch_fused_moe_mxfp4(src0_layout_ptr, (const float *) src1->data, ids_d, (float *) dst->data, nb02,
                                       ne00, ne01, n_ids, num_tokens, ne11, ids_nb0, ids_nb1, nb11, nb12, nb1, nb2,
...
static void ggml_backend_sycl_synchronize(ggml_backend_t backend) try {
    ...
    ggml_backend_sycl_context * sycl_ctx = (ggml_backend_sycl_context *) backend->context;
    const queue_ptr             stream   = sycl_ctx->stream(sycl_ctx->device, 0);
    if (g_ggml_sycl_debug && sycl_ctx) {
        GGML_SYCL_DEBUG("[SYCL] synchronize: device=%d graph_recording=%d has_last_graph_event=%d has_barrier=%d\n",

                        sycl_ctx->device, ggml_sycl_graph_recording_active() ? 1 : 0,
...
static void ggml_sycl_xmx_moe_pre_allocate_buffers(ggml_backend_sycl_context & ctx, ggml_cgraph * cgraph) {
    ...
    buffers.tokens_f16_input = sycl::malloc_device<sycl::half>(max_n_input_rows * max_in_dim, *stream);
    buffers.tokens_sorted    = sycl::malloc_device<sycl::half>(max_total_pairs * max_in_dim, *stream);
    buffers.token_map        = sycl::malloc_device<MoETokenMapping>(max_total_pairs, *stream);
    buffers.expert_counts    = sycl::malloc_device<int32_t>(max_n_experts, *stream);

    buffers.expert_offsets   = sycl::malloc_device<int32_t>(max_n_experts + 1, *stream);
    buffers.expert_write_pos = sycl::malloc_device<int32_t>(max_n_experts, *stream);

    buffers.sorted_output    = sycl::malloc_device<sycl::half>(max_total_pairs * max_out_dim, *stream);
    buffers.q_tokens         = sycl::malloc_device<int8_t>(max_total_pairs * max_in_dim, *stream);
    buffers.token_scales     = sycl::malloc_device<sycl::half>(max_total_pairs * max_blocks, *stream);
    buffers.expert_scale_buf = sycl::malloc_device<sycl::half>(max_out_dim * max_blocks, *stream);
    buffers.sorted_token_ids = sycl::malloc_device<int32_t>(max_total_pairs, *stream);
...
static ggml_status ggml_backend_sycl_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    if (use_sycl_graph) {
        ...
        // Pre-allocate XMX MoE buffers before graph recording (required for graph safety).

        if (!sycl_ctx->exec_graph) {
            ggml_sycl_xmx_moe_pre_allocate_buffers(*sycl_ctx, cgraph);
        }
...
static bool ggml_backend_sycl_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
...
#if defined(GGML_SYCL_F16)
                const bool type_ok =
                    (src0_type == GGML_TYPE_F32 || src0_type == GGML_TYPE_F16) && src0_type == dst_type;
#else
        ...
        case GGML_OP_LEAKY_RELU:
#if defined(GGML_SYCL_F16)
            return ((op->type == GGML_TYPE_F32 || op->type == GGML_TYPE_F16) && (op->src[0]->type == op->type));
#else
            return (op->type == GGML_TYPE_F32 && op->src[0]->type == GGML_TYPE_F32);
#endif
        case GGML_OP_TIMESTEP_EMBEDDING:
