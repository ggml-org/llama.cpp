 ...
 * Default: disabled (persistent kernel execution is too slow currently).
 *
...
inline bool env_persistent_tg_enabled() {
    ...
    if (enabled < 0) {
        ...
        enabled = (env != nullptr && std::strcmp(env, "1") == 0) ? 1 : 0;  // Default: disabled (persistent kernel exec too slow)
    }
    return enabled != 0;
}
