...
// Minimum query length to treat as prefill.
...
bool ggml_sycl_flash_attn_ext_onednn_supported(const ggml_tensor * dst) {
    ...
    return false;
#else
    if (!g_ggml_sycl_fa_onednn) { return false; }
    ...
    if (!mask || mask->type != GGML_TYPE_F16 || mask->ne[2] != 1 || mask->ne[3] != 1 || sinks) { return false; }
    float max_bias = 0.0f, logit_softcap = 0.0f;
    memcpy(&max_bias,      (const float *) dst->op_params + 1, sizeof(float));
    memcpy(&logit_softcap, (const float *) dst->op_params + 2, sizeof(float));
    if (max_bias != 0.0f || logit_softcap != 0.0f) { return false; }
    const int64_t d = K->ne[0];
    if (V->ne[0] != d || Q->ne[3] != 1) { return false; }
    if (K->ne[2] == 0 || Q->ne[2] % K->ne[2] != 0) { return false; }
    if (Q->ne[1] < GGML_SYCL_FA_ONEDNN_MIN_Q) { return false; }
    if (Q->ne[0] < 64) { return false; }  // precision loss on small head dims
    return true;
#endif
}
...
// Strided src (f16 or f32) -> contiguous f16 [ne0,ne1,ne2,ne3] (ne0 innermost).
// nb* are BYTE strides.
template <typename src_t>
static void cont_to_fp16_sycl(const char * src, sycl::half * dst,
...
// Uses actual dst strides (nb1, nb2, nb3) to handle both dense and interleaved layouts.
static void permute_sdpa_out_sycl(const sycl::half * out, float * dst,
    ...
    stream->parallel_for(sycl::range<1>(n), [=](sycl::id<1> ix) {
        ...
        int64_t off = (e * nb0 + h * nb1 + t * nb2 + b * nb3) / sizeof(float);
        dst[off] = (float) out[gid];
    });
}
...
void ggml_sycl_flash_attn_ext_onednn(ggml_backend_sycl_context & ctx, ggml_tensor * dst) try {
    ...
    // Copy inputs to contiguous f16 (head-major layout for the SDPA graph).
    ...
    cont_to_fp16_sycl<float>     ((const char *) Q->data, Qf.get(), d, q,   H,   mb, Q->nb[1], Q->nb[2], Q->nb[3], stream);
    cont_to_fp16_sycl<sycl::half>((const char *) K->data, Kf.get(), d, seq, Hkv, mb, K->nb[1], K->nb[2], K->nb[3], stream);
    cont_to_fp16_sycl<sycl::half>((const char *) V->data, Vf.get(), d, seq, Hkv, mb, V->nb[1], V->nb[2], V->nb[3], stream);

    // Scale as reciprocal: score / (1/kq_scale) == score * kq_scale.
    ...
    GGML_ASSERT(E.ok && "oneDNN SDPA partition failed to build for a supported shape");
    ...
    permute_sdpa_out_sycl(outf.get(), (float *) dst->data, mb, H, q, d,
