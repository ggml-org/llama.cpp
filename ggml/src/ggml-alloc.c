...
static ggml_backend_buffer_t ggml_tensor_backend_buffer(const struct ggml_tensor * tensor) {
    if (tensor == NULL) {
        return NULL;
    }
...
struct ggml_tallocr ggml_tallocr_new(ggml_backend_buffer_t buffer) {
    ...
    struct ggml_tallocr talloc = (struct ggml_tallocr) {
        ...
        /*.base      = */ base,
        /*.alignment = */ align,
        /*.offset    = */ aligned_offset(ggml_backend_buffer_has_stable_base(buffer) ? base : NULL, 0, align),
    };
    return talloc;
}
...
enum ggml_status ggml_tallocr_alloc(struct ggml_tallocr * talloc, struct ggml_tensor * tensor) {
    ...
    if (talloc->offset + size > ggml_backend_buffer_get_size(talloc->buffer)) {
        GGML_LOG_ERROR("%s: not enough space in the buffer to allocate %s (needed %zu, available %zu)\n", __func__,
                       tensor->name, size, ggml_backend_buffer_get_size(talloc->buffer) - talloc->offset);
        GGML_ABORT("not enough space in the buffer");
    }

    const size_t offset = talloc->offset;
    talloc->offset += size;

    if (ggml_backend_buffer_has_stable_base(talloc->buffer)) {
        assert((((uintptr_t) ggml_backend_buffer_get_base(talloc->buffer) + offset) % talloc->alignment) == 0);
    } else {
        assert((offset % talloc->alignment) == 0);
    }

    return ggml_backend_tensor_alloc_offset(talloc->buffer, tensor, offset);
}
...
static void remove_allocated_tensor(struct ggml_dyn_tallocr *  alloc,
                                    ...
                                    const struct ggml_tensor * tensor) {
    for (int i = 0; i < 1024; i++) {
        if (alloc->allocated_tensors[i].addr.chunk == addr.chunk &&
            alloc->allocated_tensors[i].addr.offset == addr.offset) {
...
static struct buffer_address ggml_dyn_tallocr_alloc(struct ggml_dyn_tallocr *  alloc,
                                                    size_t                     size,
                                                    const struct ggml_tensor * tensor) {
    ...
    if (cur_max > chunk->max_size) {
        ...
        for (int i = 0; i < 1024; i++) {
            if (alloc->allocated_tensors[i].tensor) {
                GGML_LOG_DEBUG(
                    "%s [%d: %zx-%zx] (%.2f MB) ", alloc->allocated_tensors[i].tensor->name,
                    alloc->allocated_tensors[i].addr.chunk, alloc->allocated_tensors[i].addr.offset,
...
static struct vbuffer * ggml_vbuffer_alloc(ggml_backend_buffer_type_t      buft,
...
static void ggml_vbuffer_tensor_alloc(struct vbuffer *      buf,
                                      struct ggml_tensor *  tensor,
                                      struct buffer_address buf_addr) {
    ggml_backend_buffer_t chunk  = buf->chunks[buf_addr.chunk];
    enum ggml_status      status = ggml_backend_tensor_alloc_offset(chunk, tensor, buf_addr.offset);
...
static bool ggml_gallocr_is_allocated(ggml_gallocr_t galloc, struct ggml_tensor * t) {
    return ggml_backend_tensor_has_storage(t)  // tensor already has logical or raw storage
           || ggml_gallocr_is_own(galloc, t);  // tensor will be allocated by galloc
}

static bool ggml_gallocr_same_allocation_origin(const struct ggml_tensor * a,
                                                const struct hash_node *   a_hn,
                                                const struct ggml_tensor * b,
                                                const struct hash_node *   b_hn) {
    ggml_backend_buffer_t a_buf = ggml_tensor_backend_buffer(a);
    ggml_backend_buffer_t b_buf = ggml_tensor_backend_buffer(b);
    if (a_buf != NULL && b_buf != NULL) {
        return a_buf == b_buf && a->buffer_offs == b->buffer_offs;
    }
    return a_hn->buffer_id == b_hn->buffer_id && a_hn->addr.chunk == b_hn->addr.chunk &&
           a_hn->addr.offset == b_hn->addr.offset;
}

// free the extra space at the end if the new tensor is smaller
static void ggml_gallocr_free_extra_space(ggml_gallocr_t       galloc,
                ...
                if (p_hn->n_children == 1 && p_hn->n_views == 0) {
                    if (ggml_is_view(parent)) {
                        ...
                        if (view_src_hn->n_views == 1 && view_src_hn->n_children == 0 &&
                            ggml_gallocr_same_allocation_origin(view_src, view_src_hn, parent, p_hn)) {
                            AT_PRINTF("reusing view parent %s (%s) for %s\n", parent->name, view_src->name, node->name);
                            assert(view_src_hn->addr.chunk == p_hn->addr.chunk &&
...
static void ggml_gallocr_free_node(ggml_gallocr_t galloc, struct ggml_tensor * node) {
    ...
    AT_PRINTF("%s: freeing %s at {chunk=%d, offset=%zu} (%zu bytes) - n_free_blocks = %d\n", __func__, node->name,
              hn->addr.chunk, hn->addr.offset, size, alloc->chunks[hn->addr.chunk]->n_free_blocks);
#ifdef GGML_ALLOCATOR_DEBUG
...
static void ggml_gallocr_alloc_graph_impl(ggml_gallocr_t       galloc,
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            ...
            AT_PRINTF("parent %s: %d children, %d views, allocated: %d\n", parent->name, p_hn->n_children,
                      p_hn->n_views, p_hn->allocated);
...
static bool ggml_gallocr_reserve_n_impl(ggml_gallocr_t       galloc,
                                        struct ggml_cgraph * graph,
                                        const int *          node_buffer_ids,
                                        const int *          leaf_buffer_ids,
                                        bool                 no_alloc) {
    ...
    for (int i = 0; i < galloc->n_buffers; i++) {
        ...
        if (realloc) {
#ifndef NDEBUG
            {
                ...
                if (cur_size > 0) {
                    GGML_LOG_DEBUG("%s: reallocating %s buffer from size %.02f MiB to %.02f MiB\n", __func__,
                                   ggml_backend_buft_name(galloc->bufts[i]), cur_size / 1024.0 / 1024.0,
            ...
            if (no_alloc) {
                ...
                galloc->buffers[i] =
                    ggml_vbuffer_alloc(galloc->bufts[i], galloc->buf_tallocs[i], GGML_BACKEND_BUFFER_USAGE_COMPUTE);
                if (galloc->buffers[i] == NULL) {
                    GGML_LOG_ERROR("%s: failed to allocate %s buffer of size %zu\n", __func__,
                                   ggml_backend_buft_name(galloc->bufts[i]), new_size);
                    return false;
                }
            }
        }
...
void ggml_gallocr_reserve_n_size(ggml_gallocr_t       galloc,
                                 struct ggml_cgraph * graph,
                                 const int *          node_buffer_ids,
                                 const int *          leaf_buffer_ids,
                                 size_t *             sizes) {
...
bool ggml_gallocr_reserve_n(ggml_gallocr_t       galloc,
                            struct ggml_cgraph * graph,
                            const int *          node_buffer_ids,
                            const int *          leaf_buffer_ids) {
...
static void ggml_gallocr_init_tensor(ggml_gallocr_t        galloc,
    ...
    assert(ggml_backend_tensor_has_storage(tensor) || tensor->view_src ||
           ggml_backend_buft_get_alloc_size(galloc->bufts[buffer_id], tensor) <= tensor_alloc->size_max);
    ...
    if (tensor->view_src != NULL) {
        ...
        if (!ggml_backend_tensor_has_storage(tensor)) {
            assert(tensor_alloc->addr.offset != SIZE_MAX);
...
static bool ggml_gallocr_node_needs_realloc(ggml_gallocr_t        galloc,
                                            struct ggml_tensor *  node,
                                            struct tensor_alloc * talloc) {
    size_t node_size = 0;
    if (!ggml_backend_tensor_has_storage(node) && !node->view_src) {
...
static bool alloc_tensor_range(struct ggml_context *      ctx,
    ...
    *buffers                   = realloc(*buffers, sizeof(ggml_backend_buffer_t) * (*n_buffers + 1));
    ...
    for (struct ggml_tensor * t = first; t != last; t = ggml_get_next_tensor(ctx, t)) {
        ...
        if (!ggml_backend_tensor_has_storage(t)) {
            if (t->view_src == NULL) {
...
static ggml_backend_buffer_t ggml_backend_alloc_ctx_tensors_from_buft_impl(struct ggml_context *      ctx,
                                                                           ggml_backend_buffer_type_t buft,
                                                                           size_t *                   nbytes_total,
                                                                           bool                       no_alloc) {
    ...
    *nbytes_total                     = 0;
    ...
    for (struct ggml_tensor * t = first; t != NULL; t = ggml_get_next_tensor(ctx, t)) {
        size_t this_size = 0;
        if (!ggml_backend_tensor_has_storage(t) && t->view_src == NULL) {
...
size_t ggml_backend_alloc_ctx_tensors_from_buft_size(struct ggml_context * ctx, ggml_backend_buffer_type_t buft) {
    ...
    ggml_backend_buffer_t buf =
        ggml_backend_alloc_ctx_tensors_from_buft_impl(ctx, buft, &nbytes_total, /*no_alloc=*/true);
    GGML_ASSERT(!buf);
    return nbytes_total;
}
