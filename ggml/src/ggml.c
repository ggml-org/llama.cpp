...
static struct ggml_tensor * ggml_gated_delta_net_impl(
    ...
    // in-place: the state is updated in `state` itself, the output holds the attention scores only
    const int64_t state_rows = inplace ? 0 : K * S_v * n_seqs;
...
struct ggml_tensor * ggml_gated_delta_net(
        ...
        struct ggml_tensor  * state,
        int64_t               K) {
    return ggml_gated_delta_net_impl(ctx, q, k, v, g, beta, state, K, false);
}
...
struct ggml_tensor * ggml_gated_delta_net_inplace(
        ...
        struct ggml_tensor  * beta,
        struct ggml_tensor  * state) {
    return ggml_gated_delta_net_impl(ctx, q, k, v, g, beta, state, 1, true);
}
