...
static void ggml_backend_opencl_buffer_get_tensor(ggml_backend_buffer_t buffer, const ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    ...
    if (tensor->type == GGML_TYPE_Q4_1) {
        ...
        if (use_adreno_kernels(backend_ctx, tensor)) {
            ...
            CL_CHECK((buf_trans_d = clCreateBuffer(context, CL_MEM_READ_WRITE,
                size_d, NULL, &err), err));
            CL_CHECK((buf_trans_m = clCreateBuffer(context, CL_MEM_READ_WRITE,
                size_m, NULL, &err), err));
            ...
            cl_int stride_k_m = K/32;
            size_t local_size_m[3] = {64, 1, 1};
            size_t global_size_m[3] = {(size_t)M, (size_t)stride_k_m, 1};

            CL_CHECK(clSetKernelArg(kernel, 0, sizeof(cl_mem), &extra->m));
            CL_CHECK(clSetKernelArg(kernel, 1, sizeof(cl_mem), &buf_trans_m));
            CL_CHECK(clSetKernelArg(kernel, 2, sizeof(cl_int), &M));
            CL_CHECK(clSetKernelArg(kernel, 3, sizeof(cl_int), &stride_k_m));

            CL_CHECK(clEnqueueNDRangeKernel(queue, kernel, 3, NULL,
                global_size_m, local_size_m, 0, NULL, NULL));
            ...
            kernel = backend_ctx->kernel_restore_block_q4_1_noshuffle;
            CL_CHECK(clSetKernelArg(kernel, 0, sizeof(cl_mem),   &buf_trans_q));
            CL_CHECK(clSetKernelArg(kernel, 1, sizeof(cl_mem),   &buf_trans_d));
            CL_CHECK(clSetKernelArg(kernel, 2, sizeof(cl_mem),   &buf_trans_m));
            ...
            CL_CHECK(clReleaseMemObject(buf_trans_q));
            CL_CHECK(clReleaseMemObject(buf_trans_d));
            CL_CHECK(clReleaseMemObject(buf_trans_m));
...
static void ggml_cl_mul_mat_q4_1_f32_adreno(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    GGML_ASSERT(src0t == GGML_TYPE_Q4_1);
    GGML_ASSERT(src1t == GGML_TYPE_F32);
    ...
    img_desc_1d.buffer = extra0_q4_1->q;
    ...
    if (N == 1) {
        CL_CHECK(clSetKernelArg(kernel,  k_arg++, sizeof(cl_mem),   &A_image1d));
        CL_CHECK(clSetKernelArg(kernel,  k_arg++, sizeof(cl_mem),   &extra0_q4_1->d));
        CL_CHECK(clSetKernelArg(kernel,  k_arg++, sizeof(cl_mem),   &extra0_q4_1->m));
        CL_CHECK(clSetKernelArg(kernel,  k_arg++, sizeof(cl_mem),   &B_image1d));
        CL_CHECK(clSetKernelArg(kernel,  k_arg++, sizeof(cl_ulong), &extra1->offset));
        CL_CHECK(clSetKernelArg(kernel,  k_arg++, sizeof(cl_mem),   &extrad->data_device));
        CL_CHECK(clSetKernelArg(kernel,  k_arg++, sizeof(cl_ulong), &extrad->offset));
        ...
        CL_CHECK(clSetKernelArg(kernel, 3, sizeof(cl_mem), &B_image1d));
        CL_CHECK(clSetKernelArg(kernel, 4, sizeof(cl_mem), &C_d));
        CL_CHECK(clSetKernelArg(kernel, 5, sizeof(int),    &ne01));
        CL_CHECK(clSetKernelArg(kernel, 6, sizeof(int),    &padded_N));
        CL_CHECK(clSetKernelArg(kernel, 7, sizeof(int),    &ne00));
        CL_CHECK(clSetKernelArg(kernel, 8, sizeof(int),    &ne1));
    }

    size_t global_work_size[3] = {
        (size_t)(ceil((float)ne1/8)), (size_t)(ne01/4), (size_t)(1)};
