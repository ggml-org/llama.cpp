...
static cl_program build_program_from_source(ggml_backend_opencl_context * backend_ctx, const char* program_buffer, const std::string &compile_opts) {
    ...
    // any failure; never blocks the build path.
    ...
    // Best-effort save; failures are logged inside.
...
#endif
        backend_ctx->program_CL_gemm = build_program_from_source(backend_ctx, kernel_src_q8_8x4_gemm.c_str(), compile_opts);
