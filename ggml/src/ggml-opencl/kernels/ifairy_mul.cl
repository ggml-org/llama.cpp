...
/**
 * Matches CPU iFairy multiplication: (ar + i*ai) * (br - i*bi),
 * using a 3-multiply rearrangement before final BF16 packing.
 */
static inline uint ifairy_mul_pair(uint a, uint b) {
    ...
    const uint r = (uint) ifairy_f32_to_bf16(ac + bd);
    const uint i = (uint) ifairy_f32_to_bf16(t - ac + bd);
    return r | (i << 16);
}

/**
 * Multiplies two tensors whose elements are packed iFairy complex values:
 * low 16 bits are real BF16, high 16 bits are imaginary BF16.
 */
kernel void kernel_ifairy_mul(
        global char * src0,
        ulong  offset0,
        global char * src1,
        ulong  offset1,
    ...
    for (int i0 = get_local_id(0); i0 < ne0; i0 += get_local_size(0)) {
        ...
        *((global uint *)(dst_ptr + i0*nb0)) = ifairy_mul_pair(a, b);
    }
}
