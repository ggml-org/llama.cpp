...
static void ggml_compute_fwht_kronecker(const struct ggml_compute_params * params, struct ggml_tensor * dst) {
    ...
    const src_t * src_data = (const src_t *) src1->data;
    float * dst_data = (float *) dst->data;
        ...
        for (int64_t p = 0; p < blocks_per_group; ++p) {
            ...
            for (int64_t i = 0; i < factor; i++) {
                ...
                for (int64_t j = 0; j < factor; ++j) {
                    sum += ggml_fwht_load(x[p * factor + j]) * ((factor == 12) ? H12[j][i] : H20[j][i]);
                }
                z[i] = sum * scale;
            }
            for (int64_t i = 0; i < factor; ++i) {
...
static void ggml_compute_forward_fwht_impl(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    const int64_t n = ne10;
    if ((n & (n - 1)) != 0) {
        ggml_compute_fwht_kronecker<src_t>(params, dst);
        return;
    }
