                    ...
                    for (int i02 = 0; i02 < ne02; i02++) {
                        ...
                        for (int i01 = ir0; i01 < ir1; i01++) {
                            const src_t * src0_ptr =
                                (src_t *) ((char *) src0->data + i01 * nb01 + i02 * nb02 + i03 * nb03);
...
static void ggml_compute_forward_scale_f32(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    if (b == 0.0f) {
        ...
        for (int i1 = ir0; i1 < ir1; i1++) {
            ggml_vec_mad1_f32(nc, (float *) ((char *) dst->data + i1 * nb1), (float *) ((char *) src0->data + i1 * nb1),
                              s, b);
        }
    }
}
...
static void ggml_compute_forward_diag_mask_f32(const ggml_compute_params * params,
                                               ggml_tensor *               dst,
                                               const float                 value) {
    const ggml_tensor * src0 = dst->src[0];
...
template <typename T>
static void rotate_pairs(const int64_t n,
        ...
        for (int64_t i2 = 0; i2 < ne2; i2++) {  // seq-len
            ...
            if (!mrope_used) {
                ...
                ggml_mrope_cache_init(p_t, p_h, p_w, p_e, sections, is_imrope, is_vision, freq_scale, freq_factors,
                                      corr_dims, ne0, ext_factor, attn_factor, cache, sin_sign, theta_scale);
            }
            ...
            for (int64_t i1 = 0; i1 < ne1; i1++) {  // attn-heads
                ...
                if (!is_vision) {
                    ...
                    for (int64_t i0 = n_dims; i0 < ne0; i0 += 2) {
                        const T * const src =
                            (T *) ((char *) src0->data + i3 * nb03 + i2 * nb02 + i1 * nb01 + i0 * nb00);
                            ...
                            for (int64_t ikw = 0; ikw < KW; ikw++) {
                                ...
                                if (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW) {
                                    ...
                                    dst_data[iic * (KH * KW) + ikh * KW + ikw] =
                                        GGML_CPU_FP32_TO_FP16(src_data[iih * IW + iiw]);
                                }
                            }
                        }
                    }
...
static void ggml_compute_forward_conv_2d_dw_cwhn(const ggml_compute_params *    params,
                                                 const ggml_tensor *            src,
                                                 const ggml_tensor *            kernel,
                                                 ggml_tensor *                  dst,
...
static void ggml_compute_forward_conv_2d_dw_whcn(const ggml_compute_params *    params,
                                                 const ggml_tensor *            src,
                                                 const ggml_tensor *            kernel,
                                                 ggml_tensor *                  dst,
                                                 const ggml_conv_2d_dw_params & p) {
...
static void ggml_compute_forward_pool_1d_ksp(const ggml_compute_params * params,
                    ...
                    for (int64_t i0 = 0; i0 < ne0; i0++) {
                        ...
                        for (int64_t sy = y_min; sy < y_max; sy++) {
                            ...
                            for (int64_t sx = x_min; sx < x_max; sx++) {
                                ...
                                const float pixel = *(const float *) ((const char *) src0->data + sx * nb00 +
                                                                      sy * nb01 + i02 * nb02 + i03 * nb03);
                                val += pixel * weight;
                                total_weight += weight;
                            }
                        }
...
static void ggml_compute_forward_flash_attn_ext_f16_one_chunk(const ggml_compute_params * params,
                                                              ggml_tensor *               dst,
                                                              int                         ir0,
                                                              int                         ir1,
                                                              int64_t                     ic_start,
...
static void ggml_compute_forward_flash_attn_ext_tiled(const ggml_compute_params * params,
                                                      ggml_tensor *               dst,
                                                      int                         ir0,
                                                      int                         ir1) {
    const ggml_tensor * q     = dst->src[0];
...
static void ggml_flash_attn_ext_reduce_partials(const ggml_compute_params * params,
                                                ggml_tensor *               dst,
...
static void ggml_compute_forward_flash_attn_back_f32(const ggml_compute_params * params,
                                                     const bool                  masked,
                                                     ggml_tensor *               dst) {
    const ggml_tensor * q = dst->src[0];
    const ggml_tensor * k = dst->src[1];
...
static void ggml_compute_forward_ssm_scan_f32(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    for (int i3 = 0; i3 < ns; ++i3) {
        const float * s0 =
            (const float *) ((const char *) src0->data + ids[i3] * (src0->nb[3]));  // {d_state, dim, nh, ns}
