...
// ggml_compute_forward_tc4_hc_comb
//
// telechat4 semantics: comb is [src, dst, n_tokens] (ne0 = src, fastest), raw
...
static void ggml_tc4_hc_comb_norm_dst(float * comb, float eps) {
    constexpr int64_t hc = 4;
...
static void ggml_tc4_hc_comb_norm_src(float * comb, float eps) {
    constexpr int64_t hc = 4;
...
static void ggml_compute_forward_tc4_hc_comb_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * mixes = dst->src[0];
    const ggml_tensor * scale = dst->src[1];
    ...
    for (int64_t it = it0; it < it1; ++it) {
        ...
        for (int32_t i = 0; i < n_iter; ++i) {
            ggml_tc4_hc_comb_norm_src(comb, eps);
            ggml_tc4_hc_comb_norm_dst(comb, eps);
        }
...
void ggml_compute_forward_tc4_hc_comb(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_tc4_hc_comb_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
// ggml_compute_forward_tc4_hc_pre

static void ggml_compute_forward_tc4_hc_pre_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * x       = dst->src[0];
    const ggml_tensor * weights = dst->src[1];
...
void ggml_compute_forward_tc4_hc_pre(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_tc4_hc_pre_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
// ggml_compute_forward_tc4_hc_post

static void ggml_compute_forward_tc4_hc_post_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * x        = dst->src[0];
    const ggml_tensor * residual = dst->src[1];
...
void ggml_compute_forward_tc4_hc_post(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_tc4_hc_post_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
