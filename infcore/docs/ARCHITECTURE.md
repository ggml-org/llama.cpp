...
через `mmproj_path` -> `--mmproj`). Audio: **вход** через чат (`input_audio` + audio-mmproj)
проксируется на llama-server; отдельные эндпоинты `/v1/audio/transcriptions` (ASR) и
`/v1/audio/speech` (TTS) НЕ реализованы — llama-server их не предоставляет (ASR = отдельный
whisper.cpp вне дерева, TTS = CLI `llama-tts`), это отдельная задача роадмапа (см. STATUS §6).
