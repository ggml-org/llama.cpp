...
struct SettingsView: View {
    ...
    @AppStorage(PleadTheFifthSettingsKey.covertTriggerPhrase)
    private var covertTriggerPhrase: String = ""
    ...
    private var pleadTheFifthSection: some View {
        Section {
            ...
            LabeledContent("Hot-key") {
                HStack(spacing: 6) {
                    Text(HotKeyMonitor.Chord.defaultChord.displayString)
                        .font(.system(.body, design: .monospaced))
                    Button("Change\u{2026}") {
                        // v2: surface a recorder sheet. v1 ships read-only.
                        hotKeyChangeHint = "Customizable in a future release; v1 binds Cmd+Shift+Backspace."
                    }
                    .disabled(true)
                }
            }
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                TextField("At least 8 characters", text: $covertTriggerDraft)
                    ...
                    .onChange(of: covertTriggerDraft) { _, newValue in
                        validateCovertTriggerDraft(newValue)
                    }
                ...
                HStack {
                    Button("Save phrase") { saveCovertTrigger() }
                        .disabled(covertTriggerDraft.isEmpty
                                  || covertTriggerDraft.trimmingCharacters(in: .whitespacesAndNewlines).count < PleadTheFifthSettings.minCovertPhraseLength)
                    Button("Test") { testCovertTrigger() }
            ...
            // Coercion mode
            Toggle("Coercion mode", isOn: $coercionMode)
                .help("Hide the visible 'Plea the Fifth' menu items. The hot-key and covert trigger still work.")
            Text("Coercion mode hides the visible 'Plea the Fifth' controls. The hot-key and covert trigger still work.")
                .font(.caption)
                .foregroundStyle(.secondary)
            // Last wipe report
            Button("View last wipe report\u{2026}") {
                openLastWipeReport()
            }
        } header: {
            Text("Plea the Fifth")
    ...
    @State private var hotKeyChangeHint: String?

    private func validateCovertTriggerDraft(_ value: String) {
        let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
        if trimmed.isEmpty {
            covertTriggerError = nil
            return
        }
        if trimmed.count < PleadTheFifthSettings.minCovertPhraseLength {
            covertTriggerError = "Phrase must be at least \(PleadTheFifthSettings.minCovertPhraseLength) characters (got \(trimmed.count))."
        } else {
            covertTriggerError = nil
        }
    }
    ...
    private func saveCovertTrigger() {
        do {
            ...
            covertTriggerPhrase = PleadTheFifthSettings.covertTriggerPhrase
            covertTriggerError = nil
    ...
    private func testCovertTrigger() {
        // The "Test" button: simulate the trigger without executing
        // the wipe. We do this by asking the monitor to evaluate a
        // sample sentence that contains the configured phrase. The
        // monitor returns true if the trigger would have fired.
    ...
    private func openLastWipeReport() {
        // The report window is owned by the macOS app's coordinator;
        // we open it through a Notification so the wiring stays out
        // of the settings view. (The app posts a name; the menu
        // item's report window listens.) For v1 the menu item also
        // owns the window - we post a notification that the app
        // routes to the menu item.
        NotificationCenter.default.post(
            name: .openLastWipeReport,
            object: nil
        )
    }
