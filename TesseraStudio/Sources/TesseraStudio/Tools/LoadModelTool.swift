...
struct LoadModelTool: TesseraTool {
    let name = "load_model"
    let description = "Load a GGUF model into the Tessera engine, optionally with a sidecar policy and imatrix."
    ...
    func execute(arguments: [String: JSONValue]) async throws -> ToolResult {
        ...
        // In production, this calls TesseraEngineBridge.load().
