...
| GGML_SYCL_FA_DECODE_KERNEL | auto (default) or vec or tile | Override the decode flash-attention kernel. Default auto: quantized KV decode uses the TILE kernel; F16 decode uses existing dispatch. Set to vec or tile to force the decode kernel for A/B testing. |
