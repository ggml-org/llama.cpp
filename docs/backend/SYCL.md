...
| GGML_SYCL_FA_ONEDNN_MAX_KV | 0 (default, disabled) or positive integer | Max KV length (in tokens) the oneDNN fused SDPA handles; longer sequences fall back to the native kernel. 0 = no limit. If GPU driver watchdog resets (DEVICE_LOST) occur during long-context inference, set this near the context depth where they start, e.g. 24576. |
