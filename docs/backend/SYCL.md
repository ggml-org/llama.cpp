...
| GGML_SYCL_COMM_DIRECT_ALLOC | 0 (default) or 1 | Workaround for a GPU crash/hang with `--split-mode tensor` on two GPUs (e.g. Arc Pro B70). Allocates the cross-device all-reduce buffers as single device allocations instead of from the VMM pool, whose allocations can span several physical chunks; current Level Zero drivers fail on a peer copy that crosses a chunk boundary (intel/compute-runtime#995). Only relevant with `GGML_SYCL_ENABLE_VMM=1` (the legacy pool allocates contiguously). Not needed once the driver is fixed. |
