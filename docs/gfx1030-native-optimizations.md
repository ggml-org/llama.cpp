...
| `GGML_TP_VOCAB_SHARDED_OUTPUT` | unset / `0`: disabled; explicit `1` enables | Splits the primary Qwen35/Qwen35MoE LM head across vocabulary rows instead of hidden columns. This removes the full-logit output AllReduce for CPU-sampled tensor-parallel inference. Native MTP target/draft backend sampling falls back to CPU; sidecar-local draft sampling is unaffected. |
...
The normal Qwen35 tensor-parallel output path splits the primary LM head along its hidden dimension and AllReduces partial full-vocabulary logits. This preserves a complete logit vector on every GPU and is required by the current target backend sampler. For workloads that sample target logits on the CPU, set `GGML_TP_VOCAB_SHARDED_OUTPUT=1` to split only the primary output head across vocabulary rows and concatenate those shards during host readback instead. Native-MTP auxiliary heads retain the existing full-logit policy when distinct; the Qwen3.8-27B artifact currently reuses the primary full-vocabulary head, so native MTP automatically uses CPU draft sampling in this mode. MTP/DFlash sidecars keep their provider-local device sampling.

The mode is deliberately explicit. Server-wide or request-level target backend sampling falls back to CPU with a warning, and the automatic gfx1030 target-backend policy is suppressed. Unset the variable to retain full-logit backend-sampling capability. `GGML_TP_SHARDED_OUTPUT=1` remains the primary enable; the vocabulary variable selects its axis for the primary head only.
