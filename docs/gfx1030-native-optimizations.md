...
| `GGML_TP_SHARDED_OUTPUT` | unset / `auto`: automatic hidden-axis/full-logit policy; explicit `0` disables; explicit `1` selects vocabulary-axis output | Enables TP output-head sharding. For explicit `1`, the primary Qwen35/Qwen35MoE LM head is vocabulary-sharded, removing the full-logit output AllReduce for CPU-sampled inference. Native MTP target/draft backend sampling falls back to CPU; sidecar-local draft sampling is unaffected. |
...
The normal Qwen35 tensor-parallel output path splits the primary LM head along its hidden dimension and AllReduces partial full-vocabulary logits. This preserves a complete logit vector on every GPU and is required by the current target backend sampler. For workloads that sample target logits on the CPU, set `GGML_TP_SHARDED_OUTPUT=1` to enable output sharding and split only the primary output head across vocabulary rows, concatenating those shards during host readback instead. Native-MTP auxiliary heads retain the existing full-logit policy when distinct; the Qwen3.8-27B artifact currently reuses the primary full-vocabulary head, so native MTP automatically uses CPU draft sampling in this mode. MTP/DFlash sidecars keep their provider-local device sampling.

The explicit `GGML_TP_SHARDED_OUTPUT=1` mode is deliberately separate from the automatic `unset`/`auto` policy: explicit `1` selects vocabulary-axis output and CPU-target sampling safety, while unset/`auto` retains hidden-axis/full-logit output for backend sampling. Server-wide or request-level target backend sampling falls back to CPU with a warning, and the automatic gfx1030 target-backend policy is suppressed in explicit `1` mode. Use `GGML_TP_SHARDED_OUTPUT=0` to disable output-head sharding.
