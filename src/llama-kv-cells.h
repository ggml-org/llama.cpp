...
public:
    ...
    llama_seq_id seq_get(uint32_t i) const {
        ...
        for (int s = 0; s < N_SEQ_WORDS; ++s) {
            if (seq[i].w[s]) return s * 64 + llama_bits::countr_zero64(seq[i].w[s]);
        }
        return -1;
    }
    ...
    void add_sequences(uint32_t i, int32_t n, llama_seq_id *_seq);
    void compact(llama_seq_id s);
    uint32_t next_head(int32_t seq_id, llama_pos p0, llama_pos p1);
  private:
    ...
    void seq_pos_rm(uint32_t i) {
        for (int s = 0; s < N_SEQ_WORDS; ++s) {
            for (auto m = seq[i].w[s]; m; m &= m - 1) {
             seq_pos_dec(s * 64 + llama_bits::countr_zero64(m), pos[i]);
            }
        }
    }
    ...
    void seq_pos_add(uint32_t i) {
        for (int s = 0; s < N_SEQ_WORDS; ++s) {
            for (auto m = seq[i].w[s]; m; m &= m - 1) {
             seq_pos_inc(s * 64 + llama_bits::countr_zero64(m), pos[i]);
            }
        }
    }
};
