            ...
            for (uint32_t i = 0; i < n_test; i++) {
                ...
                if (!can_use && cells.seq_count(idx) == 1) {
                    ...
                    if (!can_use) {
                        ...
                        // SWA mask
                        // deliberately no n_ref arg: a REFERENCE cell must never be recycled (never-evict)
...
void llama_kv_cache::apply_ubatch(const slot_info & sinfo, const llama_ubatch & ubatch) {
    ...
    // single-token decode; a prompt whose last ubatch is exactly 1 token latches one
    // early, and multi-output decode (speculative) is excluded via ref_latch_enabled.
    if (swa_type == LLAMA_SWA_TYPE_REFERENCE && ref_latch_enabled) {
        ...
        for (uint32_t ii = 0; ii < n_tps; ++ii) {
            ...
            for (uint32_t jj = 0; jj < n_kv; ++jj) {
                ...
                if (swa) {
                    ...
                    if (!in_span && llama_hparams::is_masked_swa(n_swa, swa_type, p0, p1, args.n_ref[seq_id])) {
                        goto skip;
                    }
                }
...
void llama_kv_cache::state_write(llama_io_write_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) const {
    ...
    for (uint32_t s = 0; s < n_stream; ++s) {
        ...
        for (uint32_t i = 0; i < cells.size(); ++i) {
            ...
            // deliberately no n_ref arg: a REFERENCE seq must persist all its cells
