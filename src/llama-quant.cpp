...
static float llama_nvfp4_input_scale_from_imatrix(
    ...
    int64_t n_per_row,
    int64_t n_slices) {
    if (imatrix == nullptr || n_per_row <= 0 || n_slices <= 0) {
        return 1.0f;
    }

    const size_t n = (size_t) n_per_row * (size_t) n_slices;
    double sum = 0.0;
    size_t count = 0;
    ...
    // GGUF stores the file-domain activation scale; runtime CUDA paths consume its reciprocal.
    ...
    // Pre-register NVFP4 tensor-scale keys so metadata size is final before writing any data.
    {
        ...
        for (const auto * it : tensors) {
            ...
            if (new_type == GGML_TYPE_NVFP4) {
                ...
                const std::string k2 = name + ".input_scale";
                gguf_set_val_f32(ctx_outs[i_split].get(), k1.c_str(), 1.0f);
                gguf_set_val_f32(ctx_outs[i_split].get(), k2.c_str(), 1.0f);
            }
        }
    }
    ...
    for (const auto * it : tensors) {
        ...
        if (!quantize) {
            ...
            if (is_merged_moe_gate_up) {
                ...
                if (tensor->type == GGML_TYPE_BF16 && new_type == GGML_TYPE_NVFP4) {
            ...
            } else if (tensor->type == GGML_TYPE_BF16 && new_type == GGML_TYPE_NVFP4) {
            ...
            if (new_type == GGML_TYPE_NVFP4) {
                ...
                const int64_t n_slices = tensor_meta->ne[2] * tensor_meta->ne[3];
                input_scale = llama_nvfp4_input_scale_from_imatrix(imatrix, tensor_meta->ne[0], n_slices);
                float absmax = 0.0f;
            ...
            const bool parallel_nvfp4_slices = (new_type == GGML_TYPE_NVFP4) && (n_slices > 1);
                    ...
                    while (true) {
                        ...
                        try {
                            const size_t slice_size = llama_tensor_quantize_impl(
                                ...
                                new_data_s, chunk_size, nrows, n_per_row, imatrix_s, local_workers, 1, tensor_meta->name);
                ...
                for (int64_t i04 = 0; i04 < ne3; ++i04) {
                    for (int64_t i03 = 0; i03 < ne2; ++i03) {
                        ...
                        new_size += llama_tensor_quantize_impl(
                            new_type, f32_data_s, bf16_data_s, tensor_scale,
                            new_data_s, chunk_size, nrows, n_per_row, imatrix_s, workers, nthread_use, tensor_meta->name);
                    }
                }
            }
        ...
        if (nvfp4_quantized) {
            ...
            gguf_set_val_f32(ctx_outs[cur_split].get(), k2.c_str(), input_scale);
        }
