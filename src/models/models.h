...
struct llama_model_phi4flash : public llama_model_base {
    llama_model_phi4flash(const struct llama_model_params & params)
    ...
    // The nested graph struct replaces the old llm_build_phi4flash class.
    // It inherits from llm_build_mamba_base (not llm_graph_context directly)
    // because the model has Mamba layers.
    struct graph : public llm_build_mamba_base {
        graph(const llama_model & model, const llm_graph_params & params);

    private:
        // These are the KV helpers — they live HERE on graph, not on the
        // model. They need access to gf, ctx0, hparams etc. which are all
        // members of llm_graph_context, the base of llm_build_mamba_base.
        void write_kv_iswa(
