// TeleChat4: DeepSeek-V3 style MLA attention + DeepSeekMoE, with
...
//   * the residual streams are averaged (tc4_hc_mean) at the head, no hc_head
...
static size_t tc4_elem_offset(const ggml_tensor * t, int64_t i) {
    return ggml_row_size(t->type, i);
}

static ggml_tensor * tc4_view_1d(ggml_context * ctx, ggml_tensor * t, int64_t ne0, int64_t i0) {
    return ggml_view_1d(ctx, t, ne0, tc4_elem_offset(t, i0));
}

static ggml_tensor * tc4_view_2d(
        ggml_context * ctx,
        ggml_tensor  * t,
        int64_t        ne0,
        int64_t        ne1,
        int64_t        i0) {
    return ggml_view_2d(ctx, t, ne0, ne1, t->nb[1], tc4_elem_offset(t, i0));
}

// mean over the hyper-connection streams: [n_embd, hc, n_tokens] -> [n_embd, n_tokens]
static ggml_tensor * tc4_hc_mean(ggml_context * ctx, ggml_tensor * x) {
...
static ggml_tensor * tc4_hc_affine(
        ggml_context * ctx,
        ggml_tensor  * x,
        ggml_tensor  * scale,
        ggml_tensor  * base) {
...
void llama_model_telechat4::load_arch_hparams(llama_model_loader & ml) {
    uint32_t n_vocab = 0;
    ...
    ml.get_key(LLM_KV_HYPER_CONNECTION_COUNT,               hparams.tc4_hc_mult);
    ml.get_key(LLM_KV_HYPER_CONNECTION_SINKHORN_ITERATIONS, hparams.tc4_hc_sinkhorn_iters);
    ml.get_key(LLM_KV_HYPER_CONNECTION_EPSILON,             hparams.tc4_hc_eps);
    GGML_ASSERT(hparams.tc4_hc_mult == 4 && "telechat4 requires 4 residual streams");
...
void llama_model_telechat4::load_arch_tensors(llama_model_loader & ml) {
    LLAMA_LOAD_LOCALS;
    const int64_t n_expert_shared = hparams.n_expert_shared;
    const int64_t tc4_hc_dim      = hparams.tc4_hc_mult * n_embd;
    const int64_t tc4_hc_mix_dim  = (2 + hparams.tc4_hc_mult) * hparams.tc4_hc_mult;
    ...
    GGML_ASSERT(hparams.is_mla() && "telechat4 requires MLA");
    GGML_ASSERT(hparams.n_lora_q > 0 && "telechat4 requires q_lora_rank");
    ...
    for (int i = 0; i < n_layer_all; ++i) {
        ...
        if (i < n_layer) {
            layer.hc_attn_fn    = create_tensor(tn(LLM_TENSOR_HC_ATTN_FN,    "weight", i), {tc4_hc_dim, tc4_hc_mix_dim}, flags);
            layer.hc_attn_base  = create_tensor(tn(LLM_TENSOR_HC_ATTN_BASE,  "weight", i), {tc4_hc_mix_dim}, flags);
            layer.hc_attn_scale = create_tensor(tn(LLM_TENSOR_HC_ATTN_SCALE, "weight", i), {3}, flags);
            layer.hc_ffn_fn     = create_tensor(tn(LLM_TENSOR_HC_FFN_FN,     "weight", i), {tc4_hc_dim, tc4_hc_mix_dim}, flags);
            layer.hc_ffn_base   = create_tensor(tn(LLM_TENSOR_HC_FFN_BASE,   "weight", i), {tc4_hc_mix_dim}, flags);
...
std::unique_ptr<llm_graph_context> llama_model_telechat4::build_arch_graph(const llm_graph_params & params) const {
...
llama_model_telechat4::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params) :
    llm_graph_context(params) {
    GGML_ASSERT(hparams.n_layer_nextn > 0 && "telechat4 MTP requires n_layer_nextn > 0");
    GGML_ASSERT(hparams.n_layer_nextn == 1 && "telechat4 MTP currently only supports a single MTP block");
    GGML_ASSERT(hparams.is_mla() && "telechat4 MTP requires MLA");
    ...
    GGML_ASSERT((uint32_t) il >= hparams.n_layer_dense_lead && "telechat4 MTP block expected to use MoE FFN");
    ...
    ggml_tensor * head_norm_w = layer.nextn.shared_head_norm
            ...
            : model.output_norm;
    GGML_ASSERT(head_norm_w && "telechat4 MTP: missing both nextn.shared_head_norm and output_norm");
    ...
    GGML_ASSERT(head_w && "telechat4 MTP: missing LM head (nextn.shared_head_head or model.output)");
...
ggml_tensor * llama_model_telechat4::graph::build_hc_pre(
        ggml_tensor * x,
        ggml_tensor * weights,
        int           il) const {
    GGML_UNUSED(il);
    ...
    GGML_ASSERT(x->ne[1] == hparams.tc4_hc_mult);
...
ggml_tensor * llama_model_telechat4::graph::build_hc_sinkhorn(
        ggml_tensor * comb,
        int           il) const {
    GGML_UNUSED(il);
    ...
    eps = ggml_fill(ctx0, eps, hparams.tc4_hc_eps);
    ...
    norm_dst();
    for (uint32_t i = 1; i < hparams.tc4_hc_sinkhorn_iters; ++i) {
        norm_src();
        norm_dst();
    }
...
ggml_tensor * llama_model_telechat4::graph::build_hc_pre(
    ...
    ggml_tensor * scale_pre  = tc4_view_1d(ctx0, hc_scale, 1, 0);
    ggml_tensor * scale_post = tc4_view_1d(ctx0, hc_scale, 1, 1);

    ggml_tensor * base_pre  = tc4_view_1d(ctx0, hc_base, hc, 0);
    ggml_tensor * base_post = tc4_view_1d(ctx0, hc_base, hc, hc);

    ggml_tensor * pre = tc4_view_2d(ctx0, mixes, hc, nt, 0);
    pre = tc4_hc_affine(ctx0, pre, scale_pre, base_pre);
    // the reference with_clamp kernel has no hc_pre_eps on the pre gate
    pre = ggml_sigmoid(ctx0, pre);
    cb(pre, "hc_pre", il);

    *post = tc4_view_2d(ctx0, mixes, hc, nt, hc);
    *post = tc4_hc_affine(ctx0, *post, scale_post, base_post);
...
ggml_tensor * llama_model_telechat4::graph::build_hc_post(
        ggml_tensor * x,
    ...
    for (int64_t dst = 0; dst < hc; ++dst) {
        ...
        for (int64_t src = 0; src < hc; ++src) {
            ...
            // NOTE: telechat4's vllm reference (with_clamp kernel) transposes the
...
llama_model_telechat4::graph::graph(const llama_model & model, const llm_graph_params & params) :
    llm_graph_context(params) {
    const int64_t hc = hparams.tc4_hc_mult;
