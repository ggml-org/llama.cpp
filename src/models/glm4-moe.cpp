...
void llama_model_glm4_moe::load_arch_tensors(llama_model_loader & ml) {
    ...
    for (int i = 0; i < n_layer_all; ++i) {
        auto & layer = layers[i];
        const int flags = i < n_layer || ml.load_mtp ? 0 : TENSOR_SKIP;
...
llama_model_glm4_moe::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    GGML_ASSERT(layer.ffn_gate_exps && "MTP block missing ffn_gate_exps");
    GGML_ASSERT(layer.ffn_down_exps && "MTP block missing ffn_down_exps");
    GGML_ASSERT(layer.ffn_up_exps   && "MTP block missing ffn_up_exps");
    GGML_ASSERT(layer.ffn_gate_shexp && "MTP block missing ffn_gate_shexp");
    GGML_ASSERT(layer.ffn_down_shexp && "MTP block missing ffn_down_shexp");
    GGML_ASSERT(layer.ffn_up_shexp   && "MTP block missing ffn_up_shexp");
