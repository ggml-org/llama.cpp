...
std::pair<ggml_tensor *, ggml_tensor *> llm_build_delta_net_base::build_delta_net_fused(
    ...
    if (n_tokens == 1) {
        mark_fused(LLM_FUSED_OP_GDN_AR, result, LLAMA_TENSOR_NAME_FGDN_AR, il);
    } else {
        mark_fused(LLM_FUSED_OP_GDN_CH, result, LLAMA_TENSOR_NAME_FGDN_CH, il);
    }
...
ggml_tensor * llm_build_delta_net_base::build_recurrent_attn(
    ...
    if (n_seq_tokens > 1) {
        mark_fused(LLM_FUSED_OP_GDN_CH, gdn_out, LLAMA_TENSOR_NAME_FGDN_CH, il);
    } else {
        mark_fused(LLM_FUSED_OP_GDN_AR, gdn_out, LLAMA_TENSOR_NAME_FGDN_AR, il);
    }
