...
std::pair<ggml_tensor *, ggml_tensor *> llm_build_delta_net_base::build_delta_net_fused(
    ...
    if (n_tokens == 1) {
        mark_fused(LLM_FUSED_OP_GDN_AR, result, il);
    } else {
        mark_fused(LLM_FUSED_OP_GDN_CH, result, il);
    }
...
ggml_tensor * llm_build_delta_net_base::build_recurrent_attn(
    ...
    if (n_seq_tokens > 1) {
        mark_fused(LLM_FUSED_OP_GDN_CH, gdn_out, il);
    } else {
        mark_fused(LLM_FUSED_OP_GDN_AR, gdn_out, il);
    }
