...
void llama_model_quatfit1::load_arch_hparams(llama_model_loader & ml) {
    ...
    hparams.f_attention_scale     = 1.0f; // self.scaling = 1.0 (no pre-attn scaling)
...
void llama_model_quatfit1::load_arch_tensors(llama_model_loader &) {
    LLAMA_LOAD_LOCALS;
    ...
    if (n_embd_head_k != n_embd_head_v) {
        throw std::runtime_error("Quatfit1 requires n_embd_head_k == n_embd_head_v");
    }
    if (hparams.n_embd_head_k_swa != hparams.n_embd_head_v_swa) {
        throw std::runtime_error("Quatfit1 requires n_embd_head_k_swa == n_embd_head_v_swa");
    }
...
std::unique_ptr<llm_graph_context> llama_model_quatfit1::build_arch_graph(const llm_graph_params & params) const {
...
llama_model_quatfit1::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // mirror attention in pytorch code
        ggml_tensor * Qcur;
        {
    ...
    // apply logits bias if needed (e.g. for suppress_tokens patch)
...
ggml_tensor * llama_model_quatfit1::graph::build_inp_per_layer() {
...
ggml_tensor * llama_model_quatfit1::graph::project_per_layer_inputs(ggml_tensor * inp_batch, ggml_tensor * inp_per_layer) {
