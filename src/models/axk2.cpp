...
void llama_model_axk2::load_arch_hparams(llama_model_loader & ml) {
    ...
    ml.get_key(LLM_KV_GATED_NORM_RANK, hparams.gated_norm_rank);
    ...
    switch (hparams.n_layer()) {
        case 61: type = LLM_TYPE_688B_A33B; break;
        default: type = LLM_TYPE_UNKNOWN;
    }
}

void llama_model_axk2::load_arch_tensors(llama_model_loader &) {
    LLAMA_LOAD_LOCALS;
    const bool is_mla = hparams.is_mla();
    if (!is_mla) {
        throw std::runtime_error("AXK2 architecture requires MLA");
    }
    ...
    const int64_t gated_norm_rank = hparams.gated_norm_rank;
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        layer.attn_norm_gate_a = create_tensor(tn(LLM_TENSOR_ATTN_NORM_GATE_A, "weight", i), {n_embd, gated_norm_rank}, 0);
        layer.attn_norm_gate_b = create_tensor(tn(LLM_TENSOR_ATTN_NORM_GATE_B, "weight", i), {gated_norm_rank, n_embd}, 0);
        ...
        // head-specific gate on the attention output, sharing the query LoRA bottleneck
        layer.wqkv_gate = create_tensor(tn(LLM_TENSOR_ATTN_GATE, "weight", i), {q_lora_rank, n_head * n_embd_head_v_mla}, 0);
        ...
        // DSA indexer
        layer.indexer_k_norm   = create_tensor(tn(LLM_TENSOR_INDEXER_K_NORM,   "weight", i), {hparams.indexer_head_size}, 0);
        layer.indexer_k_norm_b = create_tensor(tn(LLM_TENSOR_INDEXER_K_NORM,   "bias",   i), {hparams.indexer_head_size}, 0);
        layer.indexer_proj     = create_tensor(tn(LLM_TENSOR_INDEXER_PROJ,     "weight", i), {n_embd, hparams.indexer_n_head}, 0);
        layer.indexer_attn_k   = create_tensor(tn(LLM_TENSOR_INDEXER_ATTN_K,   "weight", i), {n_embd, hparams.indexer_head_size}, 0);
        ...
        if (i < (int) hparams.n_layer_dense_lead) {
            ...
            // the FFN norm is gated only on the MoE layers
            layer.ffn_norm_gate_a = create_tensor(tn(LLM_TENSOR_FFN_NORM_GATE_A, "weight", i), {n_embd, gated_norm_rank}, 0);
            layer.ffn_norm_gate_b = create_tensor(tn(LLM_TENSOR_FFN_NORM_GATE_B, "weight", i), {gated_norm_rank, n_embd}, 0);
...
std::unique_ptr<llm_graph_context> llama_model_axk2::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

ggml_tensor * llama_model_axk2::graph::build_gated_norm(
        ggml_tensor * cur,
        ggml_tensor * norm,
        ggml_tensor * gate_a,
        ggml_tensor * gate_b,
        int           il) {
    ggml_tensor * y = build_norm(cur, norm, NULL, LLM_NORM_RMS, il);
    cb(y, "gated_norm_y", il);

    ggml_tensor * gate = ggml_mul_mat(ctx0, gate_a, y);
    gate = ggml_silu(ctx0, gate);
    gate = ggml_mul_mat(ctx0, gate_b, gate);
    cb(gate, "gated_norm_gate", il);

    return ggml_mul(ctx0, y, ggml_sigmoid(ctx0, gate));
}

llama_model_axk2::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // norm
        cur = build_gated_norm(inpL, model.layers[il].attn_norm, model.layers[il].attn_norm_gate_a, model.layers[il].attn_norm_gate_b, il);
        cb(cur, "attn_norm", il);

        // self_attention
        {
            ggml_tensor * q_c = ggml_mul_mat(ctx0, model.layers[il].wq_a, cur);
            cb(q_c, "q_c", il);

            // note: the output gate reads the query LoRA bottleneck before the norm, while the
            // query itself reads it after - keep both around
            ggml_tensor * qr = build_norm(q_c, model.layers[il].attn_q_a_norm, nullptr, LLM_NORM_RMS, il);
            cb(qr, "qr", il);
            ...
            // MLA attention
            {
                ...
                ggml_tensor * gate = ggml_mul_mat(ctx0, model.layers[il].wqkv_gate, q_c);
        ...
        if ((uint32_t) il < hparams.n_layer_dense_lead) {
            ...
            cur = build_gated_norm(ffn_inp, model.layers[il].ffn_norm, model.layers[il].ffn_norm_gate_a, model.layers[il].ffn_norm_gate_b, il);
        }
        cb(cur, "ffn_norm", il);
