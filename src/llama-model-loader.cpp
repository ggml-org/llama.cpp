...
// submit independent RPC tensors from multiple workers so hashing happens in
// parallel; endpoint I/O is serialized by the RPC dispatcher.
...
bool llama_model_loader::load_all_data(
    ...
    if (use_mmap && !check_tensors && rpc_threads > 1) {
        ...
        for (ggml_tensor * tensor : tensors) {
            const auto * weight = get_weight(ggml_get_name(tensor));
            if (weight == nullptr || lazy.has(tensor) || tensor->data == nullptr ||
                    !ggml_buffer_is_rpc(tensor->buffer)) {
                continue;
            }
