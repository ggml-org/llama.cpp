...
bool llama_model_loader::load_all_data(
    ...
    ggml_backend_t upload_backend = [&](const char * func) -> ggml_backend_t {
        ...
        const char * async_env = getenv("LLAMA_ASYNC_UPLOADS");
        if (async_env != nullptr) {
            if (atoi(async_env) == 0) {
                LLAMA_LOG_DEBUG("%s: async uploads disabled\n", func);
                return nullptr;
            }
        } else {
#if defined(GGML_USE_HIP)
            // Disable async uploads on ROCm by default - enabling them can severely reduce multi-GPU inference
            // performance (loss of overlap/parallelism) and has also been associated with HIP runtime issues.
