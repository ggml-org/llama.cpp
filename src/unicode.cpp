...
// Tekken (Mistral v3) system regex:
// [^\r\n\p{L}\p{N}]?((?=[\p{L}])([^a-z]))*((?=[\p{L}])([^A-Z]))+|[^\r\n\p{L}\p{N}]?((?=[\p{L}])([^a-z]))+((?=[\p{L}])([^A-Z]))*|\p{N}| ?[^\s\p{L}\p{N}]+[\r\n/]*|\s*[\r\n]+|\s+(?!\S)|\s+
static std::vector<size_t> unicode_regex_split_custom_tekken(const std::string & text, const std::vector<size_t> & offsets) {
    std::vector<size_t> bpe_offsets;
    bpe_offsets.reserve(offsets.size());
    ...
    for (auto offset : offsets) {
        ...
        auto is_not_rn_L_N = [](uint32_t c, unicode_cpt_flags f) {
            return c != '\r' && c != '\n' && !f.is_letter && !f.is_number;
        };
        auto is_not_lower_letter = [](uint32_t c, unicode_cpt_flags f) {
            return f.is_letter && !(c >= 'a' && c <= 'z');
        };
        auto is_not_upper_letter = [](uint32_t c, unicode_cpt_flags f) {
            return f.is_letter && !(c >= 'A' && c <= 'Z');
        };
        ...
        for (size_t pos = offset_ini; pos < offset_end; /*pos++*/ ) {
            ...
            size_t p = pos;
            size_t prefix_len = 0;
            if (p < offset_end && is_not_rn_L_N(_get_cpt(p), _get_flags(p))) {
                prefix_len = 1;
            }

            // Pattern 1: [^\r\n\p{L}\p{N}]?((?=[\p{L}])([^a-z]))*((?=[\p{L}])([^A-Z]))+
            {
                size_t base_p = p + prefix_len;
                size_t a_ptr = base_p;
                while (a_ptr < offset_end && is_not_lower_letter(_get_cpt(a_ptr), _get_flags(a_ptr))) a_ptr++;
                
                bool alt1_matched = false;
                size_t match_len = 0;
                for (size_t a_end = a_ptr; a_end >= base_p; a_end--) {
                    size_t b_ptr = a_end;
                    while (b_ptr < offset_end && is_not_upper_letter(_get_cpt(b_ptr), _get_flags(b_ptr))) b_ptr++;
                    if (b_ptr > a_end) {
                        match_len = b_ptr - pos;
                        alt1_matched = true;
                        break;
                    }
                    if (a_end == base_p) break; // prevent underflow
                }
                if (alt1_matched) {
                    pos += match_len;
                    _add_token(pos);
            ...
            {
                size_t p4 = pos;
                if (p4 < offset_end && _get_cpt(p4) == ' ') p4++;
                
                size_t symbols_start = p4;
                while (p4 < offset_end) {
                    unicode_cpt_flags f = _get_flags(p4);
                    if (!f.is_whitespace && !f.is_letter && !f.is_number) {
                        p4++;
                    } else {
                        break;
                    }
...
static std::vector<size_t> unicode_regex_split_custom(const std::string & text, const std::string & regex_expr, const std::vector<size_t> & offsets) {
    ...
    if (regex_expr == "'s|'t|'re|'ve|'m|'ll|'d| ?\\p{L}+| ?\\p{N}+| ?[^\\s\\p{L}\\p{N}]+|\\s+(?!\\S)") {
        ...
        bpe_offsets = unicode_regex_split_custom_tekken(text, offsets);
    }
