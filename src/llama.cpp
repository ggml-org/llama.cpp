...
// Load model from file descriptor (Android SAF support)
struct llama_model * llama_model_load_from_fd(
        int fd,
        size_t file_size,
        struct llama_model_params params) {
#ifdef _WIN32
    LLAMA_LOG_ERROR("%s: fd-based loading not supported on Windows\n", __func__);
    GGML_UNUSED(fd);
    ...
    if (!params.vocab_only && ggml_backend_reg_count() == 0) {
        LLAMA_LOG_ERROR("%s: no backends loaded\n", __func__);
        return nullptr;
    }
    ...
    // device selection (same as file-based loading)
    if (params.devices) {
        for (ggml_backend_dev_t * dev = params.devices; *dev; ++dev) {
            model->devices.push_back(*dev);
        }
    ...
    if (params.split_mode == LLAMA_SPLIT_MODE_NONE) {
        if (params.main_gpu < 0) {
            ...
            LLAMA_LOG_ERROR("%s: invalid main_gpu: %d\n", __func__, params.main_gpu);
            llama_model_free(model);
            return nullptr;
        } else {
            ggml_backend_dev_t main_gpu = model->devices[params.main_gpu];
    ...
    int status = 0;
    try {
        // FD-based model loader - does not support mmap or split models
        llama_model_loader ml(fd, file_size, false, params.check_tensors, params.no_alloc, params.kv_overrides, params.tensor_buft_overrides);
