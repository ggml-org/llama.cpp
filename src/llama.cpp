...
#ifdef GGML_USE_SYCL
    // For MoE models on multi-GPU SYCL systems, expose only the primary GPU
    // to the backend scheduler. The SYCL backend handles multi-device MoE
    // expert dispatch internally. If the scheduler sees multiple SYCL devices,
    // it will split the graph via layer-split, which removes MUL_MAT_ID nodes
    // from device 0's subgraph and breaks MoE expert parallelism.
    // Auto-enabled for MoE models (n_expert > 0). Opt-out: GGML_SYCL_MOE_MULTI_GPU=0.
    if (model->hparams.n_expert > 0 && ggml_backend_sycl_moe_multi_gpu_requested() && model->devices.size() > 1) {
