...
static int llama_model_load(struct gguf_context * metadata, const std::string & fname, std::vector<std::string> & splits, llama_model & model, llama_model_params & params) {
    ...
    try {
        llama_model_loader ml(metadata, fname, splits, params.use_mmap, params.use_direct_io, params.check_tensors, params.no_alloc, params.kv_overrides, params.tensor_buft_overrides);
...
static struct llama_model * llama_model_load_from_file_impl(
    ...
    const int status = llama_model_load(metadata, path_model, splits, *model, params);
...
struct llama_model * llama_model_empty(
        struct gguf_context * metadata,
        struct llama_model_params params) {
    GGML_ASSERT(metadata != nullptr);
    std::string path_model;
    std::vector<std::string> splits = {};
    return llama_model_load_from_file_impl(metadata, path_model, splits, params);
}
// deprecated
...
struct llama_model * llama_model_load_from_file(
        ...
        struct llama_model_params params) {
    std::vector<std::string> splits = {};
    return llama_model_load_from_file_impl(nullptr, path_model, splits, params);
}
...
struct llama_model * llama_model_load_from_splits(
    ...
    return llama_model_load_from_file_impl(nullptr, splits.front(), splits, params);
}
