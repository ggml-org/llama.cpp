...
void llama_model::load_hparams(llama_model_loader & ml) {
    ...
    // Load layers_block_type if present
    // This is an array of strings, one for each layer, indicating the block type ("attention" or "mamba")
    ...
    if (hparams.n_layer > 0) {
        ...
        if (k_idx != -1 && gguf_get_kv_type(ctx, k_idx) == GGUF_TYPE_ARRAY &&
            ...
            // If the key is not found or not an array of strings, fill with a default type.
            std::string default_block_type = "attention";  // Default for Plamo if not specified
            if (arch == LLM_ARCH_PLAMO2) {
                // For Plamo2, if this KV is missing, it implies a non-hybrid (e.g. all attention) model or an older GGUF.
