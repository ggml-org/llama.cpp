...
static buft_list_t make_gpu_buft_list(ggml_backend_dev_t dev, llama_split_mode split_mode, const float * tensor_split) {
    ...
    if (split_mode == LLAMA_SPLIT_MODE_TENSOR_PARALLEL) {
        ggml_backend_reg_t reg = ggml_backend_dev_backend_reg(dev);
        auto               ggml_backend_tp_buffer_type_fn =
            (ggml_backend_tp_buffer_type_t) ggml_backend_reg_get_proc_address(reg, "ggml_backend_tp_buffer_type");
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne,
            ...
            bool is_expert_tensor = (tn_tensor == LLM_TENSOR_FFN_GATE_EXPS || tn_tensor == LLM_TENSOR_FFN_DOWN_EXPS ||
                                     tn_tensor == LLM_TENSOR_FFN_UP_EXPS || tn_tensor == LLM_TENSOR_FFN_NORM_EXPS ||
                                     tn_tensor == LLM_TENSOR_FFN_GATE_CHEXPS ||
                                     tn_tensor == LLM_TENSOR_FFN_DOWN_CHEXPS || tn_tensor == LLM_TENSOR_FFN_UP_CHEXPS);
            ...
            if (lazy_moe_debug_once && is_expert_tensor) {
                LLAMA_LOG_DEBUG("lazy_moe check: lazy_moe=%d is_expert=%d layer=%d tn=%s\n", (int) params.lazy_moe,
                                (int) is_expert_tensor, (int) info.layer, tn.str().c_str());
            ...
            if (!sycl_buft_logged) {
                ...
                if (dbg && std::string(dbg) == "1") {
                    ...
                    LLAMA_LOG_INFO(
                        "weight buft: tensor=%s op=%s layer_dev=%s backend=%s prefer_host=%d mmap=%d "
                        "allow_host_mmap=%d buft=%s host=%d\n",
                        tn.str().c_str(), ggml_op_name(op), ggml_backend_dev_name(layer_dev), backend,
            ...
            auto register_weight_usage = [&](ggml_tensor * tensor) {
                ...
                ggml_backend_sycl_register_weight_usage(ggml_get_name(tensor), usage_from_tensor(tn_tensor));
#else
                GGML_UNUSED(tensor);
#endif
            };
            ...
            case LLM_ARCH_MISTRAL3:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        layer.wq = create_tensor(tn(LLM_TENSOR_ATTN_Q, "weight", i),
                                                 { n_embd, tp_n_embd_head_k_x_n_head }, TP_COL);
                        layer.wk =
                            create_tensor(tn(LLM_TENSOR_ATTN_K, "weight", i), { n_embd, tp_n_embd_k_gqa }, TP_COL);
                        layer.wv =
                            create_tensor(tn(LLM_TENSOR_ATTN_V, "weight", i), { n_embd, tp_n_embd_v_gqa }, TP_COL);
                        layer.wo = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i),
                                                 { tp_n_embd_head_k_x_n_head, n_embd }, TP_ROW);

                        // optional bias tensors (also TP-sharded for Q/K/V)
                        layer.bq = create_tensor(tn(LLM_TENSOR_ATTN_Q, "bias", i), { tp_n_embd_head_k_x_n_head },
                                                 TENSOR_NOT_REQUIRED | TP_COL);
                        layer.bk = create_tensor(tn(LLM_TENSOR_ATTN_K, "bias", i), { tp_n_embd_gqa },
                                                 TENSOR_NOT_REQUIRED | TP_COL);
                        layer.bv = create_tensor(tn(LLM_TENSOR_ATTN_V, "bias", i), { tp_n_embd_gqa },
                                                 TENSOR_NOT_REQUIRED | TP_COL);
                        ...
                        if (n_expert == 0) {
                            ...
                            layer.ffn_gate =
                                create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), { n_embd, tp_n_ff }, TP_COL);
                            layer.ffn_down =
                                create_tensor(tn(LLM_TENSOR_FFN_DOWN, "weight", i), { tp_n_ff, n_embd }, TP_ROW);
                            layer.ffn_up =
                                create_tensor(tn(LLM_TENSOR_FFN_UP, "weight", i), { n_embd, tp_n_ff }, TP_COL);

                            // optional MLP bias (TP-sharded for gate/up)
                            layer.ffn_gate_b = create_tensor(tn(LLM_TENSOR_FFN_GATE, "bias", i), { tp_n_ff },
                                                             TENSOR_NOT_REQUIRED | TP_COL);
                            layer.ffn_down_b =
                                create_tensor(tn(LLM_TENSOR_FFN_DOWN, "bias", i), { n_embd }, TENSOR_NOT_REQUIRED);
                            layer.ffn_up_b = create_tensor(tn(LLM_TENSOR_FFN_UP, "bias", i), { tp_n_ff },
                                                           TENSOR_NOT_REQUIRED | TP_COL);
                        } else {
                            layer.ffn_gate_inp =
        ...
        default:
            {
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
                        ...
                        res = new llama_kv_cache(*this, params.type_k, params.type_v, !cparams.flash_attn,
                                                 cparams.offload_kqv, cparams.kv_unified, cparams.n_ctx_seq,
                                                 cparams.n_seq_max, 1, hparams.n_swa, hparams.swa_type, nullptr,
                                                 nullptr, params.tp_world_size, params.paged_layout);
                    }
                }
            }
    }
