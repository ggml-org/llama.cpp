...
struct ggml_backend_meta_split_state llama_meta_device_get_split_state(const struct ggml_tensor * tensor, void * userdata) {
    ...
    if (split_state.axis >= 0 && split_state.axis < GGML_MAX_DIMS) {
        ...
        float share_sum = 0.0;
        float smallest_share = 1.0/ud->n_devices;
        ...
        for (size_t j = 0; share_sum > 0.0 && j < ud->n_devices; j++) {
            ...
            if (tensor_split_share[j] < smallest_share) {
                smallest_share = tensor_split_share[j];
            }
        }
        ...
        for (size_t is = 0; is < segments.size(); is++) {
            ...
            for (size_t m = 0; m <= tc.il_eff; m++) {
                int64_t all_slices = smallest_share * n_q;
                std::fill(segment_slices.begin(), segment_slices.end(), all_slices);
                shares_allocated += ud->n_devices * all_slices;
                for (size_t k = 0; k < ud->n_devices; k++) {
                    device_slices[k] += smallest_share * n_q;
                }
                for (int64_t q = ud->n_devices * all_slices; q < n_q; q++) {
                    shares_allocated++;
                    size_t best          = 0;
                    ...
                    for (size_t j = 0; j < ud->n_devices; j++) {
                        ...
                        if (priority > best_priority && (float)device_slices[j] + 1 <= max_slices) {
