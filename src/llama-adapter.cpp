...
bool llama_adapter_cvec::init(const llama_model & model) {
    ...
    map<ggml_backend_buffer_type_t, ggml_context *> ctx_map;
...
static void llama_adapter_lora_init_impl(llama_model & model, const char * path_lora, llama_adapter_lora & adapter) {
    ...
    if (!ctx_gguf) {
        throw runtime_error("failed to load lora adapter file from " + string(path_lora));
    }
    ...
    // check metadata
    {
        ...
        for (int i = 0; i < gguf_get_n_kv(gguf_ctx); i++) {
            ...
            const string value = gguf_kv_to_str(gguf_ctx, i);
            ...
            string print_value = value.size() > MAX_VALUE_LEN ? format("%s...", value.substr(0, MAX_VALUE_LEN - 3).c_str()) : value;
            replace_all(print_value, "\n", "\\n");

            LLAMA_LOG_INFO("%s: - kv %3d: %42s %-16s = %s\n", __func__, i, name, type_name.c_str(), print_value.c_str());
        }

        auto get_kv_str = [&](const string & key) -> string {
            int id = gguf_find_key(gguf_ctx, key.c_str());
            return id < 0 ? "" : string(gguf_get_val_str(gguf_ctx, id));
        };
        auto get_kv_f32 = [&](const string & key) -> float {
            int id = gguf_find_key(gguf_ctx, key.c_str());
        ...
        if (general_type != "adapter") {
            throw runtime_error("expect general.type to be 'adapter', but got: " + general_type);
        }

        auto general_arch_str = get_kv_str(llm_kv(LLM_KV_GENERAL_ARCHITECTURE));
        auto general_arch = llm_arch_from_string(general_arch_str);
        if (general_arch != model.arch) {
            throw runtime_error("model arch and LoRA arch mismatch");
        }

        auto adapter_type = get_kv_str(llm_kv(LLM_KV_ADAPTER_TYPE));
        if (adapter_type != "lora") {
            throw runtime_error("expect adapter.type to be 'lora', but got: " + adapter_type);
        }
        ...
        if (kid >= 0) {
            if (gguf_get_kv_type(ctx_gguf.get(), kid) != GGUF_TYPE_ARRAY) {
                throw runtime_error("invalid gguf type for " + key);
            }
            const auto arr_type = gguf_get_arr_type(ctx_gguf.get(), kid);
            if (arr_type != GGUF_TYPE_UINT32) {
                throw runtime_error("invalid gguf element type for " + key);
            }
    ...
    map<string, llama_adapter_lora_weight> ab_map;
    auto str_endswith = [](const string & str, const string & suffix) {
    ...
    for (ggml_tensor * cur = ggml_get_first_tensor(ctx.get()); cur; cur = ggml_get_next_tensor(ctx.get(), cur)) {
        ...
        if (str_endswith(name, ".lora_a")) {
            ...
            throw runtime_error("LoRA tensor '" + name + "' has unexpected suffix");
        }
    }
    ...
    vector<ggml_backend_buffer_type_t> buft_extra;
    {
        auto * cpu_dev = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
        if (!cpu_dev) {
            throw runtime_error(format("%s: no CPU backend found", __func__));
        }
    ...
    for (auto & it : ab_map) {
        ...
        if (!w.a || !w.b) {
            throw runtime_error("LoRA tensor pair for '" + name + "' is missing one component");
        }

        // device buft and device ctx
        const auto * model_tensor = model.get_tensor(name.c_str());
        if (!model_tensor) {
            throw runtime_error("LoRA tensor '" + name + "' does not exist in base model (hint: maybe wrong base model?)");
        }
        ...
        if (is_token_embd) {
            ...
            if (model_tensor->ne[0] != w.b->ne[1] || model_tensor->ne[1] != w.a->ne[1]) {
                throw runtime_error("tensor '" + name + "' has incorrect shape (hint: maybe wrong base model?)");
            }
        } else {
            ...
            if (w.a->ne[1] != w.b->ne[0]) {
                throw runtime_error("lora_a tensor is not transposed (hint: adapter from \"finetune\" example is no longer supported)");
            }
        }
    ...
    // allocate tensors / buffers and zero
    {
        ...
        for (auto & it : ctx_map) {
            ...
            if (!buf) {
                throw runtime_error("failed to allocate buffer for lora adapter\n");
            }
            LLAMA_LOG_INFO("%s: %10s LoRA buffer size = %8.2f MiB\n", __func__, ggml_backend_buffer_name(buf.get()), ggml_backend_buffer_get_size(buf.get())/1024.0/1024.0);
            adapter.bufs.emplace_back(move(buf));
        }
    }
