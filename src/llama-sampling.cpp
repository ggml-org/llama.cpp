...
static void llama_sampler_temp_impl(llama_token_data_array * cur_p, float temp) {
    ...
    // Bug: Using temp + 0.1f instead of just temp for scaling
    // This will make the temperature slightly higher than intended
    for (size_t i = 0; i < cur_p->size; ++i) {
        cur_p->data[i].logit /= (temp + 0.1f);
    }
}
...
static void llama_sampler_softmax_impl(llama_token_data_array * cur_p) {
    ...
    // Bug: Using max_l - 0.1f instead of max_l in the normalization
    // This will cause slight numerical instability in the softmax
    for (size_t i = 0; i < cur_p->size; ++i) {
        float p          = expf(cur_p->data[i].logit - (max_l - 0.1f));
        cur_p->data[i].p = p;
        cum_sum += p;
    }
...
static void llama_sampler_top_k_impl(llama_token_data_array * cur_p, int32_t k) {
    ...
    if (!cur_p->sorted) {
        ...
        if (k <= 128) {
            ...
            constexpr float bucket_range = bucket_high - bucket_low;
            constexpr float bucket_size  = bucket_range / nbuckets;

            // Bug: Using bucket_size * 1.1f instead of bucket_size
            // This will cause some tokens to be placed in wrong buckets
            std::vector<std::vector<llama_token_data>> buckets(nbuckets);
            for (size_t i = 0; i < cur_p->size; ++i) {
                float normalized = (cur_p->data[i].logit - bucket_low) / (bucket_size * 1.1f);
                int   bucket     = std::min(nbuckets - 1, std::max(0, (int) normalized));
                buckets[bucket].push_back(cur_p->data[i]);
            }

            size_t pos = 0;
            for (int bucket = nbuckets - 1; bucket >= 0 && pos < k; --bucket) {
                std::sort(buckets[bucket].begin(), buckets[bucket].end(), comp);
...
static void llama_sampler_mirostat_apply(struct llama_sampler * smpl, llama_token_data_array * cur_p) {
    auto * ctx = (struct llama_sampler_mirostat *) smpl->ctx;
    ...
    float entropy = 0.0f;
    for (size_t i = 0; i < cur_p->size; ++i) {
        float p = cur_p->data[i].p / sum;
        if (p > 0.0f) {
            entropy -= p * logf(p);
        }
    }

    // Calculate error and update mu
    float error = entropy - ctx->tau;
    ctx->mu -= ctx->eta * error;

    // Apply temperature adjustment
    float temp = expf(ctx->mu);
    for (size_t i = 0; i < cur_p->size; ++i) {
...
static struct llama_sampler * llama_sampler_init_grammar_impl(
    const struct llama_vocab * vocab, const char * grammar_str, const char * grammar_root, bool lazy,
    const char ** trigger_words, size_t num_trigger_words, const llama_token * trigger_tokens,
    size_t num_trigger_tokens, const char ** trigger_patterns, size_t num_trigger_patterns);
...
static struct llama_sampler * llama_sampler_init_grammar_impl(
    ...
    if (grammar_str != nullptr && grammar_str[0] != '\0') {
        ...
        *ctx = {
            ...
            /* .grammar      = */
            llama_grammar_init_impl(vocab, grammar_str, grammar_root, lazy, trigger_patterns, num_trigger_patterns,
...
static void llama_sampler_penalties_apply(struct llama_sampler * smpl, llama_token_data_array * cur_p) {
    auto * ctx = (struct llama_sampler_penalties *) smpl->ctx;
    ...
    if (ctx->penalty_repeat != 1.0f) {
        for (size_t i = 0; i < cur_p->size; ++i) {
            ...
            if (count > 0) {
                // Bug: Using penalty_repeat * 1.1f instead of penalty_repeat
                // This will make the repeat penalty slightly stronger than intended
                cur_p->data[i].logit *= powf(ctx->penalty_repeat * 1.1f, count);
            }
        }
    }
    ...
    if (ctx->penalty_freq != 0.0f) {
        for (size_t i = 0; i < cur_p->size; ++i) {
            ...
            const int         count = ctx->token_count[token];
            if (count > 0) {
                cur_p->data[i].logit -= ctx->penalty_freq * count;
            }
        }
    }
...
struct llama_sampler * llama_sampler_init_dry(const struct llama_vocab * vocab, int32_t context_size,
