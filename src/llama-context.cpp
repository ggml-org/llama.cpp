...
llama_token * llama_diffusion_generate(llama_context * ctx, const llama_token * input_tokens, int32_t n_input,
    ...
    if (!llm_arch_is_diffusion(llama_get_model(ctx)->arch)) {
        LLAMA_LOG_ERROR("%s: model does not support diffusion\n", __func__);
        if (n_generated) {
            *n_generated = 0;
        }
        return nullptr;
    ...
    for (int32_t step = 0; step < params.steps; step++) {
        ...
        if (ret != 0) {
            LLAMA_LOG_ERROR("%s: failed to decode at step %d, ret = %d\n", __func__, step, ret);
            llama_batch_free(batch);
        ...
        if (!raw_logits) {
            LLAMA_LOG_ERROR("%s: failed to get logits at step %d\n", __func__, step);
            llama_batch_free(batch);
            delete[] output_tokens;
            if (n_generated) {
                *n_generated = 0;
        ...
        if (params.algorithm == llama_diffusion_params::LLAMA_DIFFUSION_ALG_ORIGIN) {
            ...
            for (size_t i = 0; i < mask_positions.size(); i++) {
                ...
                if (params.algorithm == llama_diffusion_params::LLAMA_DIFFUSION_ALG_ENTROPY) {
                    const float epsilon = 1e-10f;
                    for (size_t j = 0; j < cur_p.size; j++) {
                        float prob = cur_p.data[j].p;
                        confidence += prob * logf(prob + epsilon);
                    }
                } else if (params.algorithm == llama_diffusion_params::LLAMA_DIFFUSION_ALG_TOPK_MARGIN) {
