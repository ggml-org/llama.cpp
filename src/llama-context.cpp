...
llama_token llama_context::get_sampled_token_ith(int32_t idx) {
    ...
    //return resolve_token_id(idx, it->second);
    return it->second;
}
...
const llama_token * llama_context::get_sampled_token_ids_ith(int32_t idx) {
    if (idx == -1) {
...
size_t llama_context::get_sampled_logits_count(int32_t idx) const {
    if (idx == -1) {
        if (sampled_logits_map.size() == 1) {
...
size_t llama_context::get_sampled_probs_count(int32_t idx) const {
    if (idx == -1) {
        if (sampled_probs_map.size() == 1) {
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        gpu_has_sampled = !res->t_sampled_tokens.empty() || !res->t_sampled_probs.empty() || !res->t_sampled_logits.empty();
    ...
    // set output mappings
    if (n_outputs > 0 && !gpu_has_sampled) {
...
llama_token * llama_get_sampled_token_ids_ith(llama_context * ctx, int32_t i) {
    ctx->synchronize();

    return const_cast<llama_token *>(ctx->get_sampled_token_ids_ith(i));
}
