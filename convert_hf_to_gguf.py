...
def main() -> None:
    ...
    with torch.inference_mode():
        ...
        if args.mtp or args.no_mtp:
            ...
            from conversion.glm import Glm4MoeLiteModel
            if not (issubclass(model_class, _Qwen35MtpMixin) or issubclass(model_class, Step35Model) or issubclass(model_class, Glm4MoeLiteModel)):
                logger.error("--mtp / --no-mtp are only supported for Qwen3.5/3.6, Step3.5 and GLM-4.7 text variants today")
                sys.exit(1)
            if args.no_mtp:
