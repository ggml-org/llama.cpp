...
class Phi4FlashModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        max_pos_embds = self.find_hparam(["max_position_embeddings", "n_positions"])

        # FIX: Extract intermediate_size (n_ff)
        n_ff = self.find_hparam(["intermediate_size"])

        self.gguf_writer.add_embedding_length(n_embd)

        # FIX: Write the feed-forward length to the GGUF so llama.cpp knows it's 20480 and not 0
        self.gguf_writer.add_feed_forward_length(n_ff)
        ...
        # 2. Phi-4-Flash Specific Parameters
        layer_types = []
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # Helper to ensure tensor is transposed in actual memory layout
        def force_transpose_2d(tensor):
            """Force transpose with contiguous memory layout via numpy"""
            # Convert to numpy (forces materialization)
            np_array = tensor.cpu().numpy()
            # Transpose in numpy (creates new array with transposed data)
            np_transposed = np.ascontiguousarray(np_array.T)
            # Convert back to torch
            return torch.from_numpy(np_transposed)

        # Map general tensors (NO transpose for embeddings/norms)
        ...
        if name == "lm_head.weight":
            return []  # Skip, tied to embed_tokens
        ...
        # Shared block normalizations (NO transpose for norms/biases)
        ...
        # MLP (TRANSPOSE all weight matrices!)
        # Check what your HF model actually has - could be gate_proj/up_proj/down_proj
        # or fc1/fc2/fc3, etc.
        if rest == "mlp.gate_up_proj.weight":
            # Combined gate+up projection [2*n_ff, n_embd]
            return [(f"blk.{bid}.ffn_up.weight", force_transpose_2d(data_torch))]
        elif rest == "mlp.up_proj.weight":
            return [(f"blk.{bid}.ffn_up.weight", force_transpose_2d(data_torch))]
        elif rest == "mlp.down_proj.weight":
            return [(f"blk.{bid}.ffn_down.weight", force_transpose_2d(data_torch))]
        # Alternative naming
        elif rest == "mlp.fc1.weight":
            # Keep combined [2*n_ff, n_embd] — LLM_FFN_SWIGLU splits it internally
            # No force_transpose_2d — ggml reversal gives ne=[n_embd, 2*n_ff]
            return [(f"blk.{bid}.ffn_up.weight", data_torch)]
        elif rest == "mlp.fc2.weight":
            # FIX: Remove force_transpose_2d!
        ...
        if layer_type == 0:
            if rest == "attn.in_proj.weight":
                # Remove force_transpose_2d — ggml dimension reversal gives correct ne=[2560,10240]
                return [(f"blk.{bid}.ssm_in.weight", data_torch)]

            if rest == "attn.conv1d.weight":
                # print(f"Layer {bid}: conv1d.weight input shape: {data_torch.shape}")

                # PyTorch shape is [5120, 1, 4].
                # Remove the channel dimension (dim 1) to get [5120, 4]
                tensor = data_torch.squeeze(1)

                # DO NOT transpose! ggml automatically reverses dimensions when loading.
                # A PyTorch tensor of shape [5120, 4] will correctly become a ggml tensor
                # with dimensions ne = [4, 5120], which is exactly what llama.cpp expects.

                # Just ensure it's contiguous in memory before returning
                if not tensor.is_contiguous():
                    tensor = tensor.contiguous()

                # print(f"Layer {bid}: final shape: {tensor.shape}")
            ...
            if rest == "attn.x_proj.weight":
                # Remove force_transpose_2d — gives ne=[5120,192]
                return [(f"blk.{bid}.ssm_x.weight", data_torch)]

            if rest == "attn.dt_proj.weight":
                # Remove force_transpose_2d — gives ne=[160,5120]
            ...
            if rest == "attn.A_log":
                ...
                # HF: [d_inner, d_state] -> llama.cpp: [d_state, d_inner]
            ...
            if rest == "attn.out_proj.weight":
                # Remove force_transpose_2d — gives ne=[5120,2560]
        ...
        # ==================================================
            ...
            if rest == "attn.in_proj.weight":
                # HF: [d_inner, n_embd] -> llama.cpp: [n_embd, d_inner]
                return [(f"blk.{bid}.gmu_in.weight", data_torch)]

            if rest == "attn.out_proj.weight":
                # HF: [n_embd, d_inner] -> llama.cpp: [d_inner, n_embd]
        ...
        # ==================================================
            ...
            if rest == "attn.Wqkv.weight":
                ...
                if w.shape[0] == q_size:
                    # Cross-attention layers (type 4): Q only, no K/V block
                    result = reorder(w, n_q_heads)
                else:
                    # SWA (type 1) and full-attn (type 2): Q | K | V
                    q_block = w[:q_size]
                    k_block = w[q_size : q_size + k_size]
                    v_block = w[q_size + k_size:]
                    result = torch.cat([
                        reorder(q_block, n_q_heads),
                        reorder(k_block, n_kv_heads),
            ...
            if rest == "attn.Wqkv.bias":
                ...
                def reorder_bias(block, n_heads):
            ...
            # Lambda parameters (1D vectors, no transpose)
            ...
            # SubLN (1D, no transpose)
            if rest == "attn.inner_cross_attn.subln.weight":
                return [(f"blk.{bid}.attn_subln.weight", data_torch)]

            # Output projection
            if rest == "attn.out_proj.weight":
                # HF: [n_embd, n_embd] -> llama.cpp: [n_embd, n_embd] (square, but still transpose!)
