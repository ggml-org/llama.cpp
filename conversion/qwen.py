...
class _QwenMtpMixin:
    ...
    # any mtp.* tensor at all, so "no MTP block" can be told from "MTP block not counted"
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.no_mtp:
            ...
            # Qwen-3-Next doesn't include `mtp_num_hidden_layers` in config, so fall back to the
            # count of mtp.* tensors. That count is also 0 when the checkpoint has no MTP block,
            # which is not an error - only an explicit --mtp export needs one
            if n_mtp == 0:
                n_mtp = self.opt_num_mtp_layers
            if n_mtp == 0 and self.saw_mtp_tensor:
                raise ValueError("the checkpoint carries mtp.* tensors but none of them name a "
                                 "layer, so the MTP block count is unknown")
