...
common_speculative * common_speculative_init(common_params_speculative & params, uint32_t n_seq) {
    ...
    if (has_det_filter) {
        ...
        // Set vocabulary for the plugin - required for bitmask-capable plugins
        {
            ...
            if (!params.deterministic_draft.det_accept_all &&
                ...
                llama_deterministic_draft_free(result->det_filter.plugin);
                delete result;
                return nullptr;
            }
...
llama_tokens common_speculative_sample_and_accept(
    ...
    if (spec->det_filter.plugin) {
        ...
        // constraint: the draft was already filter-passed during
        // common_speculative_draft(), and the final token (rejection
        // correction or bonus) is constrained here too. Emitting
        // it unconstrained would void the "filter verifies all emitted
        // tokens" contract and desync the constraint state from the output.
        llama_tokens result;
        result.reserve(draft.size() + 1);
        ...
        // Roll the filter back to the accepted draft prefix before
