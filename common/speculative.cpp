...
common_speculative * common_speculative_init(common_params_speculative & params, uint32_t n_seq) {
    ...
    std::vector<common_speculative_config> configs = {}; // list of speculative configs to try
    {
        ...
        bool has_ngram_suffix  = (enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_NGRAM_SUFFIX));
        ...
        if (has_ngram_suffix) {
            // model-free suffix-tree drafter (suffix decoding).
            configs.push_back(common_speculative_config(COMMON_SPECULATIVE_TYPE_NGRAM_SUFFIX, params));
        }
        if (has_draft_simple) {
