...
struct common_speculative_state {
    ...
    int64_t gen_duration_ms        = 0; // total time spent in this implementation in milliseconds.
...
llama_tokens common_speculative_gen_draft(
    ...
    for (auto & impl : spec->impls) {
        ...
        const int64_t t_start_ms = impl->gen_perf ? ggml_time_ms() : 0;
        ...
        const int64_t t_now_ms = impl->gen_perf ? ggml_time_ms() : 0;
        impl->gen_duration_ms += t_now_ms - t_start_ms; // accumulate duration for this implementation
...
void common_speculative_print_stats(const struct common_speculative * spec) {
    ...
    for (const auto & impl : spec->impls) {
        // std::string via impl->gen_duration_ms (if >0, else "")
        std::string performance = impl->gen_perf
            ? ", dur = " + std::to_string(impl->gen_duration_ms) + " ms"
            : "";
