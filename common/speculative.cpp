...
struct det_draft_filter {
    ...
    uint64_t n_target_rejected_total = 0;
};
...
common_speculative * common_speculative_init(common_params_speculative & params, uint32_t n_seq) {
    ...
    bool has_compatible_type = false;
    bool has_det_filter = false;
    {
        ...
        has_compatible_type = ((enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_DRAFT_MTP)) && params.draft.ctx_dft != nullptr)
            || ((enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_DRAFT_EAGLE3)) && params.draft.ctx_dft != nullptr)
            || ((enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_DRAFT_DSPARK)) && params.draft.ctx_dft != nullptr)
            || ((enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH)) && params.draft.ctx_dft != nullptr);
    ...
    if (has_det_filter) {
        if (!has_compatible_type) {
            LOG_ERR("%s: deterministic draft requires a compatible drafter (draft-mtp, draft-eagle3, draft-dspark, or draft-dflash)\n", __func__);
            return nullptr;
        }
        ...
        // Set vocabulary for the plugin - required for bitmask-capable plugins
        {
            ...
            // rollback is load-bearing in default (target-verified) mode:
            // partial accepts must rewind the filter. Accept-all never
            // verifies, so it stays optional there. A zero-token rollback is
            // a no-op success for any plugin that implements the method.
            if (!params.deterministic_draft.det_intercept_all &&
                !llama_deterministic_draft_rollback(result->det_filter.plugin, 0, 0)) {
                LOG_ERR("%s: deterministic draft plugin does not implement rollback (required in default mode)\n", __func__);
                return nullptr;
            }

            // the bitmask is the load-bearing capability - a plugin without
            // it would silently pass or truncate every draft
            const uint32_t det_caps = llama_deterministic_draft_get_capabilities(result->det_filter.plugin);
            if ((det_caps & LLAMA_DETERMINISTIC_DRAFT_CAPABILITY_BITMASK) == 0) {
                LOG_ERR("%s: deterministic draft plugin does not advertise CAPABILITY_BITMASK\n", __func__);
                return nullptr;
            }
        }

        result->det_filter.n_max = params.deterministic_draft.n_max;
        result->det_filter.det_accept_all = params.deterministic_draft.det_intercept_all;
        result->det_filter.det_filter_bonus = params.deterministic_draft.det_filter_bonus;
...
void common_speculative_draft(common_speculative * spec) {
    ...
    // track which sequences produced a fresh draft this round - a replaying
    // sequence (partial accept) re-enters with a stale non-empty result and
    // must not be re-filtered (commit-on-accept would double-commit)
    std::vector<bool> drafted_fresh(dparams.size(), false);
    ...
    if (spec->det_filter.plugin) {
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) dparams.size(); ++seq_id) {
            ...
            // reset every round: a filter result is valid for exactly one
            // verification round, a stale valid_count would drive a bogus
            // rollback after an empty/skipped draft
            auto & fr = spec->det_filter.last_result[seq_id];
            fr = common_det_filter_result{};

            if (!drafted_fresh[seq_id]) {
                continue;
            }
            ...
            // filter-invalid one (commit-on-accept)
...
void common_speculative_accept(common_speculative * spec, llama_seq_id seq_id, uint16_t n_accepted) {
    ...
    GGML_ASSERT(impl != nullptr || n_accepted == 0);
    if (!impl) {
        return;
    }
...
void common_speculative_print_stats(const common_speculative * spec) {
    ...
    if (spec->det_filter.plugin) {
        LOG_INF("statistics %16s: #drafts = %d, #truncated = %d, #tokens pre = %d, #tokens post = %d, #target rejected = %llu\n",
                "draft-deterministic",
                spec->det_filter.n_drafts_total,
                spec->det_filter.n_truncated_total,
                spec->det_filter.n_tokens_pre_filter,
                spec->det_filter.n_tokens_post_filter,
                (unsigned long long) spec->det_filter.n_target_rejected_total);
    }
}
...
// Sample a single filter-constrained token from the target distribution at
// batch index idx. Used for emitted tokens that are not part of the already
// filter-passed draft: the accept-all bonus, and the standard-mode
// correction/bonus token. The returned token is committed to the filter
// here (filter_draft commits on accept), so common_speculative_accept()
// must not commit it again.
// Returns LLAMA_TOKEN_NULL when the constraint reached a complete terminal
// state (a legitimate end state, not an error - do not reset the matcher,
// that would desync it from the generated content) or when no valid token
// exists.
static llama_token common_speculative_sample_det_token(
    ...
    // filter checks and take the highest-probability filter-valid
    // token. This avoids the O(vocab) full bitmask fill, which can cost
    // ~1s+ per step with a large constraint set.
    common_sampler_sample(smpl, ctx, idx);
    const auto * cur_p = common_sampler_get_candidates(smpl, true);
    for (int k = 0; k < (int) cur_p->size; k++) {
        llama_token cand = cur_p->data[k].id;
        // EOG must always pass so the sequence can terminate even under the
        // constraint (filter_draft rejects it - the grammar only matches
        // content tokens, not end-of-generation).
        if (llama_vocab_is_eog(vocab, cand) ||
    ...
    // No shortlist candidate was filter-valid: fall back to the
    ...
    // Preserve EOG logits across the filter mask so the sequence can always
    // terminate (apply_bitmask rewrites masked logits to -1e30f).
    std::vector<int>   eog_tokens;
    std::vector<float> eog_logits;
    for (int i = 0; i < vocab_size; i++) {
        if (llama_vocab_is_eog(vocab, i)) {
            eog_tokens.push_back(i);
            eog_logits.push_back(logits[i]);
        }
    }
    ...
    for (size_t j = 0; j < eog_tokens.size(); j++) {
        logits[eog_tokens[j]] = eog_logits[j];
    }
...
// only the bonus token is constrained via the det filter
static llama_tokens common_speculative_sample_and_accept_all(
    ...
    // distribution - it is NOT part of the already filter-passed
    // draft, so it must be constrained here too. Without this,
    // it would silently bypass the "filter is the sole verifier"
    // guarantee that accept-all mode is supposed to provide.
    // Note this also covers the empty-draft case (the filter rejected the
    // whole draft): the single emitted token is still constrained
...
llama_tokens common_speculative_sample_and_accept(
    ...
    if (spec->det_filter.plugin) {
        // Standard target verification: the draft head was already
        // filter-passed during common_speculative_draft(). The final token
        // (rejection correction or bonus) is the target's own unconstrained
        // sample unless det_filter_bonus constrains it through the filter.
        llama_tokens result;
        result.reserve(draft.size() + 1);
        ...
        // Roll the filter back to the accepted draft prefix - it still holds
        // all drafts committed during common_speculative_draft()'s filter step.
        const auto & fr = spec->det_filter.last_result[seq_id];
        // n_max == 0 skips the filter and leaves valid_count == 0; guard
        // the subtraction so the uint64 counter does not wrap
        if (fr.valid_count > (int) i) {
            spec->det_filter.n_target_rejected_total += (uint64_t) (fr.valid_count - (int) i);
        }
        if ((int) i < fr.valid_count) {
            if (!llama_deterministic_draft_rollback(
                    spec->det_filter.plugin, seq_id, fr.valid_count - (int) i)) {
                LOG_ERR("%s: failed to rollback filter state for seq_id %d, resetting slot\n", __func__, seq_id);
                llama_deterministic_draft_reset(spec->det_filter.plugin, seq_id);
            }
        }

        // consumed: a filter result applies to one verification round only -
        // clearing it here keeps a later empty-draft round from rolling back
        // stale state
        spec->det_filter.last_result[seq_id] = common_det_filter_result{};

        if (spec->det_filter.det_filter_bonus) {
            // det_token commits+accepts internally; NULL means the constraint
            // reached a terminal state, so emit nothing (mirrors accept-all)
            ...
            const llama_token final_tok = common_sampler_sample(smpl, ctx, idxs[i]);
            common_sampler_accept(smpl, final_tok, true);
            result.push_back(final_tok);
        }
