...
class common_peg_parser_builder {
  ...
  public:
    ...
    // Wraps a child parser but emits nothing in GBNF. Shorthand for gbnf(p, "").
    common_peg_parser silent(const common_peg_parser & p) { return add(common_peg_gbnf_parser{p, ""}); }
