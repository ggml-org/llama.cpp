...
// generated text and the tokens that generated it, where token_pos[i] is where the piece of tokens[i] starts in text
// a token that renders nothing has the same position as the next one, and tokens cut off by truncating the text
// are left in place, so a piece runs from its position to the next position or the end of the text
struct common_peg_input {
    ...
    // append a chunk of text generated by chunk_tokens, with chunk_pos relative to the chunk
    void append(const std::string & chunk, const std::vector<llama_token> & chunk_tokens, const std::vector<size_t> & chunk_pos);
