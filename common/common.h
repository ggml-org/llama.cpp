...
// Grammar variants — at most one can be active at a time
struct common_grammar_user          { std::string value; };  // user-provided GBNF (--grammar / "grammar" API field)
struct common_grammar_output_format { std::string value; };  // auto-generated from JSON schema (--json-schema / "json_schema" API field)
struct common_grammar_tool_calls    { std::string value; };  // auto-generated by chat template parser for function calling

using common_grammar_variant = std::variant<
    std::monostate,
    common_grammar_user,
    common_grammar_output_format,
    common_grammar_tool_calls
>;
...
inline const std::string & common_grammar_value(const common_grammar_variant & g) {
    ...
    return std::visit([&](const auto & v) -> const std::string & {
...
inline bool common_grammar_needs_prefill(const common_grammar_variant & g) {
    return std::holds_alternative<common_grammar_output_format>(g)
        || std::holds_alternative<common_grammar_tool_calls>(g);
}
