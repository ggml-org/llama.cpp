...
struct common_multiconn_progress {
    ...
    long long                             total_content_length;
    std::vector<long long>                chunk_downloaded_bytes;
    std::vector<long long>                initial_downloaded_bytes;
    std::chrono::steady_clock::time_point start_time;
    bool                                  progress_enabled;
    bool                                  printed;

    common_multiconn_progress(long long content_length, int num_chunks, bool enable_progress) :
    ...
    void update_chunk_progress(int chunk_idx, long long downloaded_bytes) {
        if (!progress_enabled) {
            return;
        }
        ...
        for (long long bytes : chunk_downloaded_bytes) {
  ...
  private:
    void display_progress(long long total_downloaded) {
        ...
        const long long percentage = (total_downloaded * 100) / total_content_length;
...
static bool common_download_file_multiconn(const std::string & url,
    ...
    const long long min_chunk_size = 2 * 1024 * 1024;
    const long long min_file_size  = min_chunk_size * 2;  // Minimum file size to enable multi-connection
    ...
    const long long chunk_size = content_length / num_connections;
    ...
    for (int i = 0; i < num_connections; ++i) {
        long long start = static_cast<long long>(i) * (content_length / num_connections);
        long long end;
    ...
    for (size_t i = 0; i < chunks.size(); ++i) {
        futures.push_back(std::async(
            ...
            [&](size_t chunk_idx) -> bool {
                ...
                long long actual_start = chunk.start_byte + chunk.downloaded_bytes;
