...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        "- interleave: interleave allocations across all NUMA nodes (requires >=2 nodes)\n"
        "- ccd: pin threads to CCD-local cores, physical cores before SMT (EPYC)\n"
        ...
        [](common_params & params, const std::string & value) {
            ...
            else if (value == "ccd") { params.numa = GGML_NUMA_STRATEGY_CCD; }
            else { throw std::invalid_argument("invalid value"); }
        }
