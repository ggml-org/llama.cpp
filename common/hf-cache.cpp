...
void prune_old_files(const std::string & hf_repo,
                     const hf_cache::hf_files & current_model_files,
                     const hf_cache::hf_file & current_mmproj,
                     const hf_cache::hf_file & current_mtp) {
    ...
    if (!current_mtp.local_path.empty()) {
        files_to_keep.push_back(current_mtp.local_path);
        filenames_to_delete.push_back(path_str(fs::path(current_mtp.local_path).filename()));
        const auto & target = get_symlink_target(current_mtp.local_path);
        if (!target.empty()) {
            files_to_keep.push_back(target);
        }
    }
