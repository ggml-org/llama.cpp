...
static const std::vector<std::string> sidecar_tokens = {
    "mtp", "eagle3", "dflash", "dspark", "mmproj",
};
...
common_download_hf_plan common_download_get_hf_plan(const common_params_model & model, const common_download_opts & opts) {
    ...
    if (!model.hf_file.empty()) {
        ...
        if (primary.path.empty() && !sidecar.empty()) {
            auto keyword = sidecar == "mmproj" ? sidecar : sidecar + "-";
            auto found = find_best_sibling(all, "", keyword, base_tag);
...
std::vector<common_cached_model_info> common_list_cached_models() {
    ...
    for (const auto & f : files) {
        ...
        // cached `mtp-Model-Q4_0.gguf` shows up as `<repo>:Q4_0-mtp`; only a
        // sidecar token at the very start of the filename counts, anything else
        // is a primary model (possibly with an embedded draft)
        auto base = f.path;
...
bool common_download_remove(const std::string & hf_repo_with_tag) {
    ...
    // a sidecar tag (`Q4_0-mtp`) targets the sidecar file alone; a bare sidecar
    // tag (`mtp`) targets every cached quant of that sidecar type
    auto [base_tag, sidecar] = split_sidecar_tag(tag);
    if (!sidecar.empty()) {
        tag_upper = base_tag;
        for (char & c : tag_upper) {
    ...
    for (const auto & f : files) {
        ...
        if (!sidecar.empty()) {
            // a sidecar tag only removes files of that sidecar type; with a
            // bare sidecar tag (`mtp`) every quant matches, with `Q4_0-mtp`
            // only the file at that quant
            if (base.rfind(sidecar + "-", 0) != 0) {
                continue;
            }
            if (!base_tag.empty() && split.tag != tag_upper) {
                continue;
            }
        } else if (split.tag != tag_upper) {
            continue;
        }
