...
fusion, and eligible greedy MTP/DFlash target sampling. The individual
...
| `GGML_HIP_GFX1030_TARGET_BACKEND_SAMPLING=0` | Disables automatic target-side backend sampling for the certified greedy TP4 MTP and DFlash paths. | Debug/fallback override; unset by default. Explicit request-level `backend_sampling` still takes precedence. |
...
- Server MTP results depend on prompt content, temperature, and draft
  acceptance.
- Context checkpoints are forcibly disabled on AMD GPUs because restored Qwen
  checkpoints can crash flash attention (upstream issue #20176). This avoids GPU
  faults but long branching conversations may reprocess more prompt tokens.
