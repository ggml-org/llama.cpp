...
function(hf_download version out_var)
    ...
    foreach(entry ${urls})
        string(REGEX REPLACE "^([^|]+)\\|.*$" "\\1" label "${entry}")
        string(REGEX REPLACE "^[^|]+\\|(.*)$" "\\1" base  "${entry}")

        message(STATUS "UI: downloading from ${label}: ${base}")
        ...
        foreach(asset ${ASSETS})
            ...
            if(NOT rc EQUAL 0)
                ...
                message(STATUS "UI: download ${asset} from ${label} failed: ${errmsg}")
                set(ok FALSE)
                break()
            endif()
...
function(emit_files)
    ...
    string(APPEND h "    std::size_t           size;\n")
    string(APPEND h "};\n\n")
    ...
    if(present)
        ...
        foreach(asset ${ASSETS})
            ...
            string(APPEND cpp "static const std::size_t   asset_${idx}_size = ${len};\n\n")
            math(EXPR idx "${idx} + 1")
        endforeach()
