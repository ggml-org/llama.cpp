...
function(emit_files)
    ...
    # Create build.json with the llama.cpp build number for UI version detection.
    # This is separate from SvelteKit's _app/version.json (which is for SW precache).
    # The UI fetches /build.json to detect server upgrades via the llama.cpp build ID.
    # build.json is generated by the vite plugin (splashScreenPlugin) during npm build.
    # CMake no longer creates it — the vite plugin writes it to dist/ if LLAMA_BUILD_NUMBER
    # is set (passed via env var from CMake -> npm -> vite).
