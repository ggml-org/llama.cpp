...
function(ui_requested_version out_var)
...
function(ui_cache_key out_var)
    ui_requested_version(version)
    set(${out_var} "${version}|${LLAMA_UI_GZIP}|${BUILD_UI}|${HF_ENABLED}|${HF_BUCKET}|${UI_CACHE_VERSION}" PARENT_SCOPE)
endfunction()

function(ui_cache_is_valid out_var)
    set(${out_var} FALSE PARENT_SCOPE)

    # npm builds are handled by npm_build_should_skip; only cache prebuilt assets.
    if(BUILD_UI)
        return()
    endif()
    if(NOT EXISTS "${STAMP_FILE}")
        message(STATUS "UI: cache file missing: ${STAMP_FILE}")
        return()
    endif()
    if(NOT EXISTS "${UI_CPP}" OR NOT EXISTS "${UI_H}")
        message(STATUS "UI: cache miss: generated files missing")
        return()
    endif()
    if(NOT EXISTS "${SRC_DIST_DIR}/index.html" AND NOT EXISTS "${DIST_DIR}/index.html")
        message(STATUS "UI: cache miss: assets missing")
        return()
    endif()
    if(LLAMA_UI_GZIP)
        if(NOT EXISTS "${UI_BINARY_DIR}/ui-gzip/_gzip/index.html")
            message(STATUS "UI: cache miss: gzip tree missing")
            return()
        endif()
    endif()

    file(READ "${STAMP_FILE}" cached)
    string(STRIP "${cached}" cached)
    message(STATUS "UI: cache file: ${STAMP_FILE}")
    message(STATUS "UI: cache contents: ${cached}")

    ui_cache_key(key)
    if(NOT "${cached}" STREQUAL "${key}")
        message(STATUS "UI: cache miss: expected [${key}]")
        return()
    endif()
...
function(ui_stamp_matches_version out_var)
    set(${out_var} FALSE PARENT_SCOPE)
    if(NOT EXISTS "${STAMP_FILE}")
        return()
    endif()
    file(READ "${STAMP_FILE}" cached)
    string(STRIP "${cached}" cached)
    string(REGEX MATCH "^[^|]+" cached_version "${cached}")
    ui_requested_version(version)
    if("${cached_version}" STREQUAL "${version}")
        set(${out_var} TRUE PARENT_SCOPE)
    endif()
endfunction()

function(ui_write_cache)
    ui_cache_key(key)
    file(WRITE "${STAMP_FILE}" "${key}")
    message(STATUS "UI: cache updated: ${STAMP_FILE}")
    message(STATUS "UI: cache contents: ${key}")
endfunction()
...
# Fast path: skip download/gzip/embed when generated files and inputs are unchanged.
# The stamp records requested version + build configuration.
ui_cache_is_valid(cache_valid)
if(cache_valid)
    message(STATUS "UI: assets up-to-date, skipping provisioning")
    return()
endif()
...
if(NOT provisioned AND HF_ENABLED)
    ...
    ui_stamp_matches_version(stamp_matches)

    set(have_assets FALSE)
    if(EXISTS "${DIST_DIR}/index.html")
        set(have_assets TRUE)
    endif()
    if(have_assets AND stamp_matches)
        message(STATUS "UI: HF stamp matches requested version, skipping HF fetch")
        set(provisioned TRUE)
    else()
        hf_download("${VERSION}" HF_OK HF_RESOLVED)
        if(HF_OK)
            message(STATUS "UI: HF download succeeded (${HF_RESOLVED})")
            set(provisioned TRUE)
        else()
