...
def make_data(qtype: gguf.GGMLQuantizationType) -> np.ndarray:
    _block_size, type_size = gguf.GGML_QUANT_SIZES[qtype]
    rng = np.random.default_rng()
    if qtype == gguf.GGMLQuantizationType.F32:
        return rng.standard_normal((2, 4)).astype(np.float32)
    if qtype == gguf.GGMLQuantizationType.F16:
        return rng.standard_normal((2, 4)).astype(np.float16)
    return rng.integers(0, 256, size=(2, type_size * 2), dtype=np.uint8)
...
def assert_equal_dequantized(actual: torch.Tensor, expected: np.ndarray) -> None:
    expected_tensor = torch.from_numpy(expected.astype(np.float32, copy=False)).to(actual.device)
