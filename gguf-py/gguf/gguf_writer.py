...
class GGUFWriter:
    ...
    def add_adapter_token_ids_activate(self, ids: Sequence[int]) -> None:
        self.add_array(Keys.LLM.ADAPTER_TOKEN_IDS_ACTIVATE.format(arch=self.arch), ids)

    def add_adapter_token_ids_substitute(self, ids: Sequence[int]) -> None:
        self.add_array(Keys.LLM.ADAPTER_TOKEN_IDS_SUBSTITUTE.format(arch=self.arch), ids)

    def add_adapter_lora_rank(self, rank: int) -> None:
        self.add_uint32(Keys.LLM.ADAPTER_LORA_RANK.format(arch=self.arch), rank)

    def add_adapter_router_gain(self, gain: float) -> None:
        self.add_float32(Keys.LLM.ADAPTER_ROUTER_GAIN.format(arch=self.arch), gain)
