...
static void usage(char ** argv) {
    ...
    LOG("  --test-prune-layers <input> <output>\n");
    LOG("                           Run the layer-pruning metadata regression test\n");
...
static int test_prune_layers_metadata(const std::string & path_inp, const std::string & path_out, const size_t seed, const float stdev) {
    const int32_t prune_layers[] = { 1, -1 };

    gguf_context_ptr gguf_ctx_inp = get_gguf_ctx(LLM_ARCH_LFM2, false, 3);
    auto model_and_ctx = get_model_and_ctx(gguf_ctx_inp.get(), nullptr, seed, stdev, {});
    ...
    if (llama_model_quantize(path_inp.c_str(), path_out.c_str(), &qparams) != 0) {
        LOG_ERR("%s: failed to prune model\n", __func__);
        return 1;
    }
    ...
    if (!gguf_ctx) {
        LOG_ERR("%s: failed to read pruned model metadata\n", __func__);
        return 1;
    }
...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "--test-prune-layers") == 0) {
            if (i + 2 < argc) {
                prune_input  = argv[++i];
                prune_output = argv[++i];
            } else {
    ...
    try {
        if (!prune_input.empty()) {
            return test_prune_layers_metadata(prune_input, prune_output, seed, stdev);
        }
        if (!out.empty()) {
