...
struct test_flash_attn_ext : public test_case {
    ...
    std::string vars() override {
        return VARS_TO_STR17(hsk, hsv, nh, nr23, kv, nb, mask, sinks, max_bias, logit_softcap, prec, type_K, type_V, permute, kv_view, v_is_view_of_k, n_kv_max) + "," + VAR_TO_STR(mask_pattern) + "," + VAR_TO_STR(mask_broadcast);
    }
    ...
    test_flash_attn_ext(int64_t hsk = 128, int64_t hsv = 128, int64_t nh = 32, std::array<int64_t, 2> nr23 = {1, 1}, int64_t kv = 96, int64_t nb = 8,
                        ...
                        bool kv_view = true, bool v_is_view_of_k = false, int64_t n_kv_max = 0, int mask_pattern = 0, bool mask_broadcast = false)
        : hsk(hsk), hsv(hsv), nh(nh), nr23(nr23), kv(kv), nb(nb), mask(mask), sinks(sinks), max_bias(max_bias), logit_softcap(logit_softcap), prec(prec),
          type_K(type_K), type_V(type_V), permute(permute), kv_view(kv_view), v_is_view_of_k(v_is_view_of_k), n_kv_max(n_kv_max), mask_pattern(mask_pattern), mask_broadcast(mask_broadcast) {}
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // Broadcast one logical mask; backing storage also covers partial query tiles.
    for (int64_t hs : {64, 128, 256}) {
        for (int64_t nb : {1, 17, 64}) {
            test_cases.emplace_back(new test_flash_attn_ext(hs, hs, 2, {8, 2}, 512, nb, true, false, 0, 0,
                                    GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, false, 0, 1, true));
        }
    }
    ...
    for (int64_t hs : {64, 128, 256}) {
        test_cases.emplace_back(new test_flash_attn_ext(hs, hs, 2, {8, 2}, 2048, 17, true, false, 0, 0,
                                GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, false, 0, 5));
    }
