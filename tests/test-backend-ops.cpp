...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    for (int64_t state_width : { 1, 3, 4 }) {
        for (int64_t ne_b_d : { 512, 1024, 2048 }) {
            for (int64_t channels : { 31, 32, 33, 64, 65, 96, 97, 128, 129 }) {
                test_cases.emplace_back(new test_concat(
                    GGML_TYPE_F32, {state_width, channels, 1, 1}, ne_b_d, 0, 16));
            }
        }
    }
    for (int64_t state_width : { 1, 3, 4 }) {
        test_cases.emplace_back(new test_concat(GGML_TYPE_F32, {state_width, 64, 2, 1}, 2048, 0, 16));
    }
    for (int64_t ne_b_d : { 512, 1024, 2048 }) {
        test_cases.emplace_back(new test_concat(GGML_TYPE_F32, {3, 8192, 1, 1}, ne_b_d, 0, 16));
    }
    for (int64_t ne_b_d : { 1, 31, 32, 511, 513, 1537 }) {
        test_cases.emplace_back(new test_concat(GGML_TYPE_F32, {3, 64, 1, 1}, ne_b_d, 0, 16));
    }
