...
bool test_compare_outputs() {
    ...
    printf("  Output stats: non_zero=%d/%zu, min=%.4f, max=%.4f, avg=%.4f\n",
           non_zero, output_data.size(), min_val, max_val, avg);
    ...
    printf("  Mode: %s\n", (disable_opt && strcmp(disable_opt, "1") == 0) ? "AoS (GGML_SYCL_DISABLE_OPT=1)" : "SoA (default)");
...
void mul_mat_vec_cpu(const float* weight, const float* input, float* output,
...
bool test_q4_0_correctness() {
    ...
    for (int block = 0; block < blocks_per_row && block < 5; block++) {
        ...
        if (block < 3) {
            printf("    block[%d]: d_off=%zu d_raw=0x%04x d_float=%.2f block_sum=%.1f\n",
                   block, d_off, d_raw, d_float, block_sum);
        }
    }
    ...
    printf("  GPU readback[0..7] = [%.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f]\n",
           input_readback[0], input_readback[1], input_readback[2], input_readback[3],
           input_readback[4], input_readback[5], input_readback[6], input_readback[7]);
    // Check raw hex
    ...
    printf("  Max diff: %.6f at row %d (GPU=%.4f, CPU=%.4f)\n",
           max_diff, max_diff_idx, gpu_output[max_diff_idx], cpu_output[max_diff_idx]);
    printf("  First 5 GPU:  [%.4f, %.4f, %.4f, %.4f, %.4f]\n",
           gpu_output[0], gpu_output[1], gpu_output[2], gpu_output[3], gpu_output[4]);
    printf("  First 5 CPU:  [%.4f, %.4f, %.4f, %.4f, %.4f]\n",
           cpu_output[0], cpu_output[1], cpu_output[2], cpu_output[3], cpu_output[4]);
...
bool test_soa_layout_verification() {
    ...
    printf("\n  tensor->extra: %s (SoA %s)\n",
           has_extra ? "set" : "NULL",
           has_extra ? "enabled" : "disabled/AoS");
    ...
    for (int blk = 0; blk < nblocks; blk++) {
        ...
        printf("    Block %d qs[0] at offset %zu: 0x%02x (expected 0x%02x) %s\n",
               blk, qs_offset, actual_qs, qs_values[blk],
               actual_qs == qs_values[blk] ? "OK" : "MISMATCH");
        if (actual_qs != qs_values[blk]) qs_correct = false;
    }
    ...
    for (int blk = 0; blk < nblocks; blk++) {
        ...
        if (actual_d != d_values[blk]) d_correct = false;
    }
    ...
    if (!has_extra) {
        printf("\n  NOTE: SoA not enabled. Run without GGML_SYCL_DISABLE_OPT to test SoA.\n");
...
bool test_row_identification() {
    ...
    for (int row = 0; row < n_rows; row++) {
        ...
        if (nibble > 15) nibble = 15;  // Cap at 15
    ...
    for (int row = 0; row < n_rows; row++) {
        int val = row + 1;
        if (val > 7) val = 7;  // Max dequant value is +7 (nibble 15 - 8)
    ...
    if (status != GGML_STATUS_SUCCESS) {
        ...
        for (int tok = 0; tok < n_tokens; tok++) {
            ...
            printf("    Token %d (row %d): expected %.1f, got %.4f %s\n",
                   tok, row_id, expected, actual, ok ? "OK" : "FAIL");
            if (!ok) pass = false;
        }
...
// Test 15: CPU GET_ROWS correctness (the actual execution path for token_embd)
...
bool test_cpu_gpu_token_embd() {
    printf("Test 15: CPU GET_ROWS correctness (token embedding lookup)\n");
    ...
    size_t total_size = ggml_backend_buft_get_alloc_size(cpu_buft, token_embd)
                      ...
                      + ggml_backend_buft_get_alloc_size(cpu_buft, embd_f32)
                      + 4096;
    ...
    printf("  Expected values after dequant: [%.1f, %.1f, %.1f]\n",
           (float)(token_ids[0] % 8), (float)(token_ids[1] % 8), (float)(token_ids[2] % 8));
...
// Test 16: Q6_K MMVQ SoA test (output.weight path)
// This tests the path used for output.weight in Mistral/LLaMA models
bool test_q6_k_mmvq_soa() {
    printf("Test 16: Q6_K MMVQ SoA (output.weight path)\n");
    ...
    if (status != GGML_STATUS_SUCCESS) {
        ...
        for (int i = 0; i < 5 && i < n_rows; i++) {
            ...
            printf("    row[%d]: %.2f (expected %.2f, err=%.2f%%) %s\n",
                   i, actual, expected, rel_error * 100.0f, ok ? "OK" : "FAIL");
        ...
        for (size_t i = 0; i < output_data.size(); i++) {
            if (std::isnan(output_data[i]) || std::isinf(output_data[i]) ||
                fabsf(output_data[i]) > expected * 10.0f) {
                if (garbage_count < 3) {
...
// Test 17: Q6_K SoA layout verification (check actual memory layout)
bool test_q6_k_soa_layout() {
    printf("Test 17: Q6_K SoA layout verification\n");
    ...
    for (int i = 0; i < nblocks && i < 2; i++) {
        ...
        printf("    block[%d].ql[0]: expected 0x%02x, got 0x%02x %s\n",
               i, expected_ql0, actual_ql0, ok ? "OK" : "FAIL");
        if (!ok) pass = false;
    }
    ...
    printf("    Interpreting as AoS: block[0].d = 0x%04x %s\n",
           aos_d0, is_aos ? "(matches AoS - REORDER NOT WORKING!)" : "(different - SoA working)");
    if (is_aos) {
...
int main() {
    ...
    const char* disable_opt = getenv("GGML_SYCL_DISABLE_OPT");
    printf("GGML_SYCL_DISABLE_OPT=%s\n", disable_opt ? disable_opt : "(not set)");
    printf("\n");

    int passed = 0;
    int failed = 0;

    if (test_buffer_init()) passed++; else failed++;
    printf("\n");

    if (test_mul_mat_graph()) passed++; else failed++;
    printf("\n");

    if (test_prompt_decode_transition()) passed++; else failed++;
    printf("\n");

    if (test_compare_outputs()) passed++; else failed++;
    printf("\n");

    if (test_mistral_dimensions()) passed++; else failed++;
    printf("\n");

    if (test_q4_vs_q8()) passed++; else failed++;
    printf("\n");

    if (test_sequential_ops()) passed++; else failed++;
    printf("\n");

    if (test_q4_0_correctness()) passed++; else failed++;
    printf("\n");

    // New focused tests
    if (test_minimal_dimensions()) passed++; else failed++;
    printf("\n");

    if (test_single_block()) passed++; else failed++;
    printf("\n");

    if (test_soa_layout_verification()) passed++; else failed++;
    printf("\n");

    if (test_nibble_extraction()) passed++; else failed++;
    printf("\n");

    if (test_row_identification()) passed++; else failed++;
    printf("\n");

    if (test_token_embd_getrows()) passed++; else failed++;
    printf("\n");

    if (test_cpu_gpu_token_embd()) passed++; else failed++;
    printf("\n");

    // Q6_K tests (output.weight path)
    if (test_q6_k_mmvq_soa()) passed++; else failed++;
    printf("\n");

    if (test_q6_k_soa_layout()) passed++; else failed++;
    printf("\n");
    ...
    if (failed > 0) {
        ...
        printf("  # AoS mode:\n");
        printf("  GGML_SYCL_DISABLE_OPT=1 ./build/bin/test-ggml-sycl-soa\n");
    }
