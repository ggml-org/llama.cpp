...
static void test_peg_parser(common_chat_templates *                      tmpls,
    ...
    if (!parser.params_.grammar.empty()) {
        ...
        // Determine whether the matcher sampler path applies: tool-call grammar with
        // all WORD triggers and thinking tags present. In production, the matcher
        // sampler handles trigger detection while being aware of thinking blocks —
        // triggers inside <think>...</think> are suppressed.
        bool use_matcher_path = false;
        if (parser.params_.grammar_lazy && !parser.params_.thinking_end_tag.empty()) {
            use_matcher_path = true;
            for (const auto & trigger : parser.params_.grammar_triggers) {
                if (trigger.type != COMMON_GRAMMAR_TRIGGER_TYPE_WORD) {
                    use_matcher_path = false;
                    break;
                }
            }
        }
        ...
        if (use_matcher_path) {
            // Matcher sampler path: simulate thinking-aware trigger detection.
            ...
            // If the matcher path found no trigger outside thinking but the test
