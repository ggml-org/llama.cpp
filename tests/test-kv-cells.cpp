...
static void test_seq(testing & t) {
    ...
    t.test("add_sequences", [&](testing & t) {
        llama_kv_cells cells;
        cells.resize(4);
...
static void test_next_head(testing & t) {
    t.test("removes_range_of_seq", [&](testing & t) {
        ...
        t.assert_equal(0u, cells.next_head(0, 5, 8));
    ...
    t.test("returns_first_removed", [&](testing & t) {
        ...
        t.assert_equal(2u, cells.next_head(0, 0, 100));
    ...
    t.test("no_match", [&](testing & t) {
        ...
        t.assert_equal(cells.size(), cells.next_head(0, 0, 5));
        t.assert_equal(cells.size(), cells.next_head(0, 8, 100));
        // no cell has seq 2
        t.assert_equal(cells.size(), cells.next_head(2, 0, 100));
        t.assert_equal(2u, cells.get_used());
        t.assert_equal(5, cells.seq_pos_min(0));
        ...
        for (uint32_t step = 0; step < 3000; ++step) {
            ...
            switch (rng() % 12) {
                ...
                case 4: { // add_sequences
                    if (ref.cells[i].pos != -1) {
                        ...
                        if (!pick.empty()) {
                            ...
                            cells.add_sequences(i, (int32_t) k, pick.data());
                ...
                case 9: { // next_head
                    ...
                    t.assert_equal(new_head, cells.next_head(s, p0, p1));
                } break;
