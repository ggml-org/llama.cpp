...
// ============================================================================
// Test 18: rollback returns false for invalid args (no plugin loaded)
// ============================================================================
...
// ============================================================================
// Test 19: rollback succeeds with plugin (commit then rollback)
// ============================================================================
...
// ============================================================================
// Test 19: apply_bitmask returns false for invalid args (no plugin loaded)
// ============================================================================
...
// ============================================================================
// Test 20: rollback and apply_bitmask return false after failed init
// ============================================================================
...
// ============================================================================
// Test 21: set_language/get_language are scoped per slot, not plugin-wide
// ============================================================================
...
static void test_set_language_is_per_slot() {
    ...
    // Two non-default slots get different bundled languages.
    assert(llama_deterministic_draft_set_language(draft, 5, "python"));
    assert(llama_deterministic_draft_set_language(draft, 7, "c"));

    assert(strcmp(llama_deterministic_draft_get_language(draft, 5), "python") == 0);
    assert(strcmp(llama_deterministic_draft_get_language(draft, 7), "c") == 0);

    // Changing the default (slot -1) must not clobber already-configured
    // slots 5 and 7.
    assert(llama_deterministic_draft_set_language(draft, -1, "java"));
    assert(strcmp(llama_deterministic_draft_get_language(draft, 5), "python") == 0);
    assert(strcmp(llama_deterministic_draft_get_language(draft, 7), "c") == 0);

    // A brand-new slot touched after the default changes inherits it.
    uint32_t bitmask[(TEST_VOCAB_SIZE + 31) / 32];
    llama_deterministic_draft_fill_bitmask(draft, 42, bitmask, TEST_VOCAB_SIZE);
    assert(strcmp(llama_deterministic_draft_get_language(draft, 42), "java") == 0);
...
// ============================================================================
// Test 22: bootstrap detection resolves an unconfigured slot's language
// from its content alone (no set_language/set_grammar/env var at all)
// ============================================================================
...
static void test_bootstrap_detection_resolves_language() {
    ...
    assert(strcmp(llama_deterministic_draft_get_language(draft, 1), "python") == 0);

    const std::string java_code = "public class Foo {\n    public static void main(String[] args) {}\n}\n";
    int n2 = feed_bytes(draft, 2, java_code);
    assert(n2 == (int) java_code.size());
    assert(strcmp(llama_deterministic_draft_get_language(draft, 2), "java") == 0);
...
// ============================================================================
// Test 23: "c" is tried last during bootstrap detection - critical, see
// list_bundled_languages() in plugin.cpp. Without this, C's permissive
...
static void test_bootstrap_detection_tries_c_last() {
    ...
    // requirement to wrap top-level code in anything) accepts it too. If
    // "c" were tried before "javascript", it would never reject a single
    // byte, fallthrough would never trigger, and detection would
    // incorrectly settle on "c".
    const std::string js_code = "const x = 1;\nlet y = 2;\nfunction add(a, b) { return a + b; }\n";
    int n = feed_bytes(draft, 3, js_code);
    assert(n == (int) js_code.size());
    assert(strcmp(llama_deterministic_draft_get_language(draft, 3), "javascript") == 0);
...
// ============================================================================
// Test 24: state serialization round-trip with an explicit language
// (save -> reset -> restore must reproduce the exact grammar state)
// ============================================================================
...
static void test_state_serialization_roundtrip() {
    ...
    assert(draft != nullptr);
    assert(set_byte_vocab(draft));
    assert(llama_deterministic_draft_set_language(draft, 0, "c"));
...
// ============================================================================
// Test 25: state serialization round-trip while bootstrap detection is
...
static void test_state_serialization_detecting() {
    ...
    assert(llama_deterministic_draft_is_detecting(draft, 1));
    ...
    // still detecting, same active candidate, same grammar state
    assert(llama_deterministic_draft_is_detecting(draft, 1));
    std::vector<uint32_t> after((256 + 31) / 32);
    assert(llama_deterministic_draft_fill_bitmask(draft, 1, after.data(), 256));
    assert(before == after);

    // the restored matcher is live: continuing the python function resolves
    // detection to python
...
// ============================================================================
// Test 26: common-level checkpoint round-trip through
...
static void test_common_checkpoint_roundtrip() {
    ...
    assert(det != nullptr);
    assert(llama_deterministic_draft_set_language(det, 0, "c"));

    // build grammar state with real vocab tokens (valid C)
...
// ============================================================================
// Test 27: get_jump_forward - determined continuation once the grammar
...
static void test_get_jump_forward() {
    ...
    struct llama_deterministic_draft * draft = llama_deterministic_draft_init(plugin_path.c_str());
    assert(draft != nullptr);
    assert(llama_deterministic_draft_set_vocab(draft, TEST_VOCAB, TEST_VOCAB_SIZE, nullptr, 0));
    assert(llama_deterministic_draft_set_grammar(draft, TEST_GRAMMAR, "root"));
    llama_deterministic_draft_reset(draft, 0);

    // both alternatives still possible: nothing is uniquely determined
    int32_t len = -1;
    assert(llama_deterministic_draft_get_jump_forward(draft, 0, &len) == nullptr);
    assert(len == 0);

    // "int main() {\n    " leaves only the first alternative: rest is determined
    const int tokens[] = { 0, 1, 2, 3, 4, 1, 5, 6 };
    assert(llama_deterministic_draft_filter_draft(draft, 0, tokens, 8) == 8);

    const char * jump = llama_deterministic_draft_get_jump_forward(draft, 0, &len);
    assert(jump != nullptr && len > 0);

    // the jump-forward string must be a prefix of the remaining program text
    const std::string rest = "return 0;\n}";
...
// ============================================================================
// Test 28: accept-all sampling path - draft tokens returned verbatim, the
...
static void test_accept_all_sample_and_accept() {
    ...
    common_speculative_accept(spec, 0, (uint16_t) (accepted.size() - 1));
...
// ============================================================================
// Test 29: standard (target-verified) mode - the final token of each step
...
static void test_standard_mode_constrains_final_token() {
    ...
    params.speculative.deterministic_draft.enabled     = true;
    params.speculative.deterministic_draft.plugin_path = plugin_path;
    // det_accept_all stays false: standard target verification
    params.speculative.draft.ctx_tgt = ctx_tgt;
    params.speculative.draft.ctx_dft = ctx_dft.get();
    ...
    // mirror common_speculative_begin(): reset and commit the prompt
    llama_deterministic_draft_reset(det, 0);
    assert(llama_deterministic_draft_set_language(det, 0, "c"));
    ...
    const llama_token greedy = common_sampler_sample(smpl, ctx_tgt, idxs.back());
    llama_tokens draft;
    ...
    // verified against a fresh pinned plugin instance as ground truth
    ...
    assert(llama_deterministic_draft_set_language(verifier, 0, "c"));
