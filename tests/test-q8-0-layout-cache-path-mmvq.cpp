// Targeted test: ensure SoA layout pointer is used from unified cache for MMVQ (batch > 1).
// Strategy:
// 1) Upload AoS weights and force SoA layout caching.
// 2) Mutate AoS host data after caching.
// 3) Run mul_mat with batch > 1 and verify output matches original weights (not mutated AoS).
...
static void compute_reference(const block_q8_0_test * weights, const float * input, float * output,
    ...
    for (int b = 0; b < batch; ++b) {
        const float * input_col = input + b * ncols;
        float * out_col = output + b * nrows;
        for (int row = 0; row < nrows; ++row) {
            ...
            for (int blk = 0; blk < blocks_per_row; ++blk) {
                ...
                const float * x = input_col + blk * QK8_0;
                for (int j = 0; j < QK8_0; ++j) {
                    sum += d * (float) w->qs[j] * x[j];
                }
            }
            out_col[row] = sum;
        }
...
int main() {
    ...
    ggml_set_name(weight, "cache_weight_q8_0_mmvq");
    ggml_tensor * input = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, ncols, batch);
    ggml_set_name(input, "cache_input_mmvq");
    ...
    std::vector<float> ref_output(nrows * batch, 0.0f);
    compute_reference(weight_data.data(), input_data.data(), ref_output.data(), ncols, nrows, batch);
