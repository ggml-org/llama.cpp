...
// llama.h header does not pull in. We need it here to assert the
// per-scope filter storage contract.
...
int main() {
    // ── Part 1: ggml observer tensor classification (unchanged) ──────────
    // The stats view is the half of an imatrix observer cast whose offset
    // and element count identify it as the importance accumulator; the
    // storage and activation halves are the activation path. This is the
    // contract ggml_imatrix_observer_is_stats depends on.
    {
    ...
    // llama_cparams. Setting one must not clobber the other, and the
    // active scope is the value the user last set via
    // llama_set_imatrix_observer_scope.
    {
        ...
        // The active scope is what the user last set. This is the value
        // the graph build dispatches against.
        ...
        // Filter dispatch is independent per scope. The verifier filter
        // must reject the drafter's user_data (and vice versa) so a
        // context can carry two distinct filter contracts at once.
